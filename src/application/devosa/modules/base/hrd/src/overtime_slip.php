...
doInclude('../global/form_function.php');
...
{
    extract($params);
    return "<a href=\"slip_untuk_overtime.php?dataID=" . $record['id_application'] . "&cash=t\">" . getWords(
        'get slip'
    ) . "</a>";
}
...
function getData($db)
{
    ...
    if ($db->connect() && $strDataTripTypeID != "") {
        ...
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("Overtime Date"), "overtime_date", ["rowspan" => 2], ['nowrap' => ''])
        );
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("Employee ID"), "employee_id", ["rowspan" => 2], ['nowrap' => ''])
        );
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("Name"), "employee_name", ["rowspan" => 2], ['nowrap' => ''])
        );
    ...
    foreach ($myDataGrid->checkboxes as $strDataID) {
        ...
        foreach ($dataTripDetail AS $strCurrencyCode => $arrCost) {
            if (count($arrCost) > 0) {
                ...
                $strSlipContent3 .= wrapRow(
                    getWords("total"),
                    ": (" . $strCurrencyCode . ")",
                    standardFormat($intTotal),
                    true
                );
            }
        }
...
if ($db->connect()) {
    ...
    $f->addSelect(
        ...
        ["style" => "display:none;width:$strDefaultWidthPx"],
        "",
        false
    );
