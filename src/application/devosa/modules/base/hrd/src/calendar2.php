...
function getMonthlyCalendar($db, $strMonth = "", $strYear = "", $intDefaultHeight = 60, $bolPrint = false)
{
        ...
        if (isset($arrCalendar[$intDayCounter])) {
            if (count($arrCalendar[$intDayCounter]) > 0) {
                ...
                foreach ($arrCalendar[$intDayCounter] as $cal) {
                    ...
                    $strRemarks .= "</td></tr>";
                    //$strHasil2 .="{\"title\" : \"".$cal["note"].$cal["status"].$cal["type"]."\",\"start\" : \"".$year."-".$month."-".$intDayCounter."\"},";
                    $strHasil2 .= "{\"title\" : \"" . $cal["note"] . "\",\"start\" : \"" . $year . "-" . $month . "-" . $intDayCounter . "\"},";
                }
                $strRemarks .= "</table>";
            }
        }
    ...
    $strHasil .= "</tr>";
    $strHasil .= "</table>\n";
    $strHasil2 .= "{\"title\" : \"All Day Event\",\"start\" : \"\"}]";
    //return($strHasil);
    return $strHasil2;
}// getMonthlyCalendar
