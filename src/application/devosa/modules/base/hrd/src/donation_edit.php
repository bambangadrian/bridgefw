...
doInclude('libraries/classes/hrd/hrd_donation_type.php');
doInclude('libraries/classes/hrd/hrd_donation_platform.php');
doInclude('libraries/classes/hrd/hrd_donation.php');
  ...
  */
    ...
    if ($bolCanEdit) {
        ...
        $f->addSelect(
            ...
            "dataDonationCode",
            getDataListDonationType($arrData['dataDonationCode']),
            "style='width:250px'",
            "",
            true
        );
        ...
        if ($arrData['dataDonationCode'] != "" && $total > 0) {
            ...
            $arrQuota = $tblDonationPlatform->find(
                "donation_code = '" . $arrData['dataDonationCode'] . "' AND grade_code = '" . $strGradeCode . "'",
                "amount"
            );
            ...
            $f->addSelect(
                getWords(
                    "relation name <br><small><small><i> leave empty if donation is for employee</i></small></small><br>"
                ),
                "dataRelationName",
                getDataListEmployeeFamily(
                    $arrData['dataRelationName'],
                    true,
        ...
        $f->addInput(
            getWords("event date from"),
            "dataEventDateFrom",
            $arrData['dataEventDateFrom'],
        ...
        $f->addInput(
            getWords("event date thru"),
            "dataEventDateThru",
            $arrData['dataEventDateThru'],
            ["style" => "width:$strDateWidth"],
            "date",
            true,
            true,
...
function saveData()
{
    ...
    if ($db->connect()) {
        ...
        if ($total > 0) {
            ...
            $tblDonation = new cHrdDonation();
            $data = [
                "donation_code"   => $f->getValue('dataDonationCode'),
