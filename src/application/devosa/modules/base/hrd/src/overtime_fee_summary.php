...
doInclude('libraries/classes/hrd/hrd_overtime_fee_master.php');
doInclude('libraries/classes/hrd/hrd_overtime_fee_detail.php');
...
if ($bolCanEdit) {
    ...
    $f->addInput(
        getWords("date from"),
        "dataDateFrom",
        (substr(getNextDateNextMonth(date("Y-m-d"), -1), 0, 8) . "01"),
    ...
    $f->addInput(
        getWords("date thru"),
        "dataDateThru",
        (date("Y-") . addPrevZero((date("m") - 1), 2) . "-" . lastday(date("m") - 1, date("Y"))),
        ["style" => "width:$strDateWidth"],
        "date",
        true,
        true,
    ...
    $f->addSelect(
        getWords("salary month (for outdated OT)"),
        "dataSalaryMonth",
        getDataListMonth(date("m")),
        [],
        "numeric",
        true,
        true,
    ...
    $f->addSelect(
        getWords("salary year (for outdated OT)"),
        "dataSalaryYear",
        getDataListYear(date("Y")),
        [],
        "numeric",
        true,
        true,
...
$myDataGrid->addColumn(new DataGrid_Column(getWords("date from"), "date_from", ['width' => '150'], ['nowrap' => '']));
$myDataGrid->addColumn(new DataGrid_Column(getWords("date thru"), "date_thru", ['width' => '150'], ['nowrap' => '']));
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("salary month"), "salary_month", ['width' => '150'], ['nowrap' => ''])
);
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("salary year"), "salary_year", ['width' => '150'], ['nowrap' => ''])
);
...
function printDetailLink($params)
{
    ...
    if ($bolPrint) {
        ...
        return generateButton(
            ...
            getWords("open"),
            "",
            "onclick =\"document.formData.dataIDMaster.value = '" . $value . "'; document.formData.action = 'overtime_fee_detail.php'; document.formData.submit();\""
        );
    }
}
...
function saveData()
{
    ...
    if ($isNew) {
        // data baru
        if ($tblOvertimeFeeMaster->findCount(
                [
                    "id_company"   => $f->getValue('dataCompany'),
                    "date_from"    => $f->getValue('dataDateFrom'),
                    "date_thru"    => $f->getValue('dataDateThru'),
            ...
            generateOvertimeFeeDetail(
                $tblOvertimeFeeMaster->getLastInsertId(),
                $f->getValue('dataCompany'),
                $f->getValue('dataDateFrom'),
                $f->getValue('dataDateThru'),
                $f->getValue('dataSalaryMonth'),
                $f->getValue('dataSalaryYear')
            );
            $f->message = $tblOvertimeFeeMaster->strMessage;
        }
    } else {
        $bolSuccess = $tblOvertimeFeeMaster->update(/*pk*/
            "id='" . $f->getValue('dataIDMaster') . "'", /*data to update*/
            $data
        );
        $f->message = $tblOvertimeFeeMaster->strMessage;
    }
...
function generateOvertimeFeeDetail(
    $strIDMaster,
    $strDataCompany,
    $strDateFrom,
    $strDateThru,
...
)
{
    ...
    if ($db->connect() && $strIDMaster != "") {
        ...
        */
        ...
        $resDb = $db->execute($strSQL);
        while ($rowDb = $db->fetchrow($resDb)) {
            $arrOTTransport[$rowDb['id_employee']] = (is_numeric(
                $rowDb['transport_allowance']
            )) ? $rowDb['transport_allowance'] : 0;
        }
        ...
        */
            ...
            */
            if (isset($arrOTTransport[$strIDEmployee])) {
                $fltTransport = $arrOTTransport[$strIDEmployee];
            }
            /*
            Breakfast Allowance
            Untuk karyawan yang berhak Full OT dan datang minimal 1 jam seelum normal start
            */
            if (isset($arrOTBreakfast[$strIDEmployee])) {
                $fltBreakfast = $arrOTBreakfast[$strIDEmployee];
            }
            $strSaveSQL .= "
