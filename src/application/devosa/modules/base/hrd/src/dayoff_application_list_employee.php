...
function getData($db, $strDateFrom, $strDateThru, &$intRows, $strKriteria = "", $strOrder = "")
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        if ($rowDb['status'] == REQUEST_STATUS_APPROVED || $bolCanApprove) //$strResult .= "    <a href=dayoff_application_edit.php?dataID=" .$rowDb['id_application']. ">" .getWords('use');
        {
            $strResult .= "    <a href=dayoff_realization.php?dataID=" . $rowDb['id_application'] . ">" . getWords(
                    'use'
                );
        } else {
        ...
        if ($rowDb['status'] == REQUEST_STATUS_NEW || $bolCanApprove) {
            $strResult .= "    <a href=dayoff_application_edit.php?dataID=" . $rowDb['id_application'] . ">" . getWords(
                    'edit'
                );
            ...
            if ($rowDb = $db->fetchrow($resDb)) {
                ...
                if ($rowDb['status'] < $intStatus) {
                    ...
                    writeLog(
                        ACTIVITY_EDIT,
                        MODULE_EMPLOYEE,
                        $rowDb['employee_name'] . " - " . $rowDb['do_date'],
                        $intStatus
                    );
                }
            }
...
if ($db->connect()) {
    ...
    if (isset($_REQUEST['btnDelete'])) {
        ...
        if ($bolCanEdit) {
            changeStatus($db, REQUEST_STATUS_VERIFIED);
        }
