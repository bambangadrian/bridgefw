...
if ($bolIsEmployee && !isset($_REQUEST['dataEmpId'])) {
    header("location:candidate_edit_internal.php?dataEmpId=" . $arrUserInfo['employee_id']);
}
...
if ($strDataID == "" && $strDataEmployeeID != "") {
    syncArrData($arrData, $db, $strDataEmployeeID);
} //sinkronisasi dgn employee, 1/2 hardcode
...
function getData($strDataID)
{
    ...
    if ($isCandidate) {
        ...
        if ($arrData['candidate_entry_date'] == "") {
            $arrData['candidate_entry_date'] = "";
        }
    }
...
function saveData()
{
    ...
    if (saveDataCandidate()) {
        ...
        $resExec = $db->execute($strSQL);
        header(
            "location: candidate_edit.php?dataID=" . $f->getValue('dataID') . "&message=" . $f->message
        );//."&URL_REFERER=".$f->getValue("URL_REFERER"));
        exit();
    }
    return false;
}
