...
doInclude('../global/approval_func.php');
    ...
    if ($strDataID == "") {
        ...
        if ($rowDb = $db->fetchrow($resDb)) {
            ...
            if ($rowDb['status'] < 2) { //belum diapprove
                ...
                $strInputNote = ($rowDb['status'] == 1) ? getWords('revision by dept. head') : getWords(
                    "revision by employee"
                );
                $strInputNote .= " --- " . $rowDb['note'];
            ...
            if ($bolCanView) {
                $strDataDetail = ($bolPrint) ? getDataPrint(
                    $db,
                    $intTotalData,
                    $strKriteria,
                    "",
