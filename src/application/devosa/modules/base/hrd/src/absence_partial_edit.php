...
function getAttendance($db, $ajDate, $ajEmployee, $ajType)
{
    ...
    if ($ajType == 0) {
        return "0|" . substr($data['normal_start'], 0, 5) . "|" . substr(
...
if ($db->connect()) {
    ...
    if ($strDataID != "") {
        ...
        $arrData['dataDate'] = (getPostValue('dataDate') != "") ? getPostValue('dataDate') : date("m/d/Y");
    ...
    if ($bolCanEdit) {
        ...
        //  $f->addInputAutoComplete(getWords("employee ID"), "dataEmployee", getDataEmployee($strDataEmployee), "". $strReadonly, "string", true, true, true, "", "", true, null, "hrd_ajax_source.php?action=getemployee");
        ...
        $f->addSelect(
            ...
            true,
            "",
            generateButton("btnGetInfo", getWords("get attendance info"), "", "onClick=\"getAttendanceInfo()\"")
        );
        $f->addInput(
            getWords("start time"),
            "dataStartTime",
            $strDataStartTime,
            ["size" => 10, "maxlength" => 5, "class" => "t input_mask mask_time", "onBlur" => "setDuration();"],
            "string",
            false,
            true,
            true,
        ...
        $f->addInput(
            ...
            ["size" => 10, "maxlength" => 5, "class" => "t input_mask mask_time", $strReadonly],
            "string",
            false,
            true,
            true
