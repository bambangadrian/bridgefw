...
doInclude('libraries/classes/hrd/hrd_destination.php');
...
if ($bolCanEdit) {
    ...
    $f->addInput(getWords("destination code"), "dataDestinationCode", "", ["size" => 20], "string", true, true, true);
    $f->addInput(getWords("destination name"), "dataDestinationName", "", ["size" => 50], "string", true, true, true);
    $f->addSubmit(
        "btnSave",
        getWords("save"),
...
$myDataGrid->addColumnCheckbox(
    new DataGrid_Column("chkID", "destination_code", ['width' => '30'], ['align' => 'center', 'nowrap' => ''])
);
$myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", ['width' => '30'], ['nowrap' => '']));
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("destination code"), "destination_code", ['width' => '150'], ['nowrap' => ''])
);
$myDataGrid->addColumn(new DataGrid_Column(getWords("destination name"), "destination_name", "", ['nowrap' => '']));
if ($bolCanEdit) {
    $myDataGrid->addColumn(
        new DataGrid_Column(
            "",
...
function saveData()
{
    ...
    $data = [
        "destination_code" => $f->getValue('dataDestinationCode'),
    ...
    if ($isNew) {
        ...
        $bolSuccess = $dataHrdDestination->update(/*pk*/
            "destination_code='" . $f->getValue('dataID') . "'", /*data to update*/
            $data
        );
    }
    if ($bolSuccess) {
        $f->setValue('dataID', $data['destination_code']);
    }
