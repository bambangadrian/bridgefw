...
$myDataGrid->addColumn(new DataGrid_Column(getWords("grade code"), "grade_code", ['width' => '150'], ['nowrap' => '']));
foreach ($dataDonationType AS $strDonationCode => $arrDonationDetail) {
    $myDataGrid->addColumn(
        new DataGrid_Column(
            getWords($arrDonationDetail['name']),
            "amount_" . $strDonationCode,
            ['width' => '75'],
            ['align' => 'center'],
            false,
            false,
...
foreach ($dataset AS $strGradeCode => $arrDetail) {
    ...
    $dataDonationPlatform = $tblDonationPlatform->findAll(
        "grade_code = '" . $strGradeCode . "'",
        "grade_code, donation_code, amount",
        "",
        null,
        1,
        "donation_code"
...
function printQuota($params)
{
    ...
    return generateInput(
        "detailQuota_" . $record['grade_code'] . "_" . $strDonationCode,
        $value,
    ...
    foreach ($dataSalaryGrade AS $strGradeCode => $arrGradeDetail) {
        ...
        foreach ($dataDonationType AS $strDonationCode => $arrDonationDetail) {
            ...
            if (isset($arrData['detailQuota_' . $strGradeCode . '_' . $strDonationCode])) {
                ...
                $tblDonationPlatform->delete(["donation_code" => $strDonationCode, "grade_code" => $strGradeCode]);
