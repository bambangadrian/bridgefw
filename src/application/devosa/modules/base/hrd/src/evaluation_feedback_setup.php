...
doInclude('libraries/classes/hrd/hrd_evaluation_feedback_criteria.php');
...
if ($bolCanEdit) {
    ...
    $f->addLiteral(
        "",
        "",
        "<table border=0><tr><td>" . generateRadio(
            "dataForEvaluator",
            "t",
            "checked"
        ) . "</td><td>" . getWords("for evaluator") . "</td></tr><tr><td>" . generateRadio(
            "dataForEvaluator",
            "f"
        ) . "</td><td>" . getWords("for employee") . "</td></tr></table>"
    );
...
function saveData()
{
    ...
    if ($bolSave) {
        ...
        if ($isNew) {
            ...
            $bolSuccess = $tblHrdEvaluationFeedbackCriteria->update(/*pk*/
