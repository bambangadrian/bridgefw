...
doInclude('libraries/classes/hrd/hrd_evaluation_category.php');
doInclude('libraries/classes/hrd/hrd_evaluation_criteria.php');
doInclude('libraries/classes/hrd/hrd_evaluation_criteria_employee.php');
doInclude('libraries/classes/hrd/hrd_evaluation_detail.php');
doInclude('libraries/classes/hrd/hrd_evaluation_master.php');
doInclude('libraries/classes/hrd/hrd_evaluation_feedback.php');
doInclude('plugins/tbs-3.7.0plugins/opentbs/tbs_plugin_opentbs.php');
doInclude('plugins/tbs-3.7.0plugins/excel/tbs_plugin_excel.php');
//---- INISIALISASI ----------------------------------------------------
...
if ($db->connect()) {
    ...
    if ($strDataID != "") {
        ...
        $dataEvaluationMaster = $tblEvaluationMaster->findAll(
            "id = $strDataID",
            "id, id_employee, id_evaluator, evaluation_period_from, evaluation_period_thru",
            "",
            null,
            1,
            "id"
        );
        if (!isMe($dataEvaluationMaster[$strDataID]['id_evaluator'])) {
            redirectPage("evaluation_view.php?dataID=$strDataID");
        } else {
    ...
    $arrEmployee = [];
    if ($strDataEmployee != "") {
        $arrEmployee = getEmployeeInfoByCode(
            $db,
            $strDataEmployee,
            "id , employee_name, grade_code, division_code, department_code, section_code, sub_section_code, hrd_employee.position_code, position_group, due_date, permanent_date, join_date, employee_status "
        );
        $infoEmployee = $arrEmployee['id'] . " - " . $arrEmployee['employee_name'];
        //contract or permanent
        if ($arrEmployee['employee_status'] == STATUS_CONTRACT) {
            $infoEmployee .= " (Contract from " . $arrEmployee['join_date'] . " until " . $arrEmployee['due_date'] . ")";
        }
        if ($arrEmployee['employee_status'] == STATUS_PERMANENT) {
            $infoEmployee .= " (Permanent on " . $arrEmployee['permanent_date'] . ")";
        }
    ...
    $f->addInput("to", "dataDateThru", $strDataDateThru, ["style" => "width:$strDateWidth"], "date", true, true, true);
    ...
    $formFilter = $f->render();
    $isNew = ($f->getValue('dataID') == "" && !checkEvaluationRecord(
            $strDataIDEvaluator,
            $strIDEmployee,
            $strDataDateFrom,
            $strDataDateThru
    ...
    if ($strIDEmployee != "" /*&& isValidEvaluation()*/) {
        ...
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("key result area"), "criteria", "", "", false, false, "", "", "", true, 50)
        );
        $myDataGrid->addColumn(
            new DataGrid_Column(
                getWords("target"),
        ...
        $myDataGrid->addColumn(
            new DataGrid_Column(
                getWords("accomplished"),
                "accomplishment",
                "",
                ['align' => 'center', 'nowrap' => '', 'width' => '85'],
                false,
                false,
                "",
                "",
        ...
        if ($strDataID != "") {
            ...
            $dataEvaluationDetail = $tblEvaluationDetail->findAll(
                ["id_evaluation" => $strDataID],
                "id_criteria || '_' || id_category as id_key, id_criteria, id_category, achieved, score, var_score, result",
                "id_criteria",
                null,
                null,
                "id_key"
    ...
    if ($strDataID != "" || getRequestValue('btnExcel2')) //    if ($strDataID != "" || $strDataID == "")//khusus fujiko
    {
        ...
        $myDataGrid2->addColumnNumbering(new DataGrid_Column("No", "", "", ['width' => '30', 'nowrap' => '']));
        $myDataGrid2->addColumn(
            new DataGrid_Column(getWords("category"), "category", "", ['width' => '48%', 'nowrap' => ''], false, false)
        );
        $myDataGrid2->addColumn(
            new DataGrid_Column(
                getWords("weight") . " (%)",
        ...
        $myDataGrid2->addColumn(new DataGrid_Column(getWords("score"), "score", "", ['nowrap' => ''], false, false));
        $myDataGrid2->addColumn(
            new DataGrid_Column(
                getWords("result"),
                "result",
        ...
        $myDataGrid3->addColumnNumbering(new DataGrid_Column("No", "", "", ['width' => '30', 'nowrap' => '']));
        ...
        $myDataGrid3->addColumn(
            new DataGrid_Column(
                getWords("question"), "question", "", ['width' => '48%'], false, false, "", "", "", true, 50
            )
        );
        $myDataGrid3->addColumn(
            new DataGrid_Column(
        ...
        $myDataGrid3->addButton(
            "btnSave",
            "btnSave",
            "button",
            getWords("save"),
            "onClick=\"document.formData.btnSave.click();\""
        );
        $dataset3 = $myDataGrid3->getData($db, $strSQL);
        $tblEvaluationFeedback = new cHrdEvaluationFeedback();
        $dataEvaluationFeedback = $tblEvaluationFeedback->findAll(
            ["id_evaluation" => $strDataID],
            null,
            "for_evaluator, sequence",
            null,
            null,
            "question"
        );
        foreach ($dataset3 as $key => $arrDetailFeedback) {
            $intKey = $key + 1;
            $myDataGrid->strAdditionalHtml .= generateHidden(
                "detailQuestion_" . $intKey,
                $arrDetailFeedback['question']
            );
            if (isset($dataEvaluationFeedback[$arrDetailFeedback['question']])) {
                ...
                $myDataGrid->strAdditionalHtml .= generateHidden(
                    "detailAnswer_" . $intKey,
                    $dataEvaluationFeedback[$arrDetailFeedback['question']]['answer']
                );
            } else {
        ...
        $myDataGrid4->setAJAXCallBackScript(basename($_SERVER['PHP_SELF']));
        $myDataGrid4->setPageLimit("all");
        $myDataGrid4->addColumnNumbering(new DataGrid_Column("No", "", "", ['width' => '30', 'nowrap' => '']));
        //$myDataGrid3->addColumn(new DataGrid_Column(getWords("category"), "for_evaluator",  "", array('width' => '50', 'nowrap' => ''), false, false, "","printFeedbackCategory()", "", true, 30));
        $myDataGrid4->addColumn(
            new DataGrid_Column(
                getWords("question"), "question", "", ['width' => '48%'], false, false, "", "", "", true, 50
            )
        );
        ...
        $dataEvaluationFeedbackEmployee = $tblEvaluationFeedbackEmployee->findAll(
            ["id_evaluation" => $strDataID, "for_evaluator" => 'false'],
            null,
            "for_evaluator, sequence",
            null,
            null,
        ...
        foreach ($dataset4 as $key => $arrDetailFeedback) {
            $intKey = $key + 1;
            $myDataGrid->strAdditionalHtml .= generateHidden(
                "detailQuestionEmployee_" . $intKey,
                $arrDetailFeedback['question']
            );
            if (isset($dataEvaluationFeedbackEmployee[$arrDetailFeedback['question']])) {
                ...
                $myDataGrid->strAdditionalHtml .= generateHidden(
                    "detailAnswerEmployee_" . $intKey,
                    $dataEvaluationFeedbackEmployee[$arrDetailFeedbackEmployee['question']]['answer']
                );
            } else {
...
function printVarScoreInput($params)
{
    ...
    return generateInput(
        "detailVarScore_" . $counter,
        $value,
        "size=3 maxlength=3 onChange=\"$('detailResult_'+" . $counter . ").value = this.value * " . $record['weight'] . "* " . ($record['category_weight'] / 10000) . "\";"
    );
}
...
function printScoreInput($params)
{
    extract($params);
    $arrScore = [];
    $arrScore[] = ['value' => 0, 'text' => ''];
    for ($i = 1; $i <= 5; $i++) {
        $arrScore[] = [
            'value' => $record['score' . $i],
            'text'  => $record['score' . $i] . " - " . $record['score' . $i . '_note']
        ];
    }
    return generateSelect(
        "detailScore_" . $counter,
        $arrScore,
        $value,
        "style=\"width:250\" onChange=\"$('detailVarScore_'+" . $counter . ").value = this.value; $('detailResult_'+" . $counter . ").value = this.value * " . $record['weight'] . "* " . ($record['category_weight'] / 10000) . "\";  "
    );
}
...
function printFeedbackAnswer($params)
{
    ...
    return generateTextArea(
        "detailFeedbackAnswer_" . $counter,
        $value,
        "cols=110 maxlength=255 onChange=\"$('detailAnswer_'+" . $counter . ").value = this.value;\""
    );
}
...
function printFeedbackAnswerEmployee($params)
{
    extract($params);
    return generateTextArea(
        "detailFeedbackAnswerEmployee_" . $counter,
        $value,
        "cols=110 maxlength=255 onChange=\"$('detailAnswerEmployee_'+" . $counter . ").value = this.value;\""
    );
}
...
function assignIDEmployeeForValidScope()
{
    global $f;
    global $arrEmployee;
    if ($f->getValue('dataDivision') != "" && $f->getValue('dataDivision') != $arrEmployee['division_code']) {
        $strIDEmployee = "";
    } else if ($f->getValue('dataDepartment') != "" && $f->getValue(
            'dataDepartment'
        ) != $arrEmployee['department_code']
    ) {
        $strIDEmployee = "";
    } else if ($f->getValue('dataSection') != "" && $f->getValue('dataSection') != $arrEmployee['section_code']) {
        $strIDEmployee = "";
    } else if ($f->getValue('dataSubSection') != "" && $f->getValue(
            'dataSubSection'
        ) != $arrEmployee['sub_section_code']
    ) {
        $strIDEmployee = "";
    }
}
...
function checkEvaluationRecord($strIDEvaluator, $strIDEmployee, $strDateFrom, $strDateThru)
{
    global $strDataID;
    $tblEvaluationMaster = new cHrdEvaluationMaster();
    $dataEvaluationMaster = $tblEvaluationMaster->find(
        [
            "id_evaluator"           => $strIDEvaluator,
            "id_employee"            => $strIDEmployee,
            "evaluation_period_from" => $strDateFrom,
            "evaluation_period_thru" => $strDateThru
        ],
        "id"
    );
    if ($strDataID != "") {
        return true;
    } else if (count($dataEvaluationMaster) == 0) {
        return false;
    } else {
...
function isValidEvaluation()
{
    ...
    $dataWeight = $tblEvaluationCriteriaEmployee->find(
        [
            "id_employee"     => $strIDEmployee,
            "date_from"       => $strDataDateFrom,
            "date_thru"       => $strDataDateThru,
            "active"          => 't',
            "is_last_updated" => 't'
        ],
        "SUM(weight) as total_weight"
    );
    if ($dataWeight['total_weight'] != 100) {
        return false;
    } else {
        ...
        if ($dataWeight['total_weight'] != 100) {
            ...
            $dataCategory = $tblCategory->findAll(
                "individual_evaluation = 'f'",
                "id, category, weight, individual_evaluation",
                "sequence",
                null,
                null,
                "id"
            ...
            foreach ($dataCategory as $strIDCategory => $arrCategoryDetail) {
                $dataWeight = $tblEvaluationCriteria->find(
                    [
                        "id_category" => $strIDCategory,
                        "managerial"  => $strManagerial,
                        "date_from"   => $strDataDateFrom,
                        "date_thru"   => $strDataDateThru,
                    ],
                    "SUM(weight) as total_weight"
...
function calculateScore()
{
    ...
    for ($i = 1; $i <= $myDataGrid->totalData; $i++) {
        ...
        if ($arrCategoryScore[$myDataGrid->dataset[$i - 1]['id_category']]['score'] == 0) {
            $arrCategoryScore[$myDataGrid->dataset[$i - 1]['id_category']]['score'] = '';
        }
    }
    foreach ($arrCategoryScore as $strIDCategory => $arrCategoryDetail) {
        $arrCategoryScore[$strIDCategory]['result'] = /*$arrCategoryScore[$strIDCategory]['weight']/100 * */
            $arrCategoryDetail['result'];
...
function saveData()
{
    ...
    if ($db->connect()) {
        ...
        $tblEvaluationMaster = new cHrdEvaluationMaster();
        $data = [
            "id_evaluator"           => getPostValue('dataIDEvaluator'),
        ...
        // hapus data lama, insert data baru
        $tblEvaluationFeedback = new cHrdEvaluationFeedback();
        $tblEvaluationFeedback->delete(["id_evaluation" => $strDataID, "for_evaluator" => "t"]);
        ...
        // hapus data lama, insert data baru
        $tblEvaluationFeedbackEmployee = new cHrdEvaluationFeedback();
        $tblEvaluationFeedbackEmployee->delete(["id_evaluation" => $strDataID, "for_evaluator" => "f"]);
