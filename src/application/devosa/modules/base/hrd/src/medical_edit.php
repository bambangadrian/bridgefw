...
function getData($db, &$intRows, $strDataID = "")
{
    ...
    if ($bolSave && $bolError) {
        ...
        for ($i = 1; $i <= $intShown; $i++) {
            ...
            (isset($_REQUEST['detailMedicalDate' . $i]) && validStandardDate(
                    $_REQUEST['detailMedicalDate' . $i]
                )) ? $strMedicalDate = $_REQUEST['detailMedicalDate' . $i] : $strMedicalDate = date("Y-m-d");
            (isset($_REQUEST['detailMedicalDateThru' . $i]) && validStandardDate(
                    $_REQUEST['detailMedicalDateThru' . $i]
                )) ? $strMedicalDateThru = $_REQUEST['detailMedicalDateThru' . $i] : $strMedicalDateThru = date(
                "Y-m-d"
            );
            (isset($_REQUEST['detailClaimDate' . $i]) && validStandardDate(
                    $_REQUEST['detailClaimDate' . $i]
                )) ? $strClaimDate = $_REQUEST['detailClaimDate' . $i] : $strClaimDate = date("Y-m-d");
...
function saveData($db, &$strError)
{
    ...
    if ($bolOK && $strIDEmployee != "") {
        ...
        for ($i = 1; $i <= $intMax; $i++) {
            ...
            $strYear = (isset($_REQUEST['detailClaimDate' . $i]) and validStandardDate(
                    $_REQUEST['detailClaimDate' . $i]
                )) ? $_REQUEST['detailClaimDate' . $i] : "";
...
if ($db->connect()) {
    ...
    if ($bolPrint) {
        ...
        $strInputEmployee = "<input class=\"form-control string\" type=text name=dataEmployee id=dataEmployee size=15 maxlength=30 value=\"" . $arrData['dataEmployee'] . "\" $strNonCbReadonly >";
...
$strPageDesc = getWords("medical edit form");
$pageHeader = pageHeader($pageIcon, $strPageTitle, $strPageDesc);
$pageSubMenu = medicalQuotaSubmenu($strWordsInputMedicalClaim);
if ($bolPrint) {
