...
doInclude('../global/excelReader/excel_reader.php');
...
{
    global $data, $lokasiFile, $tipeFile, $intNumberOfField, $strNormalStartTime, $strNormalFinishTime, $strInputCompany, $strDateFrom, $strDateThru, $db;
    ...
    for ($i = 2; $i <= $baris; $i++) {
        $strFingerID = $data->val($i, 2); // Finger ID
        ...
        //$strIdEmployee  = getIdEmployeeByFingerid($strFingerID); // Ambil informasi ID dengan Select dari fingerid
        $strIdEmployee = getIdEmployee2($strFingerID); // Ambil informasi ID dengan Select dari NIK (finger id =NIK)
...
function saveDataAttendance($arrData, $strInputCompany, $strDateFrom, $strDateThru)
{
    ...
    foreach ($arrData AS $key => $value) {
        ...
        $strMessage .= $i . ":" . $value['finger'] . "|" . $value['nik'] . "|" . $strDate . "|" . $value['in'] . "|" . $value['out'] . "<br/>";
        if ($strIdEmployee != "") {
...
function getIdEmployeeByFingerid($fingerId)
{
    global $db;
    if ($db->connect()) {
        $strSQL = "SELECT id From hrd_employee WHERE barcode= '" . $fingerId . "' ";
        $res = $db->execute($strSQL);
        $arrData = $db->fetchrow($res);
        $strID = $arrData['id'];
    }
