...
$f->addInput(getWords("email"), "email", $arrData['email'], ["size" => 30], "string", false, true, true);
...
$f->addSelect(
    getWords("transport"),
    "transport",
    getDataListTransport($arrData['transport'], true, $emptyData),
    [],
    "string",
    false,
    true,
...
$f->addSelect(
    ...
    "family_status_code",
    getDataListFamilyStatus($arrData['family_status_code'], true, $emptyData),
    [],
    "string",
    true,
    true,
...
if ($arrData['file_cv']) {
    ...
    $f->addLabel(
        getWords("Your CV"),
        "",
        "<a href=cv/" . $arrData['file_cv'] . " target=_blank >" . $arrCV[2] . "</a>"
    );
    $f->addFile(getWords("change cv"), "dataCv", "", ['size' => 40], "string", false);
} else {
        ...
        if (is_array($arrResult)) {
            ...
            <tr name='dataLanguageRow$counter' id='dataLanguageRow$counter'>";
              ...
              <td nowrap>
                ...
                $strResult .= "
              <td nowrap align=center>" . generateLanguangeSkillToRadioButton(
                        $arrLanguageSkill,
                        'listeningSkill',
                        $counter,
                        $row['listening_skill']
...
function getQuestion($db, $strDataID)
{
    ...
    for ($i = 11; $i <= 14; $i++) {
        ...
        $strResult .= "
          <td nowrap colspan=3>" . generateTextArea(
                "answer" . $i . "_1",
                $arrData[$i]['answer1'],
                "style='width: 100%'"
            ) . "</td>
    ...
    if (!isset($arrData[$i]['answer3'])) {
        $arrData[$i]['answer3'] = "";
    }
    if (!isset($arrData[$i]['answer4'])) {
        $arrData[$i]['answer4'] = "";
    }
    $strResult .= "
          <td nowrap colspan=3>
...
{
    $arrTransport = ['Mobil' => 'Mobil', 'Motor' => 'Motor'];
