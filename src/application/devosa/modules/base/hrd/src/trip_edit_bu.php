...
doInclude('libraries/classes/hrd/hrd_trip_type.php');
doInclude('libraries/classes/hrd/hrd_trip_type_cost_setting.php');
doInclude('libraries/classes/hrd/hrd_trip_cost_type.php');
doInclude('libraries/classes/hrd/hrd_trip_cost_platform.php');
doInclude('libraries/classes/hrd/hrd_trip.php');
doInclude('libraries/classes/hrd/hrd_trip_detail.php');
...
if ($db->connect()) {
    ...
    $arrTripCostType = $tblTripCostType->findAll(
        "",
        "id, trip_cost_type_name, currency, '' as note",
        "trip_cost_type_name",
        null,
        1,
        "id"
    ...
    if ($strDataID != "") {
        ...
        if (!isset($_REQUEST['btnSave']) && (isset($_REQUEST['cash']))) {
            $strDataID = "";
        }
    } else {
        $_REQUEST['cash'] = 't';
        $arrData['dataTempFormCode'] = $arrData['dataFormCode'] = (getPostValue('dataFormCode') != "") ? getPostValue(
            'dataFormCode'
        ) : getFormCode($db, "HR.BT-", date(".m.y"), "hrd_trip");
        ...
        $arrData['dataProposalDate'] = (getPostValue('dataProposalDate') != "") ? getPostValue(
            'dataProposalDate'
        ) : date(
            "Y-m-d"
        );
    ...
    if ($bolCanEdit) {
        ...
        $f->addInput(
            ...
            "dataTempFormCode",
            $strDataFormCode,
            ["size" => 45, "readonly" => "readonly", "maxlength" => 30],
            "string",
            true,
            (!isset($_REQUEST['cash'])),
            true
        ...
        $f->addSelect(
            ...
            "dataDestination",
            getDataListDestination($strDataDestination),
            "",
            "",
            true
        );
        ...
        if ($strDataTripTypeID != "" || $strDataDateFrom == "") {
            ...
            $arrQuota = $tblTripCostPlatform->findAll(
                "id_trip_type = $strDataTripTypeID AND grade_code = '$strGradeCode'",
          ...
          onClick=\"\$(dataTripCost_" . $strCostID . ").value = (\$(dataTripCostDefault_" . $strCostID . ").value);\">&nbsp;";
                ...
                $f->addInput(
                    "",
                    "dataTripCostNote_" . $strCostID,
                    $arrTripCostType[$strCostID]['note'],
            ...
            $f->addLiteral(
                ...
                generateSubmit(
                    "btnAllowance",
                    getWords("set allowance"),
                    ["onClick" => "javascript:myClient.checkTripDate();"],
                    ""
                )
            );
            //tambah baris blank supaya semua field allowance terletak di kolom kanan
...
function saveData()
{
    ...
    if ($db->connect()) {
        ...
        $data = [
            "id_trip_type"  => $f->getValue('dataTripType'),
        ...
        $data2 = ["id_trip" => $f->getValue('dataID')];
        foreach ($arrTripCost[$f->getValue('dataTripType')] AS $strCostID) {
            ...
            $data2['amount'] = ($f->getValue('dataTripCost_' . $strCostID) != "") ? $f->getValue(
                'dataTripCost_' . $strCostID
            ) : 0;
