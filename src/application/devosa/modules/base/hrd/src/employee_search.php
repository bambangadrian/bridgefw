...
if ($db->connect()) {
    ...
    if ($strDataPosition != "") {
        $strKriteria .= "AND t0.position_code = '$strDataPosition' ";
    }
    if ($strDataFunctionalPosition != "") {
        $strKriteria .= "AND t0.functional_code = '$strDataFunctionalPosition' ";
    }
    if ($strDataGrade != "") {
...
function showData($strCriteriaMain, $strCriteriaOther, $bolLimit = true, $isFullView = false, $isExport = false)
{
    ...
    $myDataGrid->addSpannedColumn(getWords("work information"), 17);
    $myDataGrid->addColumn(
        new DataGrid_Column(
