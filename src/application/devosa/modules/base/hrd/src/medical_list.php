...
function getData($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "", $strOrder = "")
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        if (!(($strDataTreatmentType != "" || $strDataTreatmentCode != "") && $intTotal == 0)) {
            ...
            if ($intTotal > 0) {
                ...
                if (!$bolPrint) {
                    $strResult .= "  <td colspan=15 align=right><strong>" . $words['total'] . "</strong>&nbsp;</td>\n";
                } else {
                    $strResult .= "  <td colspan=14 align=right><strong>" . $words['total'] . "</strong>&nbsp;</td>\n";
                }
                $strResult .= "  <td align=right><strong>" . $strFormatter($fltTotalCost) . "</strong>&nbsp;</td>\n";
                $strResult .= "  <td align=right><strong>" . $strFormatter(
                        $fltTotalCostApproved
                    ) . "</strong>&nbsp;</td>\n";
                $strResult .= "  <td>&nbsp;</td>";
                //$strResult .= "  <td colspan=6>&nbsp;</td>\n";
                if (!$bolPrint) {
                    $strResult .= "  <td>&nbsp;</td>";
                    $strResult .= "  <td>&nbsp;</td>";
                }
    ...
    if (($strDataTreatmentType != "" || $strDataTreatmentCode != "") && $intRows > 0) {
        $strResult .= "  <tr><td colspan=15 align=right><strong>Total : </strong></td><td align=right><strong>" . $strFormatter(
                $fltGrandTotalCost
            ) . "&nbsp;</strong></td><td align=right><strong>" . $strFormatter(
                $fltGrandTotalCost
            ) . "&nbsp;</strong></td><td $strColspan>&nbsp;</td></tr></strong>";
    }
...
function getSlip($db)
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        $arrAllData[$rowDb['id_master']]['claims']['relation'][] = ($rowDb['relation'] == -1) ? "Ybs" : getwords(
            strtolower($ARRAY_FAMILY_RELATION[$rowDb['relation']])
        );
    ...
    if (!empty($arrAllData)) {
        ...
        foreach ($arrAllData AS $strIDMaster => $arrData) {
            ...
            $arrNIK = [
                substr($arrData['employee_id'], 0, 1),
                substr($arrData['employee_id'], 1, 2),
                substr($arrData['employee_id'], 3, 2),
                substr($arrData['employee_id'], 5, 3)
            ];
            ...
            if ($rowDb = $db->fetchrow($resDb)) {
                ...
                if (isProcessable($rowDb['status'], $intStatus)) {
                    ...
                    writeLog(
                        ACTIVITY_EDIT,
                        MODULE_PAYROLL,
                        $rowDb['employee_name'] . " - " . $rowDb['created'] . " - " . $rowDb['cost'],
                        $intStatus
                    );
                }
            }
...
if ($db->connect()) {
    ...
    $strInputEmployeeStatus = getComboFromArray(
    ...
    $strInputTreatmentType = getMedicalTreatmentTypeList(
        "dataTreatmentType",
        true,
        $strDataTreatmentType,
        $strEmptyOption,
        "style=\"width:$strDefaultWidthPx\""
    );
    $strInputTreatmentCode = getMedicalTreatmentCodeList(
        $db,
        "dataTreatmentCode",
        $strDataTreatmentCode,
        $strEmptyOption,
        "where code <> '0'",
        "style=\"width:$strDefaultWidthPx\""
    );
    ...
    if ($strDataBranch != "") {
        $strInfo .= "<br>" . getWords("branch") . " : " . $strDataBranch;
    }
    if ($strDataBank != "") {
        $strInfo .= "<br>" . getWords("bank") . " : " . $strDataBank;
    }
    if ($strDataDivision != "") {
        $strInfo .= "<br>" . getWords("division") . " : " . $strDataDivision;
    }
    if ($strDataDepartment != "") {
        $strInfo .= "<br>" . getWords("department") . " : " . $strDataDepartment;
    }
    if ($strDataSection != "") {
        $strInfo .= "<br>" . getWords("section") . " : " . $strDataSection;
    }
    if ($strDataSubSection != "") {
        $strInfo .= "<br>" . getWords("sub section") . " : " . $strDataSubSection;
    }
    if ($strDataCompany != "") {
        $strInfo .= "<br>" . getWords("company") . " : " . $strDataCompany;
    }
    if ($strDataEmployeeStatus != "") {
        $strInfo .= "<br>" . getWords("employee status") . " : " . $strDataEmployeeStatus;
    }
    $strInfo .= "&nbsp;<br>&nbsp;";
