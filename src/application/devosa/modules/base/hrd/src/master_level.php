...
doInclude('libraries/classes/adm/adm_menu.php');
doInclude('../global/handledata.php');
...
if ($bolCanEdit) {
    ...
    $f->addInput(getWords("level name"), "dataName", "", null, "string", true, true, true);
    ...
    $f->addSelect(getWords("Parent Level"), "dataParentID", $arrParentMenu, [], "string", false, true, true);
    ...
    $f->addButton("btnAdd", getWords("add new level"), ["onClick" => "javascript:myClient.editData(0);"]);
    $formInput = $f->render();
} else {
    $formInput = "";
}
...
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("parent Level"), "parent_menu_name", ['width' => ''], ['nowrap' => 'nowrap'])
);
if ($bolCanDelete) {
    ...
    SELECT COUNT(*) AS total 
      FROM 
        (SELECT m.*, p.name AS parent_menu_name 
          FROM adm_menu_tmp_org AS m LEFT JOIN adm_menu_tmp_org AS p
            ON m.parent_id_adm_menu = p.id_adm_menu
            WHERE m.id_adm_module = '1' 
...
foreach ($dataset as &$rowDb) {
    ...
    switch ($rowDb['menu_level']) {
        case 0 :
            $rowDb['menu_level'] = "<strong>" . getWords("Section") . "</strong>";
            break;
        case 1 :
            $rowDb['menu_level'] = getWords("Sub Section");
            break;
        default :
            $rowDb['menu_level'] = "<em>" . getWords("Sub Section level-") . ($rowDb['menu_level'] - 1) . "</em>";
    }
    if ($rowDb['visible'] == 't') {
...
$myDataGrid->caption = strtoupper(vsprintf(getWords("list of %s"), getWords("Organitation level")));
...
$strPageTitle = $dataPrivilege['menu_name'];
$strPageDesc = getWords("Organitation level Setting");;
...
function deleteData()
{
    ...
    foreach ($myDataGrid->checkboxes as $strValue) {
        $strSQL .= "DELETE FROM adm_menu_tmp_org WHERE parent_id_adm_menu = '$strValue';";
        $strSQL .= "DELETE FROM adm_menu_tmp_org WHERE id_adm_menu = '$strValue';";
    }
...
function getParentMenu($strDataModule, $default = "")
{
    $arrResult = [];
    if ($default == "") {
        $arrResult[] = ["value" => "0", "text" => "[Main Level - No Parent]", "selected" => true];
    } else {
        $arrResult[] = ["value" => "0", "text" => "[Main Level - No Parent]", "selected" => false];
    }
    $db = new CdbClass;
    if ($db->connect()) {
        $strSQL = "SELECT * FROM adm_menu_tmp_org AS m WHERE id_adm_module = '1' ORDER BY menu_level, sequence_no";
        $isFirst = true;
        $arrMenu = [];
        $res = $db->execute($strSQL);
        while ($rowDb = $db->fetchrow($res)) {
