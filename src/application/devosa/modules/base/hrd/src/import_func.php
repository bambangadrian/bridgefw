...
function processAttendance($db, $strDateFrom, $strDateTo, $arrDataAttendance)
{
    ...
    doInclude('..\global\employee_function.php');
                ...
                foreach ($arrTimeIO AS $x => $arrTime) {
                    ...
                    if ($x == 0) {
                        ...
                        if ($arrTime[1] == 0) {
                            ...
                            } else if ($objYesterday->strAttendanceFinish == "" && timeCompare(
                                    $arrTime[0],
                                    "12:00"
                                ) <= 0
                            ) {
        ...
        if ($handle != false) {
            ...
            while ($rowDb = $db->fetchrow($resDb)) {
                if ($rowDb['barcode'] != "") {
                    ...
                    $arrAtt[$rowDb['barcode']][$strTgl]['attendance_finish'] = substr(
                ...
                if (isset($arrEmp[$strKode])) {
                    ...
                    foreach ($arrWaktu AS $strTanggal => $arrTmp) {
                        ...
                        if (substr($arrShift['start_time'], 0, 5) == "24:00" && (int)substr(
                                ...
                                0,
                                2
                            ) > (int)substr($strStart, 0, 2) && (int)substr($strStart, 0, 2) < (int)substr(
                                $strNormalFinish,
                                0,
                                2
                            )
