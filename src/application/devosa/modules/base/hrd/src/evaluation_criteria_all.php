...
$db = new CdbClass;
$db->connect();
$strDataIDCategory = (getPostValue('filterIDCategory') != "") ? getPostValue('filterIDCategory') : getPostValue(
    'dataIDCategory'
);
...
$arrTemp = $dataHrdEvaluationCriteria->find(
    ["date_from" => $strDataDateFrom, "date_thru" => $strDataDateThru, "managerial" => 't'],
...
if ($bolCanEdit) {
    ...
    $f->addSelect(
        getWords("Employee Grade"),
        "dataIDLevel",
        getDataListPosition($strDataIDLevel, false, null),
        ["style" => "width:270"]
    );
    $f->addSelect(
        getWords("evaluation category"),
        "dataIDCategory",
        getDataListEvaluationCategory($strDataIDCategory, false, null, "individual_evaluation = 'f'"),
        ["style" => "width:270"]
    );
    $f->addInputAutoComplete(
        getWords("sub header"),
        "dataSubheader",
        getEvaluationSubheader(),
        ["size" => 50],
        "string",
        false
    );
    $f->addInput(getWords("weight"), "dataWeight", $fltRemainingWeight, ["size" => 5], "decimal", true, true, true);
    $f->addTextArea(
...
$fFilter->addSelect(
    getWords("category"),
    "filterIDCategory",
    getDataListEvaluationCategory($strDataIDCategory, true, null, "individual_evaluation = 'f'"),
    "",
    "",
    false
);
$fFilter->addSelect(
    getWords("Grade"),
    "filterIDLevel",
    getDataListPosition($strDataIDLevel, true, null),
    "",
    "",
    false
);
...
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("Kategori"), "category", "", ['width' => '100', 'valign' => 'top', 'nowrap' => ''])
);
$myDataGrid->addColumn(
    new DataGrid_Column(
        getWords("Emp Grade"),
...
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("sub header"), "subheader", "", ['width' => '100', 'valign' => 'top', 'nowrap' => ''])
);
$myDataGrid->addColumn(
    new DataGrid_Column(
        getWords("key result area"),
...
function getScoreTable($strDataID)
{
    ...
    if ($strDataID != "") {
        $dataHrdEvaluationCriteria = new cHrdEvaluationCriteria();
        $arrScore = $dataHrdEvaluationCriteria->find(["id" => $strDataID]);
    } else {
        $arrScore = [
            "score1"      => 5,
            "score2"      => 4,
            ...
            "score4"      => 2,
            "score5"      => 1,
            "score1_note" => $strDataScore1Note,
            "score2_note" => $strDataScore2Note,
            "score3_note" => $strDataScore3Note,
            "score4_note" => $strDataScore4Note,
            "score5_note" => $strDataScore5Note
        ];
    }
    $strResult = "";
