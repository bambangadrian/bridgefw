...
if ($showReport) {
    ...
    if ($dataMasterSalary == 0) {
        ...
        $myDataGrid->groupBy("branch_code");
...
function getSalaryReport($db, $intYear, $intMonth, $strKriteria = "")
{
    ...
    while ($row = $db->fetchrow($res)) {
        ...
        $strSQL2 = "SELECT t0.*, t1.employee_name, t1.branch_code, t1.employee_id as nik
                    FROM \"hrd_salary_detail\" AS t0
                        LEFT JOIN \"hrd_employee\" AS t1 ON t0.\"id_employee\" = t1.id
                        WHERE t0.total_net > 0 AND t0.total_gross > 0 and \"id_salary_master\" ='$salaryMasterID' $strKriteria ORDER BY t1.branch_code, join_date";
