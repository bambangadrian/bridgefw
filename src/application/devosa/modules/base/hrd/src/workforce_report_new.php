...
if ($db->connect()) {
    ...
    if ($strDataType == "" or $strDataType == "0") {
        include('workforce_report_gender_new_pie.php');
        $strWordsChardTitle = 'Employee Gender';
        $strSelectType0 = " selected";
    }
    // Age
    if ($strDataType == "1") {
        include('workforce_report_age_new.php');
        $strWordsChardTitle = 'Employee Age';
        $strSelectType1 = " selected";
    }
    // Education
    if ($strDataType == "2") {
        include('workforce_report_edu_new_pie.php');
        $strWordsChardTitle = 'Employee Education';
        $strSelectType2 = " selected";
    }
    // Position
    if ($strDataType == "3") {
        include('workforce_report_pos_new.php');
        $strWordsChardTitle = 'Employee Position';
        $strSelectType3 = " selected";
    }
    // Salary
    if ($strDataType == "4") {
        include('workforce_report_salary_new.php');
        $strWordsChardTitle = 'Employee Salary';
        $strSelectType4 = " selected";
    }
    // Salary
    if ($strDataType == "5") {
        include('workforce_report_famstatus_new.php');
        $strWordsChardTitle = 'Employee Family Status';
        $strSelectType5 = " selected";
    }
    // lokasi
    if ($strDataType == "6") {
        include('workforce_report_living_new.php');
        $strWordsChardTitle = 'Employee Working Location';
        $strSelectType6 = " selected";
    }
    //-------- end chart
    // lokasi
    if ($strDataType == "7") {
        include('workforce_report_service_new.php');
        $strWordsChardTitle = 'Employee service year';
        $strSelectType7 = " selected";
    }
    //-------- end chart
    getUserEmployeeInfo();
