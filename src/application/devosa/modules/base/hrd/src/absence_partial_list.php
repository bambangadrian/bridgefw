...
if ($db->connect()) {
    ...
    $f->addInput(
        getWords("date from"),
        "dataDateFrom",
        getInitialValue("DateFrom", date("m") . "/01/" . date("Y")),
    ...
    $f->addInput(
        getWords("date thru"),
        "dataDateThru",
        getInitialValue("DateThru", date("m/d/Y")),
        ["style" => "width:$strDateWidth"],
        "date",
        false,
        true,
    ...
    //$f->addInputAutoComplete(getWords("employee ID"), "dataEmployeeID", getDataEmployee(getInitialValue("Employee", null, $strDataEmployee)), "style=width:$strDefaultWidthPx ".$strEmpReadonly, "string", false, true, true, "", "", true, null, "hrd_ajax_source.php?action=getemployee");
