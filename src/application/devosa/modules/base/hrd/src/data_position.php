...
doInclude('libraries/classes/hrd/hrd_position.php');
...
if ($db->connect()) {
    ...
    if ($bolCanEdit) {
        ...
        // $f->addInputAutoComplete(getWords("approver ID"), "approver_id", getDataEmployee($strDataApprover), "", "string", false, true, true,"","",true,null,"hrd_ajax_source.php?action=getemployee");
    ...
    $myDataGrid->addColumn(
        new DataGrid_Column(
            getWords("overtime meal/transport fee"), "ot_meal_fee", ['width' => '200'], ['nowrap' => '']
        )
    );
