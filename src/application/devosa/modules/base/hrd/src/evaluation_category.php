...
if ($bolCanEdit) {
    ...
    $f->addInput(getWords("weight"), "dataWeight", "", ["size" => 5], "", true, true, true, "", "(%)");
    $f->addInput(getWords("category"), "dataCategory", "", ["size" => 49], "string", true, true, true);
...
$myDataGrid->addColumn(new DataGrid_Column(getWords("category"), "category", ['width' => '250'], ['nowrap' => '']));
$myDataGrid->addColumn(
    new DataGrid_Column(
        getWords("individual evaluation"),
...
$myDataGrid->addColumn(
    new DataGrid_Column(getWords("weight (%)"), "weight", ['width' => '100'], ['align' => 'center'])
);
if ($bolCanEdit) {
    $myDataGrid->addColumn(
        new DataGrid_Column(
...
function saveData()
{
    ...
    if ($isNew) {
        ...
        $bolSuccess = $dataHrdEvaluationCategory->update(/*pk*/
...
function printDetailLink($params)
{
    ...
    if ($bolPrint) {
        ...
        return generateButton(
            "btnDetail$counter",
            getWords("show detail"),
            "",
            "onclick =\"document.formData.filterIDCategory.value=document.formData.detailID$counter.value; document.formData.action = 'evaluation_criteria.php'; document.formData.submit()\""
        );
    }
}
