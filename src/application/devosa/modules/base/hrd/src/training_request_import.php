    ...
    if (is_uploaded_file($HTTP_POST_FILES["fileAttendance"]['tmp_name'])) {
        ...
        if ($dbf) {
            ...
            foreach ($arrDate AS $strDate => $tmp) {
                $strResult .= "<a href=\"javascript:insertDate('$strDate')\">" . pgDateFormat(
                        $strDate,
                        "d M Y"
                    ) . "</a><br>";
            }
...
function importProcess($db)
{
    ...
    for ($i = 2; $i <= $baris; $i++) {
        ...
        if (($strIDEmployee != 0) OR ($strIDEmployee != "")) {
            $strDateConvert = timeStamp2SingleDate(
                $strDate
            );  // Covert menjadi format yang dikenali postgresql (Format baru)
...
function importData($db, &$intTotal)
{
    ...
    } else //if(is_dir($strFilePath = $dataCompany['attendance_file_path']))
    {
        ...
        $strResult = processAttendanceAutomatic(
            $db,
            $strFilePath,
            $strFileType,
            $strDateFrom,
