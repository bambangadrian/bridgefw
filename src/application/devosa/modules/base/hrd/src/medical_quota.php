...
doInclude('libraries/classes/hrd/hrd_medical_quota_primary.php');
doInclude('libraries/classes/hrd/hrd_medical_quota_secondary.php');
...
function saveData()
{
    ...
    if ($db->connect()) {
        ...
        $strSettingSalaryCode_ot = [];
        $strSettingSalaryValue_ot = [];
        //cari kode active dan ot
        ...
        while ($rowDb = $db->fetchrow($resDb)) {
            for ($i = (count($codeBasicSalary) - 1); $i >= 0; $i--) {
                ...
                if ($rowDb['code'] == $codeBasicSalary[$i] . "_ot") {
                    $strSettingSalaryCode_ot[$jx2] = $rowDb['code'];
                    $strSettingSalaryValue_ot[$jx2] = $rowDb['value'];
                    $jx2++;
                }
            }
        }
        $j = 0;
        $basicot = [];
        for ($i = (count($strSettingSalaryCode_ot) - 1); $i >= 0; $i--) {
            if ($strSettingSalaryValue_ot[$i] == "t" && $strSettingSalaryValue_active[$i] == "t") {
                for ($y = (count($codeBasicSalary) - 1); $y >= 0; $y--) {
                    if ($codeBasicSalary[$y] . "_active" == $strSettingSalaryCode_active[$i]) {
                        $basicot[$j] = $codeBasicSalary[$y];
                        $j++;
                    }
                }
            }
        ...
        $strSettingSalaryValue_ot = [];
        //cari kode active dan ot
        $strSQL = "SELECT code,active,ot FROM hrd_allowance_type where template_name='" . $strCurrentActiveTemplate . "'";
        $resDb = $db->execute($strSQL);
        while ($rowDb = $db->fetchrow($resDb)) {
            $strSettingSalaryCode[$jx] = $rowDb['code'];
            $strSettingSalaryValue_active[$jx] = $rowDb['active'];
            $strSettingSalaryValue_ot[$jx] = $rowDb['ot'];
            $jx++;
        }
        $j = 0;
        $basicot2 = [];
        for ($i = (count($strSettingSalaryCode) - 1); $i >= 0; $i--) {
            if ($strSettingSalaryValue_active[$i] == "t" && $strSettingSalaryValue_ot[$i] == "t") {
                $basicot2[$j] = $strSettingSalaryCode[$i];
                $j++;
            }
        }
        $basic_salary_code1 = "";
        $j = 0;
        for ($i = (count($basicot) - 1); $i >= 0; $i--) {
            if ($j == 0) {
                $basic_salary_code1 .= $basicot[$i];
            } else {
                $basic_salary_code1 .= "+" . $basicot[$i];
            }
            $j++;
        ...
        if ((count($basicot) >= 1)
            OR ((count($basicot2) >= 1) && (count($basicot) >= 1))
        ) {
            $c = 1;
        }
        if ($c == 1) {
        ...
        for ($i; $i < count($basicot); $i++) {
            $strSQL = "SELECT id_employee, amount as basic_salary FROM hrd_employee_allowance
              WHERE allowance_code = '" . $basicot2[$i] . "' AND id_salary_set ='" . $strLastSalarySet . "'
             ";
            $resDb = $db->execute($strSQL);
            while ($rowDb = $db->fetchrow($resDb)) {
                $totalAmount[$rowDb['id_employee']] += $rowDb['basic_salary'];
            }
        }
        if (count($basicot) < 1 && count($basicot2) < 1) {
        ...
        if (count($basicot) >= 1 OR count($basicot2) >= 1) {
            $strSQL = "SELECT t1.id as id_employee, employee_id, employee_name, gender, inspouse,
                  medical_quota_status, t1.grade_code, employee_status, join_date, resign_date, due_date, 
        ...
        if ($c == 2) {
            $strSQL .= "LEFT JOIN (SELECT id_employee, amount as basic_salary FROM hrd_employee_allowance
              WHERE allowance_code = '" . $basicot2[0] . "' AND id_salary_set ='" . $strLastSalarySet . "') AS t2 ON t1.id = t2.id_employee
             ";
        }
...
$strPageDesc = getWords("medical quota management");
$pageHeader = pageHeader($pageIcon, $strPageTitle, $strPageDesc);
$pageSubMenu = medicalQuotaSubmenu($strWordsMedicalData);
