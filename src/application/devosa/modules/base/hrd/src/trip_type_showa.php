...
if ($bolCanEdit) {
    ...
    $f->addInput(getWords("name"), "dataName", "", ["size" => 30, "maxlength" => 127], "string", true, true, true);
    $f->addCheckBox(
        getWords("daily allowance"),
        "dataDailyAllowance",
        false,
        null,
        "string",
    ...
    */
    foreach ($arrTripCostType AS $strCostID => $arrCostDetail) {
        $f->addCheckBox(
            getWords($arrCostDetail['trip_cost_type_name']) . " (" . $arrCostDetail['currency'] . ")",
...
$myDataGrid->addColumn(new DataGrid_Column(getWords("code"), "trip_type_code", ['width' => '150'], ['nowrap' => '']));
...
foreach ($dataset AS $strKey => $arrDetail) {
    $arrTripTypeCostSetting = $tblTripTypeCostSetting->findAll(
        "id_trip_type = " . $arrDetail['id'],
...
function saveData()
{
    ...
    $data = [
        "trip_type_code"  => $f->getValue('dataCode'),
    ...
    $data2 = ["id_trip_type" => $f->getValue('dataID')];
    foreach ($arrTripCostType AS $strCostID => $arrCostDetail) {
        $data2['id_trip_cost_type'] = $strCostID;
        $data2['include'] = ($f->getValue('dataTripCost_' . $strCostID) != "") ? 't' : 'f';
        // hapus data lama, insert data baru
        $tblHrdTripTypeCostSetting->delete(
            "id_trip_type = " . $f->getValue('dataID') . " AND id_trip_cost_type = $strCostID"
        );
