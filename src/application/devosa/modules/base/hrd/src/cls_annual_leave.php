...
class clsAnnualLeave
{
        ...
        if ($strYear == "" || $strID == "" || !validStandardDate($strJoinDate)) {
            ...
            if ($arrResult['overdue'] == 'f') {
                ...
                */
                ...
                ) //bila start date <= expiry date, maka set expired
                {
    ...
    */
    ...
    function saveLeaveHistory($strID, $strYear)
    {
        ...
        $intMBCN = getSetting("mbcn"); //masa berlaku cuti normal
        $isprogresifGrandLeave = getSetting(
            "progressive"
        ); // apakah cuti besar berlaku progresif untuk tahun2 berikutnya
        if ($strID == "" || $strYear == "") {
            return false;
        }
        if (!isset($this->arrEmployee[$strID])) {
      ...
      ";
        ...
        //Normal & add posibility grand leave
            ...
            $intQuota = $intJCN;
            //echo ("saveLeaveHistory".$strYear.$intQuota);
            ...
            if ($isprogresifGrandLeave=="t"){
              ...
              if (($intTerm % $intPCB == 0) && $intTerm > 0)
              {
                ...
                $intPeriod = $intMBCB; //periode expired-nya menggunakan cuti besar ,
              }
            }
            */
            // Aturan cuti bersar wanaarta
            ...
            if ($intMod5year > 0) { // jika kelipatan lebih besar dari 1 maka masa kerja dipastikan lebih dari 5 tahun maka dapet tambahan cuti
                if ($intMod5year < 5) { // jika masa kerja kurang dari 25 tahun
                    $intQuota += 1 + $intMod5year;  // cuti ditambah 1 + kelipatan bulat per 5 tahun
                } else if ($intMod5year == 5) {         // masa kerja diatas 5 tahun
      ...
      ";
        $res = $this->data->execute($strSQL);
        /* tidak ada additional cuti tapi di kalkulasi semua/diperlakukan sama di quota tahunan
        IF ($intMod5year>0){
