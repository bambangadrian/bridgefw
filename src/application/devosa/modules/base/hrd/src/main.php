...
doInclude('libraries/classes/hrd/hrd_education_level.php');
doInclude('libraries/helper/employee_function.php');
doInclude('calendar2.php');
...
function getAlert($db)
{
    ...
    */
    if ($bolCanApprove) {
        ...
        $strSQL .= "LEFT JOIN hrd_employee AS t1 ON t0.id_employee = t1.id ";
        $strSQL .= "LEFT JOIN hrd_overtime_application AS t2 ON t0.id_application = t2.id ";
        $strSQL .= "WHERE (t2.status = " . REQUEST_STATUS_NEW . " or t2.status = " . REQUEST_STATUS_CHECKED . ") $strKriteriaOT";
        //echo $strSQL;
        $resDb = $db->execute($strSQL);
        if ($rowDb = $db->fetchrow($resDb)) {
            if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
    ...
    if ($bolCanAcknowledge) {
        ...
        $strSQL .= "LEFT JOIN hrd_employee AS t1 ON t0.id_employee = t1.id ";
        $strSQL .= "LEFT JOIN hrd_overtime_application AS t2 ON t0.id_application = t2.id ";
        $strSQL .= "AND t2.status = " . REQUEST_STATUS_APPROVED . " $strKriteriaOT";
        $resDb = $db->execute($strSQL);
        if ($rowDb = $db->fetchrow($resDb)) {
            if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                $strResult .= " <tr valign=top class=$strClass>\n";
    ...
    if ($bolCanApprove) {
        ...
        $strSQL = "SELECT count(t0.id) AS total FROM hrd_employee_temporary AS t0 ";
        $strSQL .= "LEFT JOIN hrd_employee AS t1 ON t0.id = t1.id ";
        $strSQL .= "WHERE (status = " . REQUEST_STATUS_NEW . " or status = " . REQUEST_STATUS_CHECKED . ") $strKriteriaTemp";
        $resDb = $db->execute($strSQL);
        if ($rowDb = $db->fetchrow($resDb)) {
            if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                $strResult .= " <tr valign=top class=$strClass>\n";
...
if ($db->connect()) {
    ...
    if ($strDataDivision != "") {
        ...
        $strKriteriaDiv = " where division_code= '" . $strDataDivision . "' ";
    }
    //if ($strDataSubSection != "")   $strKriteria .= "AND sub_section_code = '$strDataSubSection' ";
    ...
    if ($bolCanView) {
        ...
        if ($_SESSION['sessionUserRole'] == ROLE_ADMIN || $_SESSION['sessionUserRole'] == ROLE_SUPER) {
