...
/*
  ...
  Update:
    - 2008.12.30 (Yudi)
*/
doInclude('../global/cls_date.php');
doInclude('cls_absence.php');
doInclude('cls_worktime.php');
doInclude('cls_overtime.php');
doInclude('cls_tax_calculation.php');
doInclude('libraries/classes/hrd/hrd_leave_allowance_base.php');
doInclude('libraries/classes/hrd/hrd_leave_allowance.php');

/*
  clsSalaryCalculation : kelas untuk melakukan perhitungan gaji karyawan
*/

class clsSalaryCalculationHotel
{
    ...
    */
    function clsSalaryCalculationHotel(
        $db,
        $strID = "",
        $bolIrregular = false,
        $strDataDate = "",
    ...
    )
    {
        ...
        // ambil data karyawan
        $this->initSalaryMaster($strDataDate, $strDateFrom, $strDateThru, $strDateFromSC, $strDateThruSC);
    ...
    */
    function initEmployee($strKriteria = "")
    {
        ...
        $strSQL = "
        SELECT te.id, te.employee_id, te.employee_name, te.id_company,  te.management_code, te.division_code, te.zakat, te.gross_up,
          ...
          te.family_status_code, te.tax_status_code, te.join_date, te.due_date, te.active, tp.position_group, tp.get_ot, tg.get_add_sc,
          ...
          WHERE ((join_date, resign_date) OVERLAPS (DATE '" . $this->arrData['date_from_sc'] . "', DATE '" . $this->arrData['salary_finish_date'] . "')
    ...
    */
    ...
    )
    {
        ...
        $this->arrData = [
            ...
            "date_thru_thr"      => "",
            "date_from_sc"       => $strDateFromSC,
            // dipakai untuk acuan perhitungan masa kerja karyawan terkait dengan thr
            "date_thru_sc"       => $strDateThruSC,
            "id_company"         => "",
        ...
        if ($this->strDataID != "") {
            ...
            if ($row = $this->data->fetchrow($res)) {
                ...
                $this->arrData['date_from_sc'] = $row['date_from_sc'];
                $this->arrData['date_thru_sc'] = $row['date_thru_sc'];
                $this->arrData['revenue'] = $row['revenue'];
    ...
    */
    ...
    )
    {
        ...
        $this->arrData['date_from_sc'] = $strSCDateFrom;
        $this->arrData['date_thru_sc'] = $strSCDateTo;
        $this->arrData['revenue'] = $strRevenue;
    ...
    */
    ...
    /* getProrateDay :  fungsi untuk mengambil jumlah hari kerja karyawan yang baru masuk atau resign
      ...
      input : id employee
      output: koefisien prorate, jika karyawan tersebut dianggap sebagai prorata
    */
    function getProrateDay($strID)
    {
        ...
        if ($strID != "" && isset($this->arrEmployee[$strID])) {
            ...
            $intDaysPerMonths = strtotime($intFinish) - strtotime($intStart);
            $intDaysPerMonths = floor($intDaysPerMonths / (60 * 60 * 24));
    ...
    function calculateBasic($strKriteria = "")
    {
      ...
      ";
        ...
        while ($row = $this->data->fetchrow($res)) {
            ...
            $fltProrate = $this->getProrateDay($strIDEmp);
            $arrEmpProrate[$strIDEmp] = $fltProrate = $fltProrate;
            $arrEmpInvProrate[$strIDEmp] = $fltInvProrate = 1 - $fltProrate;
            $this->arrDA[$row['allowance_code']][$row['id_employee']] = $row;
            $this->arrDA[$row['allowance_code']][$row['id_employee']]['id_salary_master'] = $this->strDataID;
            //echo "<br>".$row['allowance_code'];
    ...
    */
    function calculateTHR()
    {
        ...
        foreach ($this->arrDetail AS $strIDEmp => $arrInfo) {
            ...
            if ($this->irregular == 't') {
                $this->arrDetail[$strIDEmp]['thr_allowance'] = $fltProportion * ($this->arrDetail[$strIDEmp]['base_ot']);
    ...
    */
    ...
    /* calculateVariableServiceCharge : fungsi untuk menghitung late deduction untuk TI
    */
    function calculateVariableServiceCharge()
    {
        //hardcode notes : Fix Service Charge is got from grade 2 allowance, change according to needs
        //hardcode notes : Fix Service Charge GM is got from grade 3 allowance, change according to needs
        $intNumberOfEmployeeGetSC = 0;
        $intNumberOfEmployeeResignProrateSC = 0;
        $intNumberOfInverseProportion = 0;
        $intTotalRevenue = $this->arrData['revenue'];
        $fltEmployeeServiceCharge = 0;
        $fltServiceCharge = $intTotalRevenue * 0.1;
        $strSalaryDateFrom = date("Y-m-d", strtotime($this->arrData['date_from']));
        $strSalaryDateThru = date("Y-m-d", strtotime($this->arrData['date_thru']));
        $strSalaryDateFromSC = date("Y-m-d", strtotime($this->arrData['date_from_sc']));
        $strSalaryDateThruSC = date("Y-m-d", strtotime($this->arrData['date_thru_sc']));
        $strSalaryDatePrev3Months = getNextDateNextMonth($strSalaryDateFrom, -3);
        $intDaysPerMonths = strtotime($strSalaryDateThru) - strtotime($strSalaryDateFrom);
        $intDaysPerMonths = floor($intDaysPerMonths / (60 * 60 * 24));
        //getNumberOfEmployeeGetSC
        //getNumberOfEmployeeResignProrateSC
        //getNumberOfInverseProportion
        foreach ($this->arrDetail AS $strID => $arrInfo) {
            $strEmployeeJoinDate = date("Y-m-d", strtotime($this->arrEmployee[$strID]['join_date']));
            $strEmployeeResignDate = date("Y-m-d", strtotime($this->arrEmployee[$strID]['resign_date']));
            if ($this->arrEmployee[$strID]['get_add_sc'] == 't' && $strEmployeeJoinDate < $strSalaryDatePrev3Months && ($this->arrEmployee[$strID]['resign_date'] = "" || $this->arrEmployee[$strID]['resign_date'] = null || $this->arrEmployee[$strID]['resign_date'] = '1970-01-01')) {
                $this->arrEmployee[$strID]['add_sc_proportion'] = 1;
                $fltEmployeeServiceCharge += $this->arrDetail[$strID]['grade2_allowance'];
                $intNumberOfEmployeeGetSC++;
            } elseif ($this->arrEmployee[$strID]['get_add_sc'] == 't' && $strEmployeeJoinDate < $strSalaryDatePrev3Months && $strEmployeeResignDate > $strSalaryDateFrom && $strEmployeeResignDate < $strSalaryDateThruSC) {
                $intDaysActive = strtotime($strEmployeeResignDate) - strtotime($strSalaryDateFromSC);
                $intDaysActive = floor($intDaysActive / (60 * 60 * 24));
                $this->arrEmployee[$strID]['add_sc_proportion'] = $intDaysActive / $intDaysPerMonths;
                $this->arrDetail[$strID]['grade2_allowance'] *= $this->arrEmployee[$strID]['add_sc_proportion'];
                $fltEmployeeServiceCharge += $this->arrDetail[$strID]['grade2_allowance'];
                $intNumberOfEmployeeGetSC++;
                $intNumberOfEmployeeResignProrateSC++;
                $intNumberOfInverseProportion += (1 - $this->arrEmployee[$strID]['add_sc_proportion']);
            } else {
                $this->arrEmployee[$strID]['get_add_sc'] == 'f';
                $fltEmployeeServiceCharge += $this->arrDetail[$strID]['grade3_allowance'];
                $fltEmployeeServiceCharge += $this->arrDetail[$strID]['grade2_allowance'];
            }
        }
        $fltLossBreakage = $fltServiceCharge * 0.08;
        $fltSekawan = $fltServiceCharge * 0.02;
        $fltServiceCharge = $fltServiceCharge - $fltLossBreakage - $fltSekawan - $fltEmployeeServiceCharge;
        $fltServiceChargePerHead = $fltServiceCharge / $intNumberOfEmployeeGetSC;
        $intNumberOfEmployeePerfect = $intNumberOfEmployeeGetSC - $intNumberOfEmployeeResignProrateSC;
        //additional because employee resign
        $fltAdditionalSC = $intNumberOfInverseProportion * $fltServiceChargePerHead;
        $fltAdditionalSCPerHead = $fltAdditionalSC / $intNumberOfEmployeePerfect;
        foreach ($this->arrDetail AS $strID => $arrInfo) {
            if ($this->arrEmployee[$strID]['get_add_sc'] == 't') {
                $this->arrDetail[$strID]['var_service'] = $this->arrEmployee[$strID]['add_sc_proportion'] * $fltServiceChargePerHead;
                if ($this->arrEmployee[$strID]['add_sc_proportion'] == 1) {
                    $this->arrDetail[$strID]['var_service'] += $fltAdditionalSCPerHead;
                }
            }
            if ((isset($this->arrConf['var_service_tax'])) && $this->arrConf['var_service_tax'] == 't') {
                $this->arrDetail[$strID]['base_tax'] += $this->arrDetail[$strID]['var_service'];
            }
        }
    }
    //calculateVariableServiceCharge
    ...
    function calculateDeduction($strKriteria)
    {
      ...
      ";
        ...
        while ($row = $this->data->fetchrow($res)) {
            ...
            $fltProrate = $this->getProrateDay($strIDEmployee);
    ...
    function calculateTax()
    {
        ...
        foreach ($this->arrDetail AS $strID => $arrInfo) {
            ...
            if (strtolower($this->arrEmployee[$strID]['nationality']) == "wna") {
                ...
                $intTaxMethod = ($this->arrEmployee[$strID]['gross_up'] == 't') ? 1 : 0;
            ...
            $this->arrDetail[$strID]['tax'] = $fltTax;
            $this->arrDetail[$strID]['irregular_tax'] = $fltIrregularTax;
            ...
            $fltTotal = $this->arrDetail[$strID]['total_net'] - $this->arrDetail[$strID]['total_deduction'] + $this->arrDetail[$strID]['tax_allowance'] + $this->arrDetail[$strID]['irregular_tax_allowance'];
    ...
    */
    function calculateJamsostek()
    {
        ...
        foreach ($this->arrDetail AS $strID => $arrInfo) {
            //ada hardcode, entah kenapa yang arrMA kalau diset sebagai Base Jamsostek tidak bisa
            //        $this->arrDetail[$strID]['base_jamsostek'] += $this->arrDetail[$strID]['grade3_allowance'];
            //        echo "<br>".$this->arrDetail[$strID]['grade3_allowance'];
            ...
            if ($this->arrEmployee[$strID]['gross_up'] == 't') {
                $this->arrDetail[$strID]['jht_2_allowance'] = $this->arrDetail[$strID]['jamsostek_deduction'];
            }
    ...
    */
    function calculateSalaryTotal()
    {
        ...
        foreach ($this->arrDetail AS $strID => $arrInfo) {
            ...
            $fltIncome += $this->arrDetail[$strID]['var_service'];
            $fltIncome += $this->arrDetail[$strID]['jht_2_allowance'];
    ...
    */
    function saveData()
    {
        ...
        if ($this->strDataID == "") // insert new
        {
            ...
            $strSCDateFrom = (validStandardDate(
                    $this->arrData['date_from_sc']
                ) && $this->arrData['date_thru_sc'] != "") ? "'" . $this->arrData['date_from_sc'] . "'" : "NULL";
            $strSCDateThru = (validStandardDate(
                    $this->arrData['date_thru_sc']
                ) && $this->arrData['date_thru_sc'] != "") ? "'" . $this->arrData['date_thru_sc'] . "'" : "NULL";
            $strSQL = "
          INSERT INTO hrd_salary_master (
            id, date_from, date_thru, date_from_thr, date_thru_thr, date_from_sc, date_thru_sc, salary_date, status, id_company, id_salary_set, irregular, hide_blank, note,
            created, created_by, modified, modified_by, revenue
          )
          VALUES (
            '" . $this->strDataID . "', '" . $this->arrData['date_from'] . "', '" . $this->arrData['date_thru'] . "', " . $strTHRDateFrom . ",  " . $strTHRDateThru . ", " . $strSCDateFrom . ",  " . $strSCDateThru . ", '" . $this->arrData['salary_date'] . "', 0, " . $this->arrData['id_company'] . ", '" . $this->arrData['id_salary_set'] . "', '" . $this->irregular . "', '" . $this->arrData['hide_blank'] . "', '" . $this->arrData['note'] . "',
            now(), '$strUserID', now(), '$strUserID', " . $this->arrData['revenue'] . "
          );
        ";
        } else {
            $strSQL = "
        ...
        if (isset($this->arrMA['leave_allowance']['active']) && $this->arrMA['leave_allowance']['active'] == 't') {
        ...
        if (isset($this->arrConf['var_service_active']) && $this->arrConf['var_service_active'] == 't') {
            $this->calculateVariableServiceCharge();
        }
