...
doInclude('libraries/classes/hrd/hrd_employee_allowance.php');
doInclude('libraries/classes/hrd/hrd_allowance_type.php');
doInclude('plugins/krumo/class.krumo.php');
...
$strWordEmployeeID = getWords("employee id");
...
$strWordSalarySet = getWords("salary setting");
...
)
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        $strResult .= "  <td class=\"right\">" . ($intStart + $intRows) . "&nbsp;</td>";
        ...
        foreach ($arrMultivalFixAllowance as $strCode => $strName) {
            ...
            $strResult .= "<td><input type=\"text\" name=\"" . $strObjectName . $intRows . "\" maxlength=20 value=\"" . $arrFix[$rowDb['id']][$strCode] . "\" class=\"numeric form-control\" $strDisable></td>\n";
        }
        // print_r($arrFix);
        foreach ($arrMultivalDailyAllowance as $strCode => $strName) {
            if ($arrFix[$rowDb['id']][$strCode] == "") {
                $arrFix[$rowDb['id']][$strCode] = 0;
            }
            $strObjectName = "data" . getWords(str_replace("_allowance", "", $strCode));
            $strResult .= "<td align=center><input type=\"text\" name=\"" . $strObjectName . $intRows . "\" maxlength=20 value=\"" . $arrFix[$rowDb['id']][$strCode] . "\" class=\"numeric form-control\" $strDisable></td>\n";
        }
        ...
        foreach ($arrColumns AS $strKode => $strVal) {
            ...
            $strResult .= "<td align=center><input type=\"text\" name=\"dataAllowance$i" . "_$intRows\" maxlength=20 value=\"$strAmount\" class=\"numeric form-control\" $strDisable></td>\n";
        }
...
function getHeaderString()
{
    ...
    global $arrMultivalFixAllowance;
    global $arrMultivalDailyAllowance;
    global $buttonAllowance;
    $strResult = "";
    foreach ($arrMultivalFixAllowance as $strCode => $strName) {
        $strCaption = strtoupper(getSetting($strCode . "_name"));
        $strResult .= "<th class=\"center width-150\">$strCaption</th>";
        $buttonAllowance[] = "<input class=\"btn btn-sm btn-primary btn-block\" type=submit name=\"btnDefault\" value=\"Get Default\" onClick=\"document.formData.dataDefault.value = '$strCode'\" $strDisable>";
    }
    foreach ($arrMultivalDailyAllowance as $strCode => $strName) {
        $strCaption = strtoupper(getSetting($strCode . "_name"));
        $strResult .= "<th class=\"center width-150\">$strCaption</th>";
...
function getAllowanceData($db)
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        $i++;
        $strDataColumn .= "<th class=\"center width-150\"><input type=hidden name=dataAllowanceType$i value=\"" . $rowDb['code'] . "\"> ";
        $arrTemp = split("_", $rowDb['code']);
        if (count($arrTemp) > 1) {
            $strDataColumn .= str_replace(["_", " "], "&nbsp;", strtoupper($rowDb['name']));
        } else {
            $strDataColumn .= strtoupper($rowDb['name']);
        }
        if (is_numeric($rowDb['amount'])) {
            $buttonAllowance[] = "<input class=\"btn btn-sm btn-primary btn-block\" type=\"submit\" name=\"btnDefault" . $rowDb['code'] . "\" value=\"Get Default\"  onclick=\"document.formData.dataDefault2.value = '" . $rowDb['code'] . "'\" $strDisable>";
        } else {
            $buttonAllowance[] = "<input class=\"btn btn-sm btn-primary btn-block\" type=\"submit\" name=\"btnDefault" . $rowDb['code'] . "\" value=\"Get Default\"  style=\"width:75px;font-size:7pt\" onclick=\"document.formData.dataDefault3.value = '" . $rowDb['code'] . "'\" $strDisable>";
        }
        $strDataColumn .= "</th>";
...
if ($db->connect()) {
    ...
    getAllowanceData($db);
    $defaultButton = '';
    for ($i = 0; $i < count($buttonAllowance); $i++) {
        $defaultButton .= '<th class="center">' . $buttonAllowance[$i] . '</th>';
    }
    // hapus data jika ada perintah
    $closeMessageButton = '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>';
    if (isset($_REQUEST['btnSave'])) {
        if ($bolCanEdit) {
            saveData($db);
            $strError = "<div class=\"alert alert-success\">" . $closeMessageButton . "Data saved</div>";
            $bolError = false;
        } else {
            $bolError = true;
            $strError = "<div class=\"alert alert-error\">" . $closeMessageButton . getWords(
                    "Sorry, you don't have authotrity to modify any data on this page"
                ) . "</div>";
        }
        $strMessage = $strError;
    ...
    $strInputEmployee = "<input class=form-control type=text name=dataEmployee id=dataEmployee maxlength=30 value=\"" . $strDataEmployee . "\" $strReadonly>";
    ...
    $strBtnSave = ($bolCanEdit) ? "<input class=\"btn btn-small btn-primary\" type=submit name=\"btnSave\" value=\"Save\">" : "";
...
$strPageDesc = getWords('employee salary settings page');
$pageHeader = pageHeader($pageIcon, $strPageTitle, $strPageDesc);
$pageSubMenu = salarySetSubmenu($strWordsEmployeeAllowance);
