...
$arrData = [
    "dataDate"          => $strNow,
    "dataDate_"         => date("m/d/Y"),
    "dataDateFrom"      => date("m/d/Y"),
    "dataDateThru"      => date("m/d/Y"),
    "dataEmployee"      => "",
    "dataEmployeeName"  => "",
    "dataSection"       => "",
    "dataType"          => "",
...
function getData($db, $strDataID = "")
{
    ...
    if ($strDataID != "") {
        $strSQL = "SELECT t1.*, t2.employee_id, t2.id as id_employee, t2.employee_name,
                to_char(t1.date_from,'MM/DD/YYYY') As date_from_,
                to_char(t1.date_from,'MM/DD/YYYY') As date_from_,
                to_char(t1.request_date,'MM/DD/YYYY') As request_date_,";
        ...
        if ($rowDb = $db->fetchrow($resDb)) {
            ...
            $arrData['dataDate_'] = $rowDb['request_date_'];
            $arrData['dataDateFrom'] = $rowDb['date_from_'];
            $arrData['dataDateThru'] = $rowDb['date_thru_'];
            $arrData['dataDateFromOri'] = $rowDb['date_from'];
            $arrData['dataDateThruOri'] = $rowDb['date_thru'];
...
function saveData($db, &$strDataID, &$strError)
{
    ...
    if (isset($_REQUEST['dataDateFrom'])) {
        ...
        $strDataDateFrom = $arrDate[2] . "-" . $arrDate[0] . "-" . $arrDate[1];
    } else {
        $strDataDateFrom = "";
    }
    if (isset($_REQUEST['dataDateThru'])) {
        $arrDate = explode("/", $_REQUEST['dataDateThru']);
        $strDataDateThru = $arrDate[2] . "-" . $arrDate[0] . "-" . $arrDate[1];
    } else {
        $strDataDateThru = "";
    }
...
if ($db->connect()) {
    ...
    $arrData['dataDateFromNow'] = isset($arrData['dataDateFrom']) ? date(
        "m/d/Y",
        strtotime($arrData['dataDateFrom'])
    ) : date("m/d/Y");
    $arrData['dataDateThruNow'] = isset($arrData['dataDateThru']) ? date(
        "m/d/Y",
        strtotime($arrData['dataDateThru'])
    ) : date("m/d/Y");
    ...
    $strInputDateFrom = "<input class=\"form-control datepicker\" type=text size=15 maxlength=10 name=dataDateFrom id=dataDateFrom value=\"" . $arrData['dataDateFromNow'] . "\" data-date-format=\"mm/dd/yyyy\">";
    $strInputDateThru = "<input class=\"form-control datepicker\" type=text size=15 maxlength=10 name=dataDateThru id=dataDateThru value=\"" . $arrData['dataDateThruNow'] . "\" data-date-format=\"mm/dd/yyyy\">";
