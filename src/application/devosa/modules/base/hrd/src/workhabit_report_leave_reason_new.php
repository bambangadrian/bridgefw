...
//get all position
$strSQL = "SELECT DISTINCT note FROM hrd_absence ";
$resSQL = $db->execute($strSQL);
$arrCode = [];
$numCode = 0;
while ($rowDb = $db->fetchrow($resSQL)) {
    $arrCode[$numCode++] = $rowDb["note"];
}
...
if ($strPeriod == "monthly") {
    ...
    for ($iMonth = 1; $iMonth <= 12; $iMonth++) {
        ...
        for ($i = 0; $i < $numCode; $i++) {
            $strExecuteSQL = "select sum(t1.duration)  AS \"jumlah\" FROM hrd_absence AS t1
      Where
      $strKriteria AND t1.note = '$arrCode[$i]' AND date_from < '" . $strPYear . $striMonthNext . "01' and date_from >= '" . $strPYear . $striMonth . "01'";
...
if ($strPeriod == "yearly") {
    ...
    for ($iYear = $startAxis; $iYear <= $untilAxis; $iYear++) {
        for ($i = 0; $i < $numCode; $i++) {
            $strExecuteSQL = "select sum(t1.duration)  AS \"jumlah\" FROM hrd_absence AS t1 where
        $strKriteria AND t1.note = '$arrCode[$i]' AND date_from <= '" . $iYear . "1231' and date_from >= '" . $iYear . "0101'";
            //  LEFT JOIN hrd_absence_type AS t2 ON t1.absence_type_code = t2.code
            $numOfEmployee = $db->execute($strExecuteSQL);
            $numOfEmployee = $db->fetchrow($numOfEmployee);
            $numOfEmployee = $numOfEmployee["jumlah"];
