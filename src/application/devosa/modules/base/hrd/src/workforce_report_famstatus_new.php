...
//get all position
$strSQL = "SELECT family_status_code FROM hrd_family_status";
$resSQL = $db->execute($strSQL);
$arrCode = [];
$numCode = 0;
while ($rowDb = $db->fetchrow($resSQL)) {
    $arrCode[$numCode++] = $rowDb["family_status_code"];
}
...
for ($iYear = $startAxis; $iYear <= $untilAxis; $iYear++) {
    ...
    for ($i = 0; $i < $numCode; $i++) {
        $strExecuteKriteria = " AND t1.family_status_code = '$arrCode[$i]' ";
...
window.onload = function () {
  var chart = new CanvasJS.Chart("chartContainer", {
      ...
      title:{text: "Employee Family Status" },
    data: [  ' . $strData . ']
        });
  chart.render();
  ...
  var chart = new CanvasJS.Chart("chartContainerStack", {
    theme:"theme3",animationEnabled: true,
    title:{text: "Employee Family Status (Stacked Chart)" },
    data: [  ' . $strDataStack . ']
  });
  chart.render();
  ...
  var chart = new CanvasJS.Chart("chartContainerStack100", {
    theme:"theme3",animationEnabled: true,
    title:{text: "Employee Family Status (Stacked Chart 100%)" },
    data: [  ' . $strData100 . ']
  });
  chart.render();
      }
