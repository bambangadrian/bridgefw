...
//$arrCode = array();
//$numCode = 0;
//while($rowDb = $db->fetchrow($resSQL)) $arrCode[$numCode++] = $rowDb["department_code"];
...
if ($strPeriod == "monthly") {
    ...
    for ($iMonth = 1; $iMonth <= 12; $iMonth++) {
      ...
      WHERE ((resign_date <= '" . $strPYear . $striMonthNext . "01' and resign_date >= '" . $strPYear . $striMonth . "01') or resign_date is null)
      and join_date<='" . $strPYear . $striMonthNext . "01' AND
      $strKriteria AND t1.employee_status = '$key' ";
...
$strMoreForm .= '<table>';
/*
$strMoreForm.='<tr><td>Year</td><td>:<select name="pyear">';
...
window.onload = function () {
  ...
  var chartstack = new CanvasJS.Chart("chartContainerStack", {
          theme:"theme3",animationEnabled: true,
          title:{
            text: "Employee Status (Stacked Chart)"
          },
          axisX:{
            interval: 1
          },
