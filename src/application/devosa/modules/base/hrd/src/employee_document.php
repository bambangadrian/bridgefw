...
function getData($db, &$intRows, $strKriteria = "", $strOrder = "")
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        $strResult .= " <div class=\"form-group\"><label class=\"col-sm-3 control-label\" for=\"dataType\">Document Title</label>";
        ...
        $strResult .= "  <input name=\"detailDoc$intRows\" type=\"file\" id=\"detailDoc$intRows\"><input type=hidden name=detailID$intRows value=\"" . $rowDb['id'] . "\">";
    ...
    for ($i = 1; $i <= $intAdd; $i++) {
        ...
        $strResult .= "<div class=\"col-md-6\">\n";
        $strResult .= " <div class=\"form-group\"><label class=\"col-sm-3 control-label\" for=\"dataType\">Document Type</label>";
        $strResult .= " <div class=\"col-sm-9\"><input type=text class=\"form-control\" maxlength=63 name=detailInstitution$intRows value=\"" . $rowDb['institution'] . "\"></div>";
        $strResult .= "	</div>\n";
        //tampilkan foto
        if ($rowDb['doc'] == "") {
...
function saveData($db, $strDataID, &$strError)
{
    ...
    for ($i = 1; $i <= $intMax; $i++) {
        ...
        (isset($_REQUEST['detailCode' . $i])) ? $strCode = $_REQUEST['detailCode' . $i] : $strCode = "DOC";
        ...
        if ($strID == "") {
            ...
            // ambil data IDnya
            $strSQL = "SELECT id FROM hrd_employee_education WHERE id_employee = '$strDataID' AND institution = '$strInstitution' ";
            $resDb = $db->execute($strSQL);
...
$strPageDesc = getWords("employee document");;
...
$pageSubMenu = employeeEditSubmenu($strWordsDokumen);
