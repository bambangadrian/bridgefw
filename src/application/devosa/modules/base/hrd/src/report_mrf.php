...
doInclude('plugins/date/date.php');
...
------------------------------------------------------------------------------------
*/
doInclude('plugins/datagrid/datagridupdateforrecruitment.php');
//---------------END ---------------------------------------------------------------
...
function previewData()
{
    ...
    $strSQL = "
      ...
      WHERE fkr.id_recruitment_need = hrd_recruitment_need.id
        AND hrd_recruitment_need.approved_time BETWEEN '" . $f->getValue('data_date_from') . "' AND '" . $f->getValue(
            'data_date_thru'
        ) . "'
...
function exportExcel()
{
    ...
    if ($isSummary) {
        ...
        if ($f->getValue("data_date_from") == $f->getValue("data_date_thru")) {
            ...
            $myDataGrid->strTitle2 = getWords("periode") . " : " . $f->getValue(
                    "data_date_from"
                ) . " - " . $f->getValue(
                    "data_date_thru"
                );
        ...
        LEFT JOIN hrd_company AS t_com ON t_rec.id_company = t_com.id
        LEFT JOIN hrd_department AS t_dep ON t_rec.department_code = t_dep.department_code
        WHERE t_rec.recruitment_date BETWEEN '" . $f->getValue('data_date_from') . "' AND '" . $f->getValue(
                'data_date_thru'
            ) . "'
