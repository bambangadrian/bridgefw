<?php
(isset($_REQUEST['dataInterval']) && $_REQUEST['dataInterval'] > 0) ? $strDataInterval = $_REQUEST['dataInterval'] : $strDataInterval = 2;
$arrData = [];
$maxAxis = 10;
$untilAxis = date("Y"); // curr year
$startAxis = $untilAxis - $maxAxis;
//getMaxAge
$strSQL = "select MAX($untilAxis - EXTRACT(YEAR FROM join_date)) AS umur FROM hrd_employee WHERE ($untilAxis - EXTRACT(YEAR FROM birthday))>0";
...
for ($iYear = $startAxis; $iYear <= $untilAxis; $iYear++) {
    ...
    for ($i = 0; $i < ($maxAge + 1) / $strDataInterval; $i++) {
        ...
        if ($maxRange > $minAge) {
            $strExecuteKriteria = " AND ($iYear - EXTRACT(YEAR FROM join_date)) BETWEEN $minRange AND $maxRange ";
...
foreach ($arrData as $key => $arrDataYear) {
    ...
    for ($iYear = $startAxis; $iYear <= $untilAxis; $iYear++) {
        ...
        $strData100 .= "{ label:\"" . $iYear . "\",y:" . number_format(
                (($arrDataYear[$iYear] / $sumData) * 100),
                2
            ) . "}";
    }
    $strData .= "]}";
  ...
  var chart = new CanvasJS.Chart("chartContainer", {
    ...
    data: [  ' . $strData . ']
        ...
        var chartstack = new CanvasJS.Chart("chartContainerStack", {
          theme:"theme3",animationEnabled: true,
          title:{
            text: "Employee Service Year (Stacked chart)"
          },
        ...
        var chart100 = new CanvasJS.Chart("chartContainerStack100", {
          theme:"theme3",animationEnabled: true,
          title:{
            text: "Employee Service Year (Stacked chart 100%)"
          },
