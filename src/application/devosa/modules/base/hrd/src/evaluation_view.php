...
if ($db->connect()) {
    ...
    if ($strDataEmployee != "") {
        $arrEmployee = getEmployeeInfoByCode(
            $db,
            $strDataEmployee,
            "id , hrd_employee.position_code,employee_name, grade_code, division_code, department_code, section_code, sub_section_code "
        );
    ...
    if ($strDataID != "") {
        ...
        $myDataGrid2->addColumn(
            new DataGrid_Column(getWords("weight") . " (%)", "weight", "", ['nowrap' => ''], false, false)
        );
        ...
        $myDataGrid3->addColumn(
            new DataGrid_Column(getWords("answer"), "answer", "", "", false, false, "", "", "", true, 50)
        );
        ...
        $dataEvaluationFeedback = $tblEvaluationFeedback->findAll(
            ["id_evaluation" => $strDataID, "for_evaluator" => 't'],
            null,
            "sequence",
            null,
            null,
        ...
        $dataEvaluationFeedback = $tblEvaluationFeedback->findAll(
            ["id_evaluation" => $strDataID, "for_evaluator" => 'f'],
            null,
            "sequence",
            null,
            null,
...
{
    extract($params);
    return generateHidden("detailFeedbackQuestion_" . $counter, $record['question']) . " " . generateTextArea(
        "detailFeedbackAnswer_" . $counter,
        $value,
        "cols=110 maxlength=255"
    );
}
...
function calculateScore($myDataGrid)
{
    ...
    foreach ($arrCategoryScore as $strIDCategory => $arrCategoryDetail) {
        $arrCategoryScore[$strIDCategory]['result'] = /*$arrCategoryScore[$strIDCategory]['weight']/100 * */
            $arrCategoryScore[$strIDCategory]['score'];
...
function saveData()
{
    ...
    if ($db->connect()) {
        ...
        $tblEvaluationFeedback = new cHrdEvaluationFeedback();
        $tblEvaluationFeedback->delete(["id_evaluation" => $strDataID, "for_evaluator" => "f"]);
