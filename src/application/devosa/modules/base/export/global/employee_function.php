    ...
    if ($strIDEmployee != "") {
        ...
        if ($rowDb = $db->fetchrow($resDb)) {
            ...
            if ($rowTmp = $db->fetchrow($resTmp)) {
                ...
                echo $strBranch = $rowTmp['branch_new'];
                die();
            }
            // falidasikan tanggal
            ...
            $strSQL .= "section_code = '$strSection', sub_section_code = '$strSubSection', branch_code = '$strBranch' ";
...
function getTimeDiff($db, $strIDEmployee)
{
    ...
    $arrResult = $tblBranch->find(
        ["branch_code" => $arrEmp['branch_code']],
