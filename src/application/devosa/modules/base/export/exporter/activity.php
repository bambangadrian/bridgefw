...
{
    $arrResult = [];
    $arrPartialCode = ["LATE START", "MID LEAVE", "EARLY FINISH"];
...
function checkInvalidAttendance($db, $strDateFrom, $strDateThru, $strKriteria = "")
{
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
        ...
        if (getIntervalHour(
                $strNormalStart,
                $strAttendanceFinish
            ) < 240 && $strNormalStart != $strAttendanceFinish && getIntervalHour(
                $strNormalStart,
                $strNormalFinish
            ) > 240
        ) {
            ...
            if (!isset($arrAbsence[$strCurDate][$rowDb['id']])) {
                $tblAbsence->insert(
                    [
                        ...
                        "absence_type_code" => "A",
                        "note"              => "alpha generated by system",
