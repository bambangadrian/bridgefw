...
DynarchMenu.prototype.createMenuItem = function (li, parent, horiz, arrow) {
    ...
    if (tmp.href && /\S/.test(tmp.href)) {
        if (/^javascript:(.*)$/i.test(tmp.href))action = new DynarchMenu.JSAction(RegExp.$1); else action = new DynarchMenu.LinkAction(tmp.href, tmp.target);
    ...
    if (horiz) {
        item = CE("td", null, doc);
        info.labelTD = item;
        if (info.separator)item.innerHTML = "<div unselectable='on'></div>"; else {
        ...
        if (info.separator && !html_popup) {
            ...
            tmp.className = "icon";
            if (icon)tmp.appendChild(info.icon = this.cloneEl(icon, doc)); else tmp.innerHTML = _dynarch_menu_ediv;
            tmp = CE("td", item);
            tmp.className = "label";
            info.labelTD = tmp;
            if (html_popup)tmp.appendChild(this.cloneEl(label, doc)); else tmp.innerHTML = label;
            tmp = CE("td", item);
            tmp.className = "end";
            tmp.innerHTML = _dynarch_menu_ediv;
...
DynarchMenu._documentMouseUp = function (ev) {
    ...
    if (menu) {
        ...
        if (!el)_dynarch_top.DynarchMenu._closeOtherMenus(null); else if (info && info.exec)info.exec();
    }
...
DynarchMenu._documentKeyPress = function (ev) {
    ...
    DynarchMenu._forAllMenus(function (menu) {
        ...
        function do_ud(up) {
            if (tmp) {
                if (!item)item = up ? tmp.getFirstItem() : tmp.getLastItem(); else item = up ? tmp.getNextItem(item) : tmp.getPrevItem(item);
                if (item) {
        ...
        switch (ev.keyCode) {
            ...
            case 37:
                ...
                if (tmp.parent && !tmp.parent.horiz)do_27(); else {
                    if (tmp.parent) {
                        tmp = tmp.parent;
...
DynarchMenu.prototype.createMenuTree = function (ul, horiz) {
    ...
    if (base) {
        ...
        if (this.config.lazy)this.config.container.appendChild(ret); else if (this._df)this._df.appendChild(ret); else this._ca[this._ca.length] = ret;
    }
    if (horiz) {
        info.parent = null;
        div = CE("tr", tmp);
...
DynarchMenu._removeEvent = function (el, evname, func) {
    if (el.removeEventListener)el.removeEventListener(evname, func, true); else if (el.detachEvent)el.detachEvent("on" + evname, func); else el["on" + evname] = null;
};
DynarchMenu._addEvent = function (el, evname, func) {
    if (el.addEventListener)el.addEventListener(evname, func, true); else if (el.attachEvent)el.attachEvent("on" + evname, func); else el["on" + evname] = func;
};
...
DynarchMenu._createElement = function (tagName, parent, doc) {
    ...
    if (is_ie)el.unselectable = "on"; else if (is_gecko)el.style.setProperty("-moz-user-select", "none", "");
...
DynarchMenu._related = function (element, ev) {
    ...
    if (is_ie) {
        type = ev.type;
        if (type == "mouseover")related = ev.fromElement; else if (type == "mouseout")related = ev.toElement;
...
DynarchMenu.DefaultAction.prototype.exec = function () {
    if (!this.info.submenu) {
        if (typeof this.action == "function")return this.action(this.info); else try {
            var retval = false;
            eval(this.action);
            return retval;
        } catch (e) {
...
DynarchMenu.EventHandlers = {
    popup_resetActive: function (ev) {
        ...
        if (!item.separator)return item.hover(); else if (item.html_popup) {
...
DynarchMenu.MenuTree.prototype.close = function (by_key, instant) {
    ...
    if (this.base) {
        ...
        if (instant || (this.parent.base && !self.config.vertical))this._close(); else this._T_close = setTimeout(function () {
            info._close();
            info._T_close = null;
        }, self.config.timeout);
    }
...
DynarchMenu.MenuTree.prototype.open = function (el, item, instant, pos) {
    ...
    var info = this;
    if (instant)this._open(el, item, pos); else DynarchMenu._OT = setTimeout(function () {
...
DynarchMenu.MenuTree.prototype._open = function (el, item, pos) {
    ...
    if (!(base && cfg.crossFrames)) {
        if (horiz)p.y += el.offsetHeight; else {
            if (!is_khtml) {
                p.x += el.offsetWidth;
            } else if (el) {
...
DynarchMenu._nfo = {
    product: "hmenu-2.9",
    licensee: "Dynarch.com user: dedys",
    license_key: "linkware-7293688",
    purchase_date: "Mon Dec 11 19:48:52 2006 GMT",
    license_type: "linkware"
