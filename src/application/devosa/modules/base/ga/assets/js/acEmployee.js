...
function AC_getNameByCode(code) {
    strHasil = "";
    if (code != "") {
        total = AC_allCanons.length;
        for (i = 0; i < total; i++) {
            if (AC_allCanons[i] == code) {
                break;
            }
        }
        i++; // tambah satu index
        if (i <= total) {
            strHasil = AC_allCanons[i];
        }
    }
    return strHasil;
}
...
function AC_OnLoad() {
    ...
    if (!AC_isIE) {
        ...
        var compForm = window.document.forms.form1;
        if (compForm)
    ...
    if (typeof AC_targetElements == "undefined" && AC_targetElements == null) {
        //AC_targetElements = ["<tag:tagFieldCodeName />"];
    }
...
function AC_CreateDropDown(editCtrl, suggestions) {
    ...
    if (AC_dropDownList == null) {
        ...
        AC_dropDownList.style.width = 250;
