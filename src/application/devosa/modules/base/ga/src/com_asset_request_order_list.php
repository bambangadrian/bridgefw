...
doInclude('libraries/classes/ga/ga_request_order.php');
//===== END include=================================
...
function getData($db)
{
    ...
    if ($db->connect()) {
        ...
        $myDataGrid->addColumn(
            new DataGrid_Column(
                getWords("purchase request number"), "purchase_no", ['width' => '100'], ['nowrap' => '']
            )
        );
        $myDataGrid->addColumn(
            ...
            if ($rowDb = $db->fetchrow($resDb)) {
                ...
                if ($rowDb['status'] < $intStatus && $rowDb['status'] != REQUEST_STATUS_DENIED) {
                    ...
                    writeLog(
                        ACTIVITY_EDIT,
                        MODULE_PAYROLL,
                        $rowDb['employee_id'] . " - " . $rowDb['employee_name'],
                        $intStatus
                    );
                }
            }
