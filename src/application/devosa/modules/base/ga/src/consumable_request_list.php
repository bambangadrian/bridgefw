...
doInclude('libraries/classes/ga/consumable_request.php');
//================ END INCLUDE==========================================
            ...
            if ($rowDb = $db->fetchrow($resDb)) {
                ...
                if ($rowDb['status'] < $intStatus && $rowDb['status'] != REQUEST_STATUS_DENIED) {
                    ...
                    writeLog(
                        ACTIVITY_EDIT,
                        MODULE_PAYROLL,
                        $rowDb['employee_name'] . " - " . $rowDb['employee_id'] . " - " . $rowDb['request_date'],
                        $intStatus
                    );
                }
            }
...
if ($db->connect()) {
    ...
    //--------------------------------------------------------------------------------------------------------------------------------------
    $f->addInput(
        getWords("assignment date From"),
        "dataAssignmentDateFrom",
        "",
    ...
    $f->addInput(
        getWords("assignment date Thru"),
        "dataAssignmentDateThru",
        "",
        ["style" => "width:$strDateWidth"],
        "date",
        false,
