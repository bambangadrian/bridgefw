...
doInclude('libraries/classes/ga/asset_maintenance.php');
//================ END INCLUDE=====================================
...
function getData($db)
{
    ...
    if (validStandardDate($arrData['dataMaintenanceDateFrom']) && validStandardDate(
            $arrData['dataMaintenanceDateThru']
        )
    ) {
    ...
    if ($db->connect()) {
        ...
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("Maint. Date"), "maintenance_date", ['width' => '70'], ['nowrap' => ''])
        );
        $myDataGrid->addColumn(
            new DataGrid_Column(getWords("Due Date"), "due_date", ['width' => '50'], ['nowrap' => ''])
        );
...
if ($db->connect()) {
    ...
    $f->addInput(
        getWords("Maintenance date From"),
        "dataMaintenanceDateFrom",
        "",
    ...
    $f->addInput(
        getWords("Maintenance date Thru"),
        "dataMaintenanceDateThru",
        "",
        ["style" => "width:$strDateWidth"],
        "date",
        false,
