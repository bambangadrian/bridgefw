...
function hideHeaderCheck(fieldName) {
    //array TH
    var aHeaders = $(_gridName).tHead.rows;
    //jumlah row TH (misal ada rowspan=2 maka nilai nHeaders = 2
    var nHeaders = aHeaders.length;
    var counter;
    var pointer = null;

    //jika ada rowspan, maka ini untuk memroses header rowSpan dan colSpan header
    if (nHeaders > 1) {
        //dimulai dari row paling bawah
        for (var i = nHeaders - 1; i >= 0; i--) {
            counter = 0;
            for (var j = 0; j < aHeaders[i].cells.length; j++) {
                //awal
                if (pointer == null) {
                    if (aHeaders[i].cells[j].style.display != "none") counter++;
                    if (aHeaders[i].cells[j].id == 'hdrGrid' + fieldName) {
                        //mengambil informasi rowSpan jika ada
                        if (getAttributeHeader(aHeaders[i].cells[j], "rowSpan") != null) {
                            if (getAttributeHeader(aHeaders[i].cells[j], "rowSpan") == nHeaders) {
                                //jika rowSpannya sebanyak jumlah row TH, maka tidak perlu di proses karena berarti ini adalah single cell (tidak perlu adjust colspan cell)
                                return false;
                            }
                        }
                        pointer = counter + 1;
                    ...
                    if (getAttributeHeader(aHeaders[i].cells[j], "ori_colspan") > 0) {
                        if (aHeaders[i].cells[j].style.display == "none") continue;
                        colSpan = getAttributeHeader(aHeaders[i].cells[j], "colSpan");
                        //alert(colSpan);
...
function showHideField(colNumber, fieldName) {
    ...
    //hide/show selected TH rows
    isShow = showHideObjectCell($('hdrGrid' + fieldName));

    //check spanning column header (if any)
    if (isShow)
        showHeaderCheck(fieldName);
    else if (isShow != null)
        hideHeaderCheck(fieldName);

    //hide/show all TD rows in the same column with TH
