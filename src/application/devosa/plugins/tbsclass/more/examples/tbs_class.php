    ...
    function meth_Locator_FindBlockNext(&$Txt, $BlockName, $PosBeg, $ChrSub, $Mode, &$P1, &$FieldBefore)
    {
        ...
        if ($Mode === 1) {
            ...
            if (tbs_Locator_EnlargeToTag($Txt, $Loc, $Block, false) === false) {
                return $this->meth_Misc_Alert(
                    $Loc,
                    'at least one tag corresponding to ' . $Loc->PrmLst['block'] . ' is not found. Check opengin tags, closing tags and levels.',
                    false,
                    'in block\'s definition'
                );
            }
            ...
            if ($Loc->SubOk or ($Mode === 3)) {
                ...
                $Loc->BlockSrc = substr($Txt, $Loc->PosBeg, $PosBeg1 - $Loc->PosBeg) . substr(
                        $Txt,
                        $PosEnd1 + 1,
                        $Loc->PosEnd - $PosEnd1
                    );
        ...
        if ($nPosEnd !== false) {
            ...
            if (($nDecOk) and ($nPosCurr >= 5)) {
                if ((substr(
                            ...
                            $nPosCurr - 3,
                            3
                        ) === '000') and ($FrmStr[$nPosCurr - 4] !== '') and ($FrmStr[$nPosCurr - 5] === '0')
                ) {
                    $nPosCurr = $nPosCurr - 4;
            ...
            if ($Cnt > 0) {
                $this->_FrmSimpleLst[$FrmStr] = ['type' => 'date', 'str_us' => $FrmPHP, 'str_loc' => $FrmLOC];
            } else {
                $this->_FrmSimpleLst[$FrmStr] = ['type' => 'else', 'string' => $FrmStr];
            }
...
function tbs_Locator_PrmCompute(
    &$Txt,
    &$Loc,
    &$SubName,
    $Status,
