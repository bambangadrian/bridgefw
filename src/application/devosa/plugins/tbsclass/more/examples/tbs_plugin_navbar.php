    ...
    function meth_Merge_NavigationBar(&$Txt, $BlockName, $Options, $PageCurr, $RecCnt, $PageSize)
    {
        ...
        while ($Loc = $TBS->meth_Locator_FindTbs($Txt, $BlockName, $PosBeg, '.')) {
            if (isset($Loc->PrmLst['block'])) {
                $PrmLst = array_merge($PrmLst, $Loc->PrmLst);
            }
            $PosBeg = $Loc->PosEnd;
        }
        // Prepare options
        if (!is_array($Options)) {
            $Options = ['navsize' => intval($Options)];
        }
        $Options = array_merge($Options, $PrmLst);
        // Default options
        if (!isset($Options['navsize'])) {
            $Options['navsize'] = 10;
        }
        if (!isset($Options['navpos'])) {
            $Options['navpos'] = 'step';
        }
        if (!isset($Options['pagemin'])) {
            $Options['pagemin'] = 1;
        }
        // Check options
        if ($Options['navsize'] <= 0) {
            $Options['navsize'] = 10;
        }
        if ($PageSize <= 0) {
            $PageSize = 1;
        }
        if ($PageCurr < $Options['pagemin']) {
            $PageCurr = $Options['pagemin'];
        }
        $CurrPos = 0;
        $CurrNav = ['curr' => $PageCurr, 'first' => $Options['pagemin'], 'last' => -1, 'bound' => false];
