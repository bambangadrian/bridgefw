    ...
    public function DataAlert($Msg)
    {
        if (is_array($this->TBS->_CurrBlock)) {
            return $this->TBS->meth_Misc_Alert(
                'when merging block "' . implode(',', $this->TBS->_CurrBlock) . '"',
                $Msg
            );
        } else {
            return $this->TBS->meth_Misc_Alert(
                'when merging block ' . $this->TBS->_ChrOpen . $this->TBS->_CurrBlock . $this->TBS->_ChrClose,
                $Msg
            );
        }
    }
    ...
    public function DataOpen(&$Query, $QryPrms = false)
    {
        ...
        switch ($this->Type) {
                ...
                if ($this->SubType === 0) {
                    ...
                    if ($Item0[0] === '~') {
                        ...
                        if ($this->TBS->ObjectRef !== false) {
                            ...
                            $i = $this->DataAlert(
                                'invalid query \'' . $Query . '\' because property ObjectRef is not set.'
                            );
                        }
                    } else {
                        if (isset($this->TBS->VarRef[$Item0])) {
                            ...
                            $i = $this->DataAlert(
                                'invalid query \'' . $Query . '\' because VarRef item \'' . $Item0 . '\' is not found.'
                            );
                        }
                    }
                    // Check sub-items
                    ...
                    while (($i !== false) && ($i < $ItemNbr) && ($Empty === false)) {
                        ...
                        if (is_array($Var)) {
                            ...
                            $i = $this->DataAlert(
                                'invalid query \'' . $Query . '\' because item \'' . $ItemLst[$i] . '\' is neither an Array nor an Object. Its type is \'' . gettype(
                                    $Var
                                ) . '\'.'
                            );
                        }
            ...
            case 1: // Num
                ...
                if (is_array($Query)) {
                    ...
                    if (isset($Query['max'])) {
                        ...
                        $this->RecSet = $this->DataAlert(
                            'the \'num\' source is an array that has no value for the \'max\' key.'
                        );
                    }
                    if ($this->NumStep == 0) {
                        $this->RecSet = $this->DataAlert(
                            'the \'num\' source is an array that has a step value set to zero.'
                        );
                    }
                } else {
            ...
            case 4: // Custom method from ObjectRef
                ...
                if ($this->RecSet === false) {
                    $this->DataAlert(
                        'method ' . get_class(
                            $this->FctOpen[0]
                        ) . '::' . $this->FctOpen[1] . '() has failed to open query {' . $Query . '}'
                    );
                }
                break;
            case 5: // Custom method of object
                ...
                if ($this->RecSet === false) {
                    $this->DataAlert(
                        'method ' . get_class($this->SrcId) . '::tbsdb_open() has failed to open query {' . $Query . '}'
                    );
                }
                break;
            case 7: // PostgreSQL
            ...
            case 8: // SQLite
                ...
                if ($this->RecSet === false) {
                    $this->DataAlert(
                        'SQLite error message when opening the query:' . sqlite_error_string(
                            sqlite_last_error($this->SrcId)
                        )
                    );
                }
                break;
...
class clsTinyButStrong
{
            ...
            if ($Chrs !== '') {
                ...
                if ($Err) {
                    $this->meth_Misc_Alert(
                        'with clsTinyButStrong() function',
    ...
    // *-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-
    static function f_Loc_PrmCompute(
        &$Txt,
        &$Loc,
        &$SubName,
        $Status,
...
*/
    ...
    static function f_Xml_AttFind(&$Txt, &$Loc, $Move = false, $AttDelim = false)
    {
        ...
        if ($Move) {
            return self::f_Xml_AttMove($Txt, $Loc, $AttDelim);
        }
        return true;
    }
...
*/
    ...
    function __call($meth, $args)
    {
        if (isset($this->ExtendedMethods[$meth])) {
            ...
            $this->meth_Misc_Alert(
                'Method not found',
                '\'' . $meth . '\' is neither a native nor an extended method of TinyButStrong.'
            );
        }
    }
        ...
        if ($Ok !== false) {
            if (!is_null($File)) {
                ...
                if (!$this->f_Misc_GetFile($x, $File, $this->_LastFile, $this->IncludePath)) {
                    return $this->meth_Misc_Alert(
                        'with LoadTemplate() method',
    ...
    public function GetBlockSource($BlockName, $AsArray = false, $DefTags = true, $ReplaceWith = false)
    {
        ...
        $PosBeg1 = 0;
        $PosEndPrec = false;
        while ($Loc = $this->meth_Locator_FindBlockNext(
            $this->Source,
            $BlockName,
            $Pos,
            '.',
    ...
    public function PlugIn($Prm1, $Prm2 = 0)
    {
        if (is_numeric($Prm1)) {
            ...
            if (!isset($this->_piOnCommand[$PlugInId])) {
                return $this->meth_Misc_Alert(
                    'with PlugIn() method',
                    'plug-in \'' . $PlugInId . '\' can\'t run any command because the OnCommand event is not defined or activated.'
                );
            }
            $ArgLst = func_get_args();
            if ($p === false) {
        ...
        return $this->meth_Misc_Alert(
            'with PlugIn() method',
            '\'' . $Prm1 . '\' is an invalid plug-in key, the type of the value is \'' . gettype(
                $Prm1
            ) . '\'.'
        );
    }
            ...
            if ($Pos === false) {
                ...
                if ($ReadPrm) {
                    ...
                    if ($PosEnd === false) {
                        $this->meth_Misc_Alert(
                            '',
                            'can\'t found the end of the tag \'' . substr(
                                $Txt,
                                $Pos,
                                $PosX - $Pos + 10
                            ) . '...\'.'
    ...
    function &meth_Locator_SectionNewBDef(&$LocR, $BlockName, $Txt, $PrmLst, $Cache)
    {
        ...
        if ($this->_PlugIns_Ok && isset($this->_piOnCacheField)) {
            $pi = true;
            $ArgLst = [0 => $BlockName, 1 => false, 2 => &$Txt, 3 => ['att' => true]];
        } else {
            $pi = false;
        }
        // Cache TBS locators
                ...
                if ($Loc->PosBeg > $PrevEnd) { // No embedding
                    if (isset($Loc->PrmLst['att'])) {
                        ...
                        if ($this->f_Xml_AttFind($Txt, $Loc, true, $this->AttDelim)) {
                            ...
                            $this->meth_Misc_Alert(
                                '',
                                'TBS is not able to merge the field ' . $LocSrc . ' because the entity targeted by parameter \'att\' cannot be found.'
                            );
                        }
                    ...
                    if ($PrevIsAMF) {
                        ...
                        $this->meth_Misc_Alert(
                            '',
                            'TBS is not able to merge the field ' . $LocSrc . ' because parameter \'att\' makes this fied moving forward over another TBS field.'
                        );
                    }
                }
                $PrevIsAMF = false;
    ...
    function meth_Locator_SectionAddGrp(&$LocR, $BlockName, &$BDef, $Type, $Field, $Prm)
    {
        ...
        if ($BDef->FDef->LocNbr == 0) {
            $this->meth_Misc_Alert(
                'Parameter ' . $Prm,
                'The value \'' . $Field0 . '\' is unvalide for this parameter.'
            );
        }
        if ($Type === 'H') {
            ...
            for ($i = $SubStart; $i < $Loc->SubNbr; $i++) {
                ...
                if (is_array($Value)) {
                    ...
                    if (method_exists($Value, $x)) {
                        if ($this->MethodsAllowed || !in_array(
                                strtok($Loc->FullName, '.'),
                                ['onload', 'onshow', 'var']
                            )
                        ) {
                            ...
                            if (!isset($Loc->PrmLst['noerr'])) {
                                $this->meth_Misc_Alert(
                                    $Loc,
                                    '\'' . $x . '\' is a method and the current TBS settings do not allow to call methods on automatic fields.',
                                    true
                                );
                            }
                            $x = '';
                        ...
                        if (!isset($Loc->PrmLst['noerr'])) {
                            $this->meth_Misc_Alert(
                                $Loc,
                                'item ' . $x . '\' is neither a method nor a property in the class \'' . get_class(
                                    $Value
                                ) . '\'.',
                                true
                            );
                    ...
                    if (!isset($Loc->PrmLst['noerr'])) {
                        $this->meth_Misc_Alert(
                            $Loc,
                            'item before \'' . $x . '\' is neither an object nor an array. Its type is ' . gettype(
                                $Value
                            ) . '.',
                            true
                        );
        ...
        if ($Loc->FirstMerge) {
            if (isset($Loc->PrmLst['frm'])) {
                ...
                if (isset($Loc->PrmLst['strconv'])) {
                    ...
                    $this->meth_Conv_Prepare(
                        $Loc,
                        "no"
                    ); // uddin tambah baris ini untuk conversi html jika tidak di set default: "no"
                ...
                foreach ($OpeLst as $i => $ope) {
                    if ($ope === 'list') {
                        ...
                        if ($x === 'max:') {
                            ...
                            $this->meth_Misc_Alert(
                                $Loc,
                                'parameter ope doesn\'t support value \'' . $ope . '\'.',
                                true
                            );
                        }
                    }
            ...
            foreach ($Loc->OpeAct as $i => $ope) {
                switch ($ope) {
                    ...
                    case 14:
                        ...
                        break;
                    case 15:
                        $CurrVal = ($Loc->OpeUtf8) ? mb_convert_case($CurrVal, MB_CASE_UPPER, 'UTF-8') : strtoupper(
                            $CurrVal
                        );
                        break;
                    case 16:
                        $CurrVal = ($Loc->OpeUtf8) ? mb_convert_case($CurrVal, MB_CASE_LOWER, 'UTF-8') : strtolower(
                            $CurrVal
                        );
                        break;
                    case 17:
                        ...
                        break;
                    case 18:
                        $CurrVal = ($Loc->OpeUtf8) ? mb_convert_case($CurrVal, MB_CASE_TITLE, 'UTF-8') : ucwords(
                            $CurrVal
                        );
                        break;
                }
            ...
            if ($x !== '') {
                if ($this->f_Misc_GetFile($CurrVal, $x, $this->_LastFile, $this->IncludePath)) {
                    ...
                    if (!isset($Loc->PrmLst['noerr'])) {
                        $this->meth_Misc_Alert(
                            $Loc,
                            'the file \'' . $x . '\' given by parameter file is not found or not readable.',
                            true
                        );
                    }
                }
            ...
            if ($x !== '') {
                ...
                if ($this->meth_Misc_RunSubscript($CurrVal, $Loc->PrmLst) === false) {
                    if (!isset($Loc->PrmLst['noerr'])) {
                        $this->meth_Misc_Alert(
                            $Loc,
                            'the file \'' . $x . '\' given by parameter script is not found or not readable.',
                            true
                        );
                    }
                }
                ...
                if (isset($Loc->PrmLst['.'])) {
                    ...
                    if ($Loc->PrmLst['magnet'] === '#') {
                        ...
                        if (isset($Loc->AttBeg)) {
                            ...
                            $this->meth_Misc_Alert(
                                $Loc,
                                'parameter \'magnet=#\' cannot be processed because the corresponding attribute is not found.',
                                true
                            );
                        }
    ...
    function meth_Locator_FindBlockNext(&$Txt, $BlockName, $PosBeg, $ChrSub, $Mode, &$P1, &$FieldBefore)
    {
        ...
        if ($Mode === 1) {
            ...
            if ($this->f_Loc_EnlargeToTag($Txt, $Loc, $Block, false) === false) {
                return $this->meth_Misc_Alert(
                    $Loc,
                    'at least one tag corresponding to ' . $Loc->PrmLst['block'] . ' is not found. Check opening tags, closing tags and embedding levels.',
                    false,
                    'in block\'s definition'
                );
            }
            if ($Loc->SubOk || ($Mode === 3)) {
                ...
                $Loc->BlockSrc = substr($Txt, $Loc->PosBeg, $beg - $Loc->PosBeg) . substr(
                        $Txt,
                        $end + 1,
                        $Loc->PosEnd - $end
                    );
            ...
            if ($Loc === false) {
                if ($Pid > 0) { // parentgrp mode => disconnect $Txt from the source
                    ...
                    if ($LocR->BlockFound) {
                        ...
                        // Add a Footer block
                        $BDef = &$this->meth_Locator_SectionNewBDef(
                            $LocR,
                            $BlockName,
                ...
                if ($Loc->PosDefBeg >= 0) {
                    ...
                    foreach ($dynprm as $dp) {
                        $n = 0;
                        if ((isset($Loc->PrmLst[$dp])) && (strpos(
                                    $Loc->PrmLst[$dp],
                                    $this->_ChrOpen . $BlockName
                                ) !== false)
                        ) {
                            $n++;
                            if ($n == 1) {
                ...
                if (isset($Loc->PrmLst['nodata'])) { // Nodata section
                    ...
                    $BDef->WhenCond = &$this->meth_Locator_SectionNewBDef(
                        $LocR,
                        $BlockName,
                        $Loc->PrmLst['when'],
                        [],
                    ...
                    $this->meth_Locator_SectionAddGrp(
                        $LocR,
                        $BlockName,
                        $BDef,
                        'H',
                        $Loc->PrmLst['headergrp'],
                        'headergrp'
                    );
                    ...
                    if ($SrLoc !== false) {
                        ...
                        do {
                            ...
                            $SrBDef = &$this->meth_Locator_SectionNewBDef(
                                $LocR,
                                $SrName,
                                $SrLoc->BlockSrc,
                                $SrLoc->PrmLst,
                    ...
                    $BlockLst = $this->meth_Locator_FindParallel(
                        $Txt,
                        $Loc->PosBeg,
                        $Loc->PosEnd,
                        $Loc->PrmLst['parallel']
            ...
            if ($tagR !== '') {
                if (in_array($tagR, $conf['ignore'])) {
                    ...
                    if ($mode_column === false) {
                        return $this->meth_Misc_Alert(
                            "Parallel",
                            "There is a column definition ($tagR) after a row (" . $Rows[$RowIdx - 1]['tag'] . ")."
                        );
                    }
                    ...
                    $att = $conf['cols'][$tagR];
                    $this->meth_Locator_FindParallelCol(
                        $SrcP,
                        $PosR,
                        $tagR,
                        $pRO,
                    ...
                    if ($locRE === false) {
                        return $this->meth_Misc_Alert(
                            "Parallel",
                            "The row closing tag is not found. (tagR=$tagR, p=$p, pROe=$pROe)"
                        );
                    }
                    // Inner source
        ...
        foreach ($Rows as $r => $Row) {
            ...
            if (isset($Cells[$RefCellB]) && $Cells[$RefCellB]['IsBegin']) {
                if (isset($Cells[$RefCellE]) && $Cells[$RefCellE]['IsEnd']) {
                    ...
                    return $this->meth_Misc_Alert(
                        "Parallel",
                        "At row " . $Row['count'] . " having entity [" . $Row['tag'] . "], the column $RefCellE is missing or is not the last in a set of spanned columns. (The block is defined from column $RefCellB to $RefCellE)"
                    );
                }
            } else {
                return $this->meth_Misc_Alert(
                    "Parallel",
                    "At row " . $Row['count'] . " having entity [" . $Row['tag'] . "],the column $RefCellB is missing or is not the first in a set of spanned columns. (The block is defined from column $RefCellB to $RefCellE)"
                );
            }
        }
        return $Blocks;
    ...
    function meth_Locator_FindParallelCol(
        $SrcR,
        &$PosC,
        $tagC,
        $pCO,
    ...
    function meth_Merge_Block(&$Txt, $BlockLst, &$SrcId, &$Query, $SpePrm, $SpeRecNum, $QryPrms = false)
    {
        ...
        while ($BlockId < $BlockNbr) {
            ...
            if ($LocR->BlockFound) {
                ...
                if ($Src->OnDataPrm = isset($LocR->PrmLst['ondata'])) {
                    ...
                    if (isset($Src->OnDataPrmDone[$Src->OnDataPrmRef])) {
                        ...
                        if ($this->meth_Misc_UserFctCheck($Src->OnDataPrmRef, 'f', $ErrMsg, $ErrMsg, true)) {
                            ...
                            $Src->OnDataPrm = $this->meth_Misc_Alert(
                                $LocR,
                                '(parameter ondata) ' . $ErrMsg,
                                false,
                                'block'
                            );
                        }
                ...
                // Dynamic query
                if ($LocR->P1) {
                    if (($LocR->PrmLst['p1'] === true) && ((!is_string($Query)) || (strpos(
                                    $Query,
                                    '%p1%'
                                ) === false))
        ...
        while ($Loc = $this->meth_Locator_FindTbs($Txt, $Id, $Pos, '.')) {
            ...
            if ($Loc->SubLst[0] === '') {
                ...
                if ($ObjOk) {
                    ...
                    $this->meth_Misc_Alert(
                        $Loc,
                        'property ObjectRef is neither an object nor an array. Its type is \'' . gettype(
                            $this->ObjectRef
                        ) . '\'.',
                        true
                    );
                ...
                if (isset($Loc->PrmLst['noerr'])) {
                    ...
                    $this->meth_Misc_Alert(
                        $Loc,
                        'the key \'' . $Loc->SubLst[0] . '\' does not exist or is not set in VarRef. (' . $msg . ')',
                        true
                    );
                }
            }
    ...
    function meth_Merge_AutoSpe(&$Txt, &$Loc)
    {
        ...
        if (isset($Loc->SubLst[1])) {
            switch ($Loc->SubLst[1]) {
                ...
                case 'script_name':
                    $x = basename(
                        ((isset($_SERVER)) ? $_SERVER['PHP_SELF'] : $GLOBALS['HTTP_SERVER_VARS']['PHP_SELF'])
                    );
                    break;
                case 'template_name':
                    $x = $this->_LastFile;
                ...
                case 'template_date':
                    $x = '';
                    if ($this->f_Misc_GetFile($x, $this->_LastFile, '', [], false)) {
                        $x = $x['mtime'];
                    }
                    break;
                case 'template_path':
                    ...
                    if (array_key_exists($col, $Src->CurrRec)) {
                        ...
                        if (!$col_opt) {
                            $this->meth_Misc_Alert(
                                ...
                                'key \'' . $col . '\' is not found in record #' . $Src->RecNum . ' of block [' . $BDef->Name . ']. This key can become optional if you designate it with parenthesis in the main block, i.e.: sub' . $i . '=(' . $col . ')'
                            );
                        }
                        unset($data);
                        $data = [];
                ...
                if ($DelField) {
                    ...
                    if ($LocA->PosBeg2 === false) {
                        if ($this->f_Loc_EnlargeToTag($Txt, $LocA, $LocA->PrmLst['block'], false) === false) {
                            $this->meth_Misc_Alert(
                                $LocA,
                                'at least one tag corresponding to ' . $LocA->PrmLst['block'] . ' is not found. Check opening tags, closing tags and embedding levels.',
                                false,
                                'in block\'s definition'
                            );
                        }
                    ...
                    if ($DelBlock) {
                        ...
                        if (isset($LocA->PrmLst['parallel'])) {
                            ...
                            $parallel = $this->meth_Locator_FindParallel(
                                $Txt,
                                $LocA->PosBeg,
                                $LocA->PosEnd,
                                $LocA->PrmLst['parallel']
                            );
                            if ($parallel === false) {
    ...
    function meth_Misc_Assign($Name, &$ArgLst, $CallingMeth)
    {
        ...
        if (!isset($this->Assigned[$Name])) {
            ...
            return $this->meth_Misc_Alert(
                'with ' . $CallingMeth . '() method',
                'key \'' . $Name . '\' is not defined in property Assigned.'
            );
        }
        ...
        if (($CallingMeth !== false) && (strcasecmp($CallingMeth, $meth) != 0)) {
            return $this->meth_Misc_Alert(
                'with ' . $CallingMeth . '() method',
                'the assigned key \'' . $Name . '\' cannot be used with method ' . $CallingMeth . ' because it is defined to run with ' . $meth . '.'
            );
        }
        $n = count($a);
        for ($i = 0; $i < $n; $i++) {
        ...
        if ($CallingMeth === false) {
            if (in_array(strtolower($meth), ['mergeblock', 'mergefield'])) {
                ...
                return $this->meth_Misc_Alert(
                    'The assigned field \'' . $Name . '\'. cannot be merged because its type \'' . $a[0] . '\' is not supported.'
                );
            }
        }
        if (!isset($a['merged'])) {
    ...
    function meth_Misc_UserFctCheck(&$FctInfo, $FctCat, &$FctObj, &$ErrMsg, $FctCheck = false)
    {
        ...
        if ($FctStr[0] === '~') {
            ...
            for ($i = 0; $i <= $iMax; $i++) {
                ...
                if (is_object($ObjRef)) {
                    ...
                    if (method_exists($ObjRef, $x)) {
                        ...
                        $ErrMsg = 'Expression \'' . $FctStr . '\' is invalid because \'' . $x . '\' is not a method in the class \'' . get_class(
                                $ObjRef
                            ) . '\'.';
                        return false;
                        ...
                        $ErrMsg = 'Expression \'' . $FctStr . '\' is invalid because sub-item \'' . $x . '\' is neither a method nor a property in the class \'' . get_class(
                                $ObjRef
                            ) . '\'.';
                        return false;
                    }
            ...
            $IsObj = ($FctCat === 'o');
            if ($IsObj && method_exists($FctObj, 'tbsdb_open') && (!method_exists(
                    $FctObj,
                    '+'
                ))
            ...
            if ($FctCheck && ($this->FctPrefix !== '') && (strncmp(
                        $this->FctPrefix,
                        $FctStr,
                        strlen($this->FctPrefix)
                    ) !== 0)
            ) {
    ...
    function meth_PlugIn_Install($PlugInId, $ArgLst, $Auto)
    {
        ...
        if (class_exists($PlugInId)) {
            ...
            if (function_exists($FctRef)) {
                ...
                return $this->meth_Misc_Alert(
                    $ErrMsg,
                    'no class named \'' . $PlugInId . '\' is found, and no function named \'' . $FctRef . '\' is found.'
                );
            }
        }
    ...
    function meth_PlugIn_SetEvent($PlugInId, $Event, $NewRef = '')
    {
        ...
        if (strpos(
                ',OnCommand,BeforeLoadTemplate,AfterLoadTemplate,BeforeShow,AfterShow,OnData,OnFormat,OnOperation,BeforeMergeBlock,OnMergeSection,OnMergeGroup,AfterMergeBlock,OnSpecialVar,OnMergeField,OnCacheField,',
                ',' . $Event . ','
            ) === false
        ) {
            return $this->meth_Misc_Alert(
                'with plug-in \'' . $PlugInId . '\'',
                'The plug-in event named \'' . $Event . '\' is not supported by TinyButStrong (case-sensitive). This event may come from the OnInstall() method.'
            );
        }
        $PropName = '_pi' . $Event;
        if ($NewRef === false) {
        ...
        if (is_object($PiRef)) {
            ...
            if (!method_exists($PiRef, $NewRef)) {
                return $this->meth_Misc_Alert(
                    'with plug-in \'' . $PlugInId . '\'',
                    'The plug-in event named \'' . $Event . '\' is declared but its corresponding method \'' . $NewRef . '\' is found.'
                );
            }
            $FctRef = [&$PiRef, $NewRef];
        } else {
            ...
            if (!function_exists($FctRef)) {
                return $this->meth_Misc_Alert(
                    'with plug-in \'' . $PlugInId . '\'',
                    'The expected function \'' . $FctRef . '\' is not found.'
                );
            }
        }
