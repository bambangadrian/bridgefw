...
class clsOpenTBS extends clsTbsZip
{
    ...
    function OnInstall()
    {
        ...
        if (!isset($TBS->OtbsAutoLoad)) {
            $TBS->OtbsAutoLoad = true;
        } // TBS will load the subfile regarding to the extension of the archive
        if (!isset($TBS->OtbsConvBr)) {
            $TBS->OtbsConvBr = false;
        }  // string for NewLine conversion
    ...
    function BeforeLoadTemplate(&$File, &$Charset)
    {
        ...
        if ($TBS->_Mode != 0) {
            return;
        } // If we are in subtemplate mode, the we use the TBS default process
        ...
        if (($FilePath !== '') || ($File === '')) {
            if ($Charset === OPENTBS_ALREADY_XML) {
                ...
                $TBS->LoadTemplate('', [&$this, 'ConvXmlOnly']); // Define the function for string conversion
            } else {
                $TBS->LoadTemplate('', [&$this, 'ConvXmlUtf8']); // Define the function for string conversion
            }
        }
        // Load the subfile(s)
    ...
    function BeforeShow(&$Render, $File = '')
    {
        ...
        if ($this->OpenXmlCTypes !== false) {
            $this->OpenXML_CTypesCommit($Debug);
        }    // Commit special OpenXML features if any
        if ($this->OpenDocManif !== false) {
            $this->OpenDoc_ManifestCommit($Debug);
        }  // Commit special OpenDocument features if any
        if ($this->OpenXmlRid !== false) {
            $this->OpenXML_Rels_CommitNewRids($Debug);
        } // Must be done also after the loop because some Rid can be added with [onshow]
        ...
        if (($TBS->ErrCount > 0) && (!$TBS->NoErr) && (!$Debug)) {
            $TBS->meth_Misc_Alert(
                'Show() Method',
                'The output is cancelled by the OpenTBS plugin because at least one error has occured.'
            );
            exit;
        }
        if ($Debug) {
            ...
            $this->Flush(
                ...
                $File,
                $ContentType
            ); // $Render is used because it can contain options OPENTBS_DOWNLOAD and OPENTBS_NOHEADER.
    ...
    function OnCommand($Cmd, $x1 = null, $x2 = null, $x3 = null, $x4 = null, $x5 = null)
    {
        ...
        if ($Cmd == OPENTBS_RESET) {
            ...
            if ($this->ExtEquiv === 'xlsx') {
                ...
                if ($o->file === false) {
                    return $this->RaiseError(
                        "($Cmd) Error with sheet '$x1'. The corresponding XML subfile is not referenced."
                    );
                }
            ...
            if ($this->ExtType == 'odf') {
                ...
                switch ($this->ExtEquiv) {
                    case 'docx':
                        $MainTags = ['w:commentRangeStart', 'w:commentRangeEnd', 'w:commentReference'];
                        $CommFiles = ['wordprocessingml.comments+xml'];
                        $CommTags = ['w:comment'];
                        $Inner = true;
                        break;
                    case 'xlsx':
                        $CommFiles = ['spreadsheetml.comments+xml'];
                        $CommTags = ['comment', 'author'];
                        break;
                    case 'pptx':
                        $CommFiles = ['presentationml.comments+xml'];
                        $CommTags = ['p:cm'];
                        break;
                    default:
                        return 0;
            ...
            $code = $x1;
            $file = $x2;
            $prms = ['default' => 'current', 'adjust' => 'inside'];
            if (is_array($x3)) {
    ...
    function TbsInitArchive()
    {
        ...
        $this->TbsNoField = []; // idx of sub-file having no TBS fields
        $this->IdxToCheck = []; // index of files to check
        $this->PrevVals = []; // Previous values for 'mergecell' operator
        $this->ImageIndex = 1;          // Serial for inserted images
        $this->ImageInternal = []; // Internal names of inserted image
        $this->ExtEquiv = false;
        $this->ExtType = false;
        $this->OtbsSheetSlidesDelete = [];
        $this->OtbsSheetSlidesVisible = [];
        ...
        $this->MsExcel_NoTBS = []; // shared string containing no TBS field
        $this->MsExcel_KeepRelative = [];
    ...
    function TbsLoadSubFileAsTemplate($SubFileLst)
    {
        ...
        foreach ($SubFileLst as $SubFile) {
            ...
            if ($idx === false) {
                $ok = $this->RaiseError(
                    'Cannot load "' . $SubFile . '". The file is not found in the archive "' . $this->ArchFile . '".'
                );
                ...
                if ($this->LastReadNotStored) {
                    ...
                    if ($TBS === false) {
                        $this->TbsSwitchMode(
                            true
                        ); // Configuration which prevents from other plug-ins when calling LoadTemplate()
    ...
    function TbsStorePark()
    {
        if ($this->TbsCurrIdx !== false) {
            $this->TbsStoreLst[$this->TbsCurrIdx] = ['src' => $this->TBS->Source, 'onshow' => true];
            $this->TBS->Source = '';
            $this->TbsCurrIdx = false;
        }
    }
    ...
    function TbsStorePut($idx, $src, $onshow = null)
    {
        if ($idx === $this->TbsCurrIdx) {
            ...
            $this->TbsStoreLst[$idx] = ['src' => $src, 'onshow' => $onshow];
        }
    }
        ...
        if ($idx === $this->TbsCurrIdx) {
            ...
            if ($this->LastReadComp > 0) {
                if ($caller === false) {
                    ...
                    return $this->RaiseError(
                        "(" . $caller . ") unable to uncompress '" . $this->TbsGetFileName($idx) . "'."
                    );
                }
            } else {
                return $txt;
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
    ...
    function TbsPicPrepare(&$Txt, &$Loc, $IsCaching)
    {
        ...
        if (isset($Loc->PrmLst['att'])) {
            return $this->RaiseError(
                'Parameter att is used with parameter ope=changepic in the field [' . $Loc->FullName . ']. changepic will be ignored'
            );
        }
        $backward = true;
        ...
        if ($this->ExtType === 'odf') {
            ...
            if ($att === false) {
                return $this->RaiseError(
                    'Parameter ope=changepic used in the field [' . $Loc->FullName . '] has failed to found the picture.'
                );
            }
        } else {
            return $this->RaiseError(
                'Parameter ope=changepic used in the field [' . $Loc->FullName . '] is not supported with the current document type.'
            );
        }
        ...
        if (isset($Loc->PrmLst['unique']) && $Loc->PrmLst['unique']) {
            ...
            if ($this->ExtType === 'odf') {
                ...
                if ($InternalPicPath === false) {
                    $this->RaiseError(
                        'The picture to merge with field [' . $Loc->FullName . '] cannot be found. Value=' . $Value
                    );
                }
            }
    ...
    function TbsPicGetDim_OpenXML_dml($Txt, $Pos, $Forward, $FieldPos, $FieldLen)
    {
        ...
        $dim_shape = $this->TbsPicGetDim_Any(
            $Txt,
            $Pos,
            $Forward,
            $FieldPos,
        ...
        $dim_inner = $this->TbsPicGetDim_Any(
            $Txt,
            $Pos,
            $Forward,
            $FieldPos,
        ...
        $dim_drawing = $this->TbsPicGetDim_Drawings(
            $Txt,
            $Pos,
            $FieldPos,
            $FieldLen,
    ...
    function TbsPicGetDim_Drawings($Txt, $Pos, $FieldPos, $FieldLen, $Offset, $dim_inner)
    {
        ...
        $p = $loc->PosBeg;
        $res = [];
        $el_lst = ['w' => 'xdr:colOff', 'h' => 'xdr:rowOff'];
    ...
    function TbsPicExternalPath(&$Value, &$PrmLst)
    {
        ...
        if (isset($PrmLst['from'])) {
            if (!isset($PrmLst['pic_prepared'])) {
                $TBS->meth_Merge_AutoVar($PrmLst['from'], true);
            } // merge automatic TBS fields in the path
        ...
        if ((!isset($PrmLst['pic_prepared'])) && isset($PrmLst['default'])) {
            $TBS->meth_Merge_AutoVar($PrmLst['default'], true);
        } // merge automatic TBS fields in the path
        ...
        if (!file_exists($FullPath)) {
            if (isset($PrmLst['default'])) {
                ...
                if ($x === 'current') {
                    ...
                    return $this->RaiseError(
                        'The default picture "' . $x . '" defined by parameter "default" of the field [' . $Loc->FullName . '] is not found.'
                    );
                }
            } else {
                return false;
    ...
    function TbsPicAdd(&$Value, &$PrmLst, &$Txt, &$Loc, $Prm)
    {
        ...
        if (isset($PrmLst['as'])) {
            if (!isset($PrmLst['pic_prepared'])) {
                $TBS->meth_Merge_AutoVar($PrmLst['as'], true);
            } // merge automatic TBS fields in the path
        ...
        if ($this->ExtType === 'odf') {
            ...
            $BackNbr = max(
                substr_count($TBS->OtbsCurrFile, '/') - 1,
                0
            ); // docx=>"media/img.png", xlsx & pptx=>"../media/img.png"
            ...
            if ($tDim !== false) {
                ...
                if ($adjust == '%') {
                    if ($tDim['wb'] > $tDim['hb']) { // the last attribute must be processed first
                        $edit_lst = ['w' => $adjust_coef * $w, 'h' => $adjust_coef * $h];
                    } else {
                        $edit_lst = ['h' => $adjust_coef * $h, 'w' => $adjust_coef * $w];
                    }
                } elseif ($adjust == 'samewidth') {
                    $edit_lst = ['h' => $tDim['wv'] * $h / $w];
                } elseif ($adjust == 'sameheight') {
                    $edit_lst = ['w' => $r * $tDim['hv']];
                } else { // default value
                    if ($tDim['r'] >= $r) {
                        $edit_lst = ['w' => $r * $tDim['hv']]; // adjust width
                    } else {
                        $edit_lst = ['h' => $tDim['wv'] * $h / $w]; // adjust height
                    }
                }
                // edit dimensions
                foreach ($edit_lst as $what => $new) {
                    ...
                    if ($adjust == '%') {
                        if ($tDim['cpt'] !== false) {
                            $new = $new * $tDim['cpt'];
                        } // apply the coef to Point conversion if any
    ...
    function TbsSearchInFiles($files, $str, $returnFirstFound = true)
    {
        ...
        foreach ($this->TbsStoreLst as $idx => $s) {
            if (($idx !== $this->TbsCurrIdx) && isset($idx_keys[$idx])) {
                ...
                $p = strpos($s['src'], $str);
                if ($p !== false) {
                    $keys_ok[] = ['key' => $key, 'idx' => $idx, 'src' => &$s['src'], 'pos' => $p, 'curr' => false];
                    if ($returnFirstFound) {
                        return $keys_ok[0];
                    }
                }
        ...
        foreach ($keys_todo as $key => $idx) {
            ...
            $p = strpos($txt, $str);
            if ($p !== false) {
                $keys_ok[] = ['key' => $key, 'idx' => $idx, 'src' => $txt, 'pos' => $p, 'curr' => false];
                if ($returnFirstFound) {
                    return $keys_ok[0];
                }
            }
        ...
        if ($returnFirstFound) {
            return ['key' => false, 'idx' => false, 'src' => false, 'pos' => false, 'curr' => false];
        } else {
            return $keys_ok;
        }
    }
    ...
    function TbsSheetCheck()
    {
        if (count($this->OtbsSheetSlidesDelete) > 0) {
            $this->RaiseError(
                "Unable to delete the following sheets because they are not found in the workbook: " . (str_replace(
                    ['i:', 'n:'],
                    '',
                    implode(', ', $this->OtbsSheetSlidesDelete)
                )) . '.'
            );
        }
        if (count($this->OtbsSheetSlidesVisible) > 0) {
            $this->RaiseError(
                "Unable to change visibility of the following sheets because they are not found in the workbook: " . (str_replace(
                    ['i:', 'n:'],
                    '',
                    implode(', ', array_keys($this->OtbsSheetSlidesVisible))
                )) . '.'
            );
        }
    }
    ...
    function TbsDeleteColumns(&$Txt, $Value, $PrmLst, $PosBeg, $PosEnd)
    {
        ...
        if ($ext === 'docx') {
            $el_table = 'w:tbl';
            $el_delete = [
                ['p' => 'w:tblGrid', 'c' => 'w:gridCol', 's' => false],
                ['p' => 'w:tr', 'c' => 'w:tc', 's' => false],
            ];
        } elseif ($ext === 'odt') {
            $el_table = 'table:table';
            $el_delete = [
                ['p' => false, 'c' => 'table:table-column', 's' => 'table:number-columns-repeated'],
                ['p' => 'table:table-row', 'c' => 'table:table-cell', 's' => false],
            ];
        } else {
            return false;
        }
    ...
    function TbsSheetSlide_DeleteDisplay($id_or_name, $ok, $delete)
    {
        ...
        foreach ($id_or_name as $item => $action) {
            ...
            $item_ref = (is_string($item)) ? 'n:' . htmlspecialchars(
                    $item
                ) : 'i:' . $item; // help to make the difference beetween id and name
            if ($delete) {
                if ($ok) {
    ...
    function Ext_PrepareInfo($Ext = false)
    {
        ...
        if (isset($GLOBAL['_OPENTBS_AutoExt'][$Ext])) {
            ...
            $i = [
                'main'     => 'content.xml',
                'br'       => '<text:line-break/>',
                'ctype'    => 'application/vnd.oasis.opendocument.',
                'pic_path' => 'Pictures/',
                'rpl_what' => '&apos;',
                'rpl_with' => '\''
            ];
            if ($this->FileExists('styles.xml')) {
                $i['load'] = ['styles.xml'];
            } // styles.xml may contain header/footer contents
            if ($Ext === 'odf') {
                $i['br'] = false;
            }
            if ($Ext === 'odm') {
            ...
            $block_alias = [
                ...
                'tbs:page'      => [&$this, 'OpenDoc_GetPage'], // ODT
                'tbs:slide'     => 'draw:page',       // ODP
                'tbs:sheet'     => 'table:table',     // ODS (table for ODT)
                'tbs:draw'      => [&$this, 'OpenDoc_GetDraw'],
                'tbs:drawgroup' => 'draw:g',
                'tbs:drawitem'  => [&$this, 'OpenDoc_GetDraw'],
            ...
            if ($set_option) {
                $TBS->SetOption(
                    ...
                    [
                        'parent' => 'table:table',
                        'ignore' => ['table:covered-table-cell', 'table:table-header-rows'],
                        'cols'   => ['table:table-column' => 'table:number-columns-repeated'],
                        'rows'   => ['table:table-row'],
                        'cells'  => ['table:table-cell' => 'table:number-columns-spanned'],
                    ]
                );
            }
        } elseif ($Frm === 'openxml') {
            ...
            if ($TBS->OtbsConvertApostrophes) {
                $x = [chr(226) . chr(128) . chr(152), chr(226) . chr(128) . chr(153)];
            } else {
                $x = null;
            }
            ...
            if ($Ext === 'docx') {
                $i = [
                    'br'         => '<w:br/>',
                    'ctype'      => $ctype . 'wordprocessingml.document',
                    'pic_path'   => 'word/media/',
                    'rpl_what'   => $x,
                    'rpl_with'   => '\'',
                    'pic_entity' => 'w:drawing'
                ];
                $i['main'] = $this->OpenXML_MapGetMain('wordprocessingml.document.main+xml', 'word/document.xml');
                $i['load'] = $this->OpenXML_MapGetFiles(['wordprocessingml.header+xml', 'wordprocessingml.footer+xml']);
                $block_alias = [
                    'tbs:p'         => 'w:p',
                    'tbs:title'     => 'w:p',
                    'tbs:section'   => [&$this, 'MsWord_GetSection'],
                    'tbs:table'     => 'w:tbl',
                    'tbs:row'       => 'w:tr',
                    'tbs:cell'      => 'w:tc',
                    'tbs:page'      => [&$this, 'MsWord_GetPage'],
                ...
                if ($set_option) {
                    $TBS->SetOption(
                        ...
                        [
                            'parent' => 'w:tbl',
                            'ignore' => ['w:tblPr', 'w:tblGrid'],
                            'cols'   => ['w:gridCol' => ''],
                            'rows'   => ['w:tr'],
                            'cells'  => ['w:tc' => ''], // <w:gridSpan w:val="2"/>
                        ]
                    );
                }
                ...
                $i = [
                    'br'         => false,
                    'ctype'      => $ctype . 'spreadsheetml.sheet',
                    'pic_path'   => 'xl/media/',
                    'pic_entity' => 'xdr:twoCellAnchor'
                ];
                ...
                $i = [
                    'br'         => false,
                    'ctype'      => $ctype . 'presentationml.presentation',
                    'pic_path'   => 'ppt/media/',
                    'rpl_what'   => $x,
                    'rpl_with'   => '\'',
                    'pic_entity' => 'p:pic'
                ...
                $i['load'] = $this->OpenXML_MapGetFiles(['presentationml.notesSlide+xml']); // auto-load comments
                $block_alias = [
                    'tbs:p'         => 'a:p',
                    'tbs:title'     => 'a:p',
                    'tbs:table'     => 'a:tbl',
    ...
    function Ext_DeductFormat(&$Ext, $Search)
    {
        ...
        if (strpos(',docx,xlsx,xlsm,pptx,', ',' . $Ext . ',') !== false) {
            return 'openxml';
        }
        if (!$Search) {
            return false;
    ...
    function XML_DeleteElements(&$Txt, $TagLst, $OnlyInner = false)
    {
        ...
        foreach ($TagLst as $tag) {
            ...
            while (($p1 = $this->XML_FoundTagStart($Txt, $t_open, $p1)) !== false) {
                ...
                if ($OnlyInner) {
                    if ($p2 !== false) {
                        $Txt = substr_replace($Txt, '', $pe1 + 1, $p2 - $pe1 - 1);
                    }
    ...
    function XML_DeleteAttributes(&$Txt, $Pos, $AttLst, $StrLst)
    {
        ...
        foreach ($AttLst as $att) {
            ...
            if ($p1 !== false) {
                ...
                if ($p2 !== false) {
                    $x = substr_replace($x, '', $p1, $p2 - $p1 + 1);
                }
            }
        }
        // Delete strings
        foreach ($StrLst as $str) {
            $x = str_replace('', $str, $x);
        }
        $x_len2 = strlen($x);
        if ($x_len2 != $x_len) {
            $Txt = substr_replace($Txt, $x, $Pos, $x_len);
        }
        return $Pos + $x_len2;
    }
    ...
    function Misc_ColNum($ColRef, $IsODF)
    {
        if ($IsODF) {
            $p = strpos($ColRef, '.');
            if ($p !== false) {
                $ColRef = substr($ColRef, $p);
            } // delete the table name wich is in prefix
    ...
    function OpenXML_Rels_GetObj($DocPath, $TargetPrefix)
    {
        ...
        if (!isset($this->OpenXmlRid[$DocPath])) {
            $o = (object)null;
            $o->RidLst = [];    // Current Rids in the template ($Target=>$Rid)
            $o->TargetLst = []; // Current Targets in the template ($Rid=>$Target)
            $o->RidNew = [];    // New Rids to add at the end of the merge
            $o->DirLst = [];    // Processed target dir
        ...
        if (!isset($o->DirLst[$TargetPrefix])) {
            ...
            while (($p = strpos($Txt, $zTarget, $p + 1)) !== false) {
                ...
                if ($p2 === false) {
                    return $this->RaiseError(
                        "(OpenXML) end of attribute Target not found in position " . $p1 . " of sub-file " . $o->FicPath
                    );
                }
                ...
                if ($p1 === false) {
                    return $this->RaiseError(
                        "(OpenXML) beginning of tag not found in position " . $p . " of sub-file " . $o->FicPath
                    );
                }
                ...
                if ($p1 !== false) {
                    ...
                    if ($p2 === false) {
                        return $this->RaiseError(
                            "(OpenXML) end of attribute Id not found in position " . $p1 . " of sub-file " . $o->FicPath
                        );
                    }
	...
	*/
    ...
    function OpenXML_Rels_CommitNewRids($Debug)
    {
        foreach ($this->OpenXmlRid as $doc => $o) {
            if (count($o->RidNew) > 0) {
                ...
                if ($p === false) {
                    return $this->RaiseError(
                        "(OpenXML) closing tag </Relationships> not found in subfile " . $o->FicPath
                    );
                }
                ...
                $this->OpenXmlRid[$doc]->RidNew = []; // Erase the Rid done because there can be another commit
            }
        }
    }
            ...
            if (strpos($f, '/charts/') !== false) {
                ...
                if (($n >= 2) && ($f[$n - 1] === 'charts')) {
                    ...
                    if (substr($f, -4) === '.xml') {
                        ...
                        $this->OpenXmlCharts[$f] = ['idx' => $i, 'clean' => false, 'series' => false];
                    }
                }
            }
        }
    ...
    function OpenXML_ChartChangeSeries($ChartRef, $SeriesNameOrNum, $NewValues, $NewLegend = false)
    {
        ...
        if (!isset($this->OpenXmlCharts[$ref])) {
            ...
            } // try with $ChartRef as title
            if (!isset($this->OpenXmlCharts[$ref])) {
                return $this->RaiseError(
                    "(ChartChangeSeries) : unable to found the chart corresponding to '" . $ChartRef . "'."
                );
            }
        }
        ...
        if (is_array($SeriesNameOrNum)) {
            return $this->RaiseError(
                "(ChartChangeSeries) '$ChartRef' : The series reference is an array, a string or a number is expected. " . $ChartRef . "'."
            );
        } // usual mistake in arguments
        $ser = $this->OpenXML_ChartSeriesFound($Txt, $SeriesNameOrNum, $Delete);
        if ($ser === false) {
            return $this->RaiseError(
                "(ChartChangeSeries) '$ChartRef' : unable to found series '" . $SeriesNameOrNum . "' in the chart '" . $ref . "'."
            );
        }
        if ($Delete) {
    ...
    function OpenXML_ChartGetInfoFromFile($idx, $Txt = false)
    {
        ...
        if ($o->ChartLst === false) {
            ...
            while ($t = clsTbsXmlLoc::FindStartTag($Txt, 'c:chart', $p)) {
                ...
                $o->ChartLst[] = ['rid' => $rid, 'title' => $title, 'descr' => $descr, 'name' => $name];
                $p = $t->PosEnd;
            }
        }
        return $o->ChartLst;
    ...
    function OpenXML_SharedStrings_GetVal($id)
    {
        ...
        if (!isset($this->OpenXmlSharedStr[$id])) {
            ...
            while ($last_id < $id) {
                ...
                if ($p2 === false) {
                    return $this->RaiseError(
                        "(Excel SharedStrings) id $id is searched but no closing tag found for id $last_id."
                    );
                }
                $this->OpenXmlSharedStr[$last_id] = ['beg' => $p1, 'end' => $p2, 'len' => ($p2 - $p1)];
            }
        }
    ...
    function OpenMXL_GarbageCollector()
    {
        ...
        foreach ($this->CdFileLst as $idx => $f) {
            ...
            if (substr($n, 0, $pic_path_len) == $pic_path) {
                ...
                $pictures[] = ['name' => $n, 'idx' => $idx, 'nbr' => 0, 'short' => $short];
    ...
    function MsExcel_ConvertToRelative_Item(&$Txt, &$Loc, $Tag, $Att, $IsRow)
    {
        ...
        while (($p = clsTinyButStrong::f_Xml_FindTagStart($Txt, $Tag, true, $p, true, true)) !== false) {
            ...
            if (isset($Loc->PrmPos[$Att])) {
                ...
                if ($missing_nbr < 0) {
                    return $this->RaiseError(
                        '(Excel Consistency) error in counting items <' . $Tag . '>, found number ' . $r . ', previous was ' . $item_num
                    );
    ...
    {
        return $this->OpenXML_DeleteFile('xl/calcChain.xml', ['xl/workbook.xml']);
    }
                ...
                if (is_numeric($Value)) {
                    ...
                    if (strpos($Value, 'e') !== false) { // exponential representation
                        $Value = var_export(
                            (float)$Value,
                            true
                        ); // this string conversion is not affected by the decimal separator given by the locale setting
    ...
    function MsExcel_SheetInit()
    {
        ...
        $this->MsExcel_Sheets = [];   // sheet info sorted by location
        $idx = $this->FileGetIdx('xl/workbook.xml');
        $this->MsExcel_Sheets_WkbIdx = $idx;
        if ($idx === false) {
            return;
        }
        $Txt = $this->TbsStoreGet(
            $idx,
            'SheetInfo'
        ); // use the store, so the file will be available for editing if needed
        if ($Txt === false) {
            return false;
        }
        $this->TbsStorePut($idx, $Txt);
    ...
    function MsExcel_SheetGet($IdOrName)
    {
        ...
        foreach ($this->MsExcel_Sheets as $o) {
            ...
            if ($o->sheetId == $IdOrName) {
                return $o;
            }
        }
        return $this->RaiseError(
            "($Caller) The sheet '$IdOrName' is not found inside the Workbook. Try command OPENTBS_DEBUG_INFO to check all sheets inside the current Workbook."
        );
    }
    ...
    {
        $this->OpenXML_DeleteFile('xl/' . $file, ['xl/workbook.xml']);
    ...
    function MsPowerpoint_InitSlideLst($Master = false)
    {
        ...
        while ($loc = clsTbsXmlLoc::FindStartTag($Txt, $tag, $p)) {
            ...
            if ($rid === false) {
                ...
                $lst[] = ['file' => $f, 'idx' => $this->FileGetIdx($f), 'rid' => $rid];
            } else {
                $this->RaiseError(
                    "(Init Slide List) Slide corresponding to rid=$rid is not found in the Rels file of '$PresFile'."
                );
            }
            $p = $loc->PosEnd;
        }
    ...
    function MsPowerpoint_CleanRpr(&$Txt, $elem)
    {
        ...
        while (($p = $this->XML_FoundTagStart($Txt, '<' . $elem, $pe)) !== false) {
            $pe = $this->XML_DeleteAttributes($Txt, $p, ['noProof', 'lang', 'err', 'smtClean', 'dirty'], []);
        }
    }
    ...
    function MsPowerpoint_SlideDelete()
    {
        ...
        foreach ($del_lst2 as $f) {
            ...
            if (strpos($txt, $z)) {
                if ($first_kept === false) {
                    return $this->RaiseError(
                        "(Slide Delete and Display) : no slide left to replace the default slide in 'viewProps.xml.rels'."
                    );
                }
                $ok = true;
    ...
    function MsWord_Clean(&$Txt)
    {
        ...
        $this->XML_DeleteElements($Txt, ['w:proofErr', 'w:noProof', 'w:lang', 'w:lastRenderedPageBreak']);
                ...
                if ($p !== false) {
                    ...
                    if (($pc !== false) && ($po !== false) && ($pc < $po)) { // means that the attribute is actually inside a tag
                        $p2 = strpos(
                            ...
                            '"',
                            $p + $rs_len
                        ); // position of the delimiter that closes the attribute's value
                        if (($p2 !== false) && ($p2 < $pc)) {
    ...
    function MsWord_CleanDuplicatedLayout(&$Txt)
    {
        ...
        while (($wro_p = $this->XML_FoundTagStart($Txt, $wro, $wro_p)) !== false) { // next <w:r> tag
            ...
            if ($wto_p === false) {
                return false;
            } // error in the structure of the <w:r> element
            $first = true;
            $last_att = '';
            $first_att = '';
            do {
                ...
                if (($wto_p < $wrc_p) && ($wtc_p < $wrc_p)) { // if the <w:t> is actually included in the <w:r> element
                    if ($first) {
                        ...
                        $superflous = '</w:t></w:r>' . substr(
                                $Txt,
                                $wro_p,
                                ($wto_p + $wto_len) - $wro_p
                            ); // without the last symbol, like: '</w:t></w:r><w:r>....<w:t'
                        $superflous = str_replace(
                            '<w:tab/>',
                            '',
                            $superflous
                        ); // tabs must not be deleted between parts => they nt be in the superflous string
                    ...
                    $x = substr(
                        ...
                        $p_att,
                        1
                    ); // must be ' ' or '>' if the string is the superfluous AND the <w:t> tag has or not attributes
                    if ((($x === ' ') || ($x === '>')) && (substr($Txt, $wtc_p, $superflous_len) === $superflous)) {
                        $p_end = strpos($Txt, '>', $wtc_p + $superflous_len); //
                        if ($p_end === false) {
                            return false;
                        } // error in the structure of the <w:t> tag
                        ...
                        $Txt = substr_replace(
                            ...
                            $wtc_p,
                            $p_end - $wtc_p + 1
                        ); // delete superflous part + <w:t> attributes
                        $nbr++;
                        $ok = true;
                    }
                }
            ...
            if (($last_att !== '') && (strpos($first_att, $preserve) === false) && (strpos(
                        $last_att,
                        $preserve
                    ) !== false)
            ) {
    ...
    function MsWord_GetPage($Tag, $Txt, $Pos, $Forward, $LevelStop)
    {
        ...
        if ($Forward) {
            $p--;
        } // if it's forward, we stop the block before the paragraph with page-break
        return $p;
    }
    ...
    function MsWord_InitHeaderFooter()
    {
        ...
        $types_ok = ['default' => true, 'first' => false, 'even' => false];
            ...
            while ($loc = clsTbsXmlLoc::FindStartTag($Txt, $entity, $p)) {
                ...
                if (isset($types_ok[$type]) && $types_ok[$type]) {
                    ...
                    if (isset($rels->TargetLst[$rid])) {
                        ...
                        $files[] = ['file' => ('word/' . $target), 'type' => $type, 'place' => $place];
                    }
                }
            }
        }
    ...
    function OpenDoc_ChangeCellType(&$Txt, &$Loc, $Ope, $IsMerging, &$Value)
    {
        ...
        static $TypeLst = [
            'float'      => ['attval' => 'office:value'],
            'percentage' => ['attval' => 'office:value'],
            'currency'   => ['attval' => 'office:value', 'attcurr' => 'office:currency'],
            'boolean'    => ['attval' => 'office:boolean-value'],
            'date'       => ['attval' => 'office:date-value', 'frm' => 'yyyy-mm-ddThh:nn:ss'],
            'time'       => ['attval' => 'office:time-value', 'frm' => '"PT"hh"H"nn"M"ss"S"'],
        ];
        if (!isset($OpeLst[$Ope])) {
            return false;
        }
    ...
    function OpenDoc_SheetSlides_Init($sheet, $force = false)
    {
        ...
        $this->OpenDoc_SheetSlides = [];     // sheet/slide info sorted by location
        $idx = $this->Ext_GetMainIdx();
        if ($idx === false) {
            return;
        }
                ...
                if (isset($loc->PrmLst['style:name'])) {
                    ...
                    if (isset($styles_to_edit[$name])) {
                        ...
                        if (strpos($src, ' ' . $att_display . '="') === false) {
                            $src = str_replace(
                                '<' . $tag_property . ' ',
                                '<' . $tag_property . ' ' . $att_display . '="' . $yes_display . '" ',
                                $src
                            );
                        }
                        // add new styles
    ...
    function OpenDoc_SheetSlides_Debug($sheet, $nl, $sep, $bull)
    {
        ...
        foreach ($this->OpenDoc_SheetSlides as $idx => $loc) {
            $name = str_replace(['&amp;', '&quot;', '&lt;', '&gt;'], ['&', '"', '<', '>'], $loc->PrmLst[$att]);
    ...
    function OpenDoc_StylesInit()
    {
        ...
        $this->OpenDoc_Styles = [];     // sheet info sorted by location
        $Styles = [];
    ...
    function OpenDoc_ChartChangeSeries($ChartRef, $SeriesNameOrNum, $NewValues, $NewLegend = false)
    {
        ...
        if (is_numeric($ChartRef)) {
            ...
            if (!isset($this->OpenDocCharts[$idx])) {
                return $this->RaiseError("(ChartChangeSeries) : unable to found the chart $ChartCaption.");
            }
        } else {
        ...
        if ($data_idx === false) {
            return $this->RaiseError("(ChartChangeSeries) : unable to found the data in the chart $ChartCaption.");
        }
        ...
        if ($s_info === false) {
            return $this->RaiseError(
                "(ChartChangeSeries) : unable to found the series $s_caption in the chart " . $this->_ChartCaption . "."
            );
        }
        if ($NewLegend !== false) {
        ...
        } elseif ($s_use_cat && isset($NewValues[0]) && isset($NewValues[1]) && is_array($NewValues[0]) && is_array(
                $NewValues[1]
            )
        ) {
            // syntax 2: $NewValues = array( array('cat1','cat2',...), array(val1,val2,...) );		
                ...
                if ($elCell = clsTbsXmlLoc::FindElement($elRow, 'table:table-cell', $p_cell)) {
                    if ($i == $col_cat) {
                        ...
                        if ($s_use_cat) {
                            ...
                            $val_idx = $i - $s_col;
                            if ($data_r === false) {
                                $data_r = array_shift($data);
                            } // (may return null) delete the row in order to keep only unused
            ...
            for ($i = 1; $i <= $col_nbr; $i++) {
                if (($s_col <= $i) && ($i <= $s_colend)) {
                    ...
                    if ($s_use_cat && ($i == $col_cat)) {
                        $x .= '<table:table-cell office:value-type="string"><text:p>' . htmlspecialchars(
                                $cat
    ...
    function OpenDoc_ChartFindSeries(&$chart, $Txt)
    {
        ...
        $s_idx = 0;
        $series = [];
        $cols = []; // all columns attached to a series
        $cols_name = [];
        while ($elSeries = clsTbsXmlLoc::FindElement($Txt, 'chart:series', $p)) {
            ...
            // Add the series
            $series[$s_idx] = ['name' => false, 'col_name' => $col_name, 'cols' => $s_cols, 'ref' => $ref];
        ...
        if ($elHeaders === false) {
            return $this->RaiseError(
                "(ChartFindSeries) : unable to found the series names in the chart " . $this->_ChartCaption . "."
            );
        }
        $p = 0;
        $col_num = 0;
    ...
    function OpenDoc_ChartFindCol(&$cols, &$el, $att, $s_idx)
    {
        ...
        if ($x === false) {
            return $this->RaiseError(
                "(ChartFindCol) : unable to find cell references for series number #" . ($idx + 1) . " in the chart " . $this->_ChartCaption . "."
            );
        }
...
class clsTbsXmlLoc
{
    ...
    function __construct(&$Txt, $Name, $PosBeg, $SelfClosing = null, $Parent = false)
    {
        ...
        if ($this->PosEnd === false) {
            $this->PosEnd = strlen($Txt) - 1;
        } // should no happen but avoid errors
        $this->Txt = &$Txt;
        $this->Name = $Name;
        $this->PosBeg = $PosBeg;
    ...
    static function FindStartTagHavingAtt(&$Txt, $Att, $PosBeg, $Forward = true)
    {
        ...
        $x = (strpos(
                $Att,
                '='
            ) === false) ? (' ' . $Att . '="') : (' ' . $Att); // get the item more precise if not yet done
        $search = true;
        do {
            if ($Forward) {
                $p = strpos($Txt, $x, $p + 1);
    ...
    function ReplaceSrc($new)
    {
        ...
        if ($new === '') {
            ...
            if ($this->pET_PosBeg !== false) {
                $this->pET_PosBeg += $diff;
            } // CAUTION: right only if the tag name is the same
        }
    }
    ...
    {
        return ($this->pET_PosBeg === false) ? false : substr(
            $this->Txt,
            $this->pST_PosEnd + 1,
            $this->pET_PosBeg - $this->pST_PosEnd - 1
        );
    }
    ...
    function FindEndTag($Encaps = false)
    {
        if (is_null($this->SelfClosing)) {
            ...
            if (!$SelfClosing) {
                if ($Encaps) {
                    $loc = clsTinyButStrong::f_Xml_FindTag(
                        $this->Txt,
                        $this->FindName(),
                        null,
                        $pe,
