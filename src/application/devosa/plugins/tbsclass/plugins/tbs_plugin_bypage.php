...
class tbsByPage
{
    ...
    function OnInstall()
    {
        $this->Version = '1.0.5';
        $this->PageSize = 0;
        return ['OnCommand', 'BeforeMergeBlock', 'AfterMergeBlock'];
    }
    ...
    function BeforeMergeBlock(&$TplSource, &$BlockBeg, &$BlockEnd, $PrmLst, &$Src)
    {
        if ($this->PageSize <= 0) {
            return;
        }   // ByPage Mode not actived
        if (isset($Src->ByPage)) {
            return;
        }  // ByPage Mode already processed for the current Data source
        if ($Src->RecSet === false) {
            return;
        } // No data available
        if ($Src->RecSaved) {
            ...
            while (($Src->CurrRec !== false) and ($RecNum !== $RecStop)) {
                ...
                if ($Src->CurrRec !== false) {
                    ...
                    if ($Modulo === $ModuloStop) {
                        $Src->RecBuffer = [$Src->RecKey => $Src->CurrRec];
