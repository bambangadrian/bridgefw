...
class clsOpenTBS extends clsTbsZip
{
    ...
    function BeforeLoadTemplate(&$File, &$Charset)
    {
        ...
        if (($SubFileLst !== '') && ($SubFileLst !== false)) {
            ...
            foreach ($SubFileLst as $SubFile) {
                ...
                if ($idx === false) {
                    $this->RaiseError(
                        'The file "' . $SubFile . '" is not found in the archive "' . $this->ArchFile . '".'
                    );
                    ...
                    if ($this->LastReadNotStored) {
                        if ($this->LastReadComp <= 0) { // the contents is not compressed
                            if ($this->ExtInfo !== false) {
                                ...
                                if (($i['ext'] === 'docx') && isset($TBS->OtbsClearMsWord) && $TBS->OtbsClearMsWord) {
    ...
    function BeforeShow(&$Render, $File = '')
    {
        ...
        if (isset($this->OtbsSheetODS)) {
            $this->OpenDoc_SheetDeleteAndDisplay();
        }
        if (isset($this->OtbsSheetXLSX)) {
            $this->MsExcel_SheetDeleteAndDisplay();
        }
        if ($this->Ext_Get() == 'docx') {
            $this->MsWord_RenumDocPr();
        }
        ...
        $TBS->Plugin(-10); // reactivate other plugins
        $this->TbsCurrIdx = false;
        if (isset($this->OpenXmlRid)) {
            $this->OpenXML_RidCommit($Debug);
        }       // Commit special OpenXML features if any
        if (isset($this->OpenXmlCTypes)) {
            $this->OpenXML_CTypesCommit($Debug);
        }    // Commit special OpenXML features if any
        if (isset($this->OpenDocManif)) {
        ...
        if ($ope === 'addpic') {
            ...
            if (!isset($PrmLst['pic_change'])) {
                $this->TbsPicFound(
                    $Txt,
                    $Loc
                );  // add parameter "att" which will be processed just before the value is merged
                $PrmLst['pic_change'] = true;
            }
            $this->TbsPicAdd($Value, $PrmLst, $Txt, $Loc, 'ope=changepic');
        } elseif (substr($ope, 0, 4) === 'xlsx') {
            if (!isset($Loc->PrmLst['xlsxok'])) {
                $this->MsExcel_ChangeCellType($Txt, $Loc, $ope);
            }
    ...
    function OnCommand($Cmd, $x1 = null, $x2 = null, $x3 = null, $x4 = null, $x5 = null)
    {
        if ($Cmd == OPENTBS_INFO) {
            ...
            return $this->OpenXML_ChartChangeSeries(
                $ChartNameOrNum,
                $SeriesNameOrNum,
                $NewValues,
                $NewLegend,
            ...
            if ($this->Ext_Get() === 'xlsx') {
                ...
                if ($this->FileExists($loc->xlsxTarget)) {
                    ...
                    return $this->RaiseError(
                        "($Cmd) sub-file '" . $loc->xlsxTarget . "' is not found inside the Workbook."
                    );
                }
            }
            return true;
            ...
            if (!isset($this->OtbsSheetOk)) {
                if ($ext == 'xlsx') {
                    $this->OtbsSheetXLSX = true;
                }
                if ($ext == 'ods') {
                    $this->OtbsSheetODS = true;
                }
                $this->OtbsSheetDelete = [];
                $this->OtbsSheetVisible = [];
            ...
            foreach ($x1 as $sheet => $action) {
                ...
                $sheet_ref = (is_string($sheet)) ? 'n:' . htmlspecialchars(
                        $sheet
                    ) : 'i:' . $sheet; // help to make the difference beetween id and name
                if ($delete) {
                    if ($x2) {
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
    ...
    function TbsDebug_Info($Exit)
    {
        ...
        if ($this->Ext_Get() === 'ods') {
            $this->OpenDoc_SheetDebug($nl, $sep, $bull);
        }
        ...
        if (isset($this->ExtInfo['frm'])) {
            if ($this->ExtInfo['frm'] === 'odf') {
                ...
                if (isset($Loc->PrmLst['adjust'])) {
                    $Loc->otbsDim = $this->TbsPicGetDim_ODF($Txt, $Loc->PosBeg);
                }
    ...
    function TbsPicGetDim_OpenXML_dml($Txt, $Pos)
    {
        ...
        if (($dim_inner !== false) && ($dim_inner['wb'] < $dim_shape['wb'])) {
            $dim_inner = false;
        } // <a:ext> isoptional but must always be after the corresponding <wp:extent>, otherwise it may be the <a:ext> of another picture
        return [$dim_inner, $dim_shape]; // dims must be soerted in reverse order of location
    }
    ...
    function TbsPicAdd(&$Value, &$PrmLst, &$Txt, &$Loc, $Prm)
    {
        ...
        if (!file_exists($FullPath)) {
            if (isset($PrmLst['default'])) {
                ...
                $ok = $this->RaiseError(
                    'The picture "' . $FullPath . '" that is supposed to be added because of parameter "' . $Prm . '" of the field [' . $Loc->FullName . '] is not found. You can use parameter default=current to cancel this message'
                );
            }
        }
        ...
        if ($ok) {
            ...
            if ($this->FileGetIdxAdd($InternalPath) === false) {
                $this->FileAdd($InternalPath, $FullPath, TBSZIP_FILE, true);
            }
        ...
        if (isset($Loc->otbsDim)) {
            ...
            if ($ok) {
                $this->TbsPicAdjust($Txt, $Loc, $FullPath);
            }
        }
    ...
    function Ext_PrepareInfo($Ext = false)
    {
        ...
        if (isset($GLOBAL['_OPENTBS_AutoExt'][$Ext])) {
            ...
            if (!isset($this->TBS->OtbsClearMsWord)) {
                $this->TBS->OtbsClearMsWord = true;
            }
            if (!isset($this->TBS->OtbsMsExcelConsistent)) {
                $this->TBS->OtbsMsExcelConsistent = true;
            }
            ...
            if ($Ext === 'docx') {
                ...
                $i['load'] = $this->OpenXML_MapGetFiles(['presentationml.notesSlide+xml']);
            }
            $i['pic_ext'] = [
                'png'  => 'png',
                'bmp'  => 'bmp',
    ...
    function Ext_GetFormat(&$Ext, $Search)
    {
        ...
        if (strpos(',docx,xlsx,pptx,', ',' . $Ext . ',') !== false) {
            return 'openxml';
        }
        if (!$Search) {
            return false;
    ...
    function XML_GetInnerVal($Txt, $Tag, $Concat = false)
    {
        ...
        while (($p = $this->XML_FoundTagStart($Txt, $Tag, $p3)) !== false) {
            ...
            $p2 = strpos($Txt, '>', $p);
            if ($p2 == false) {
                return $this->RaiseError('(XML) the end of tag ' . $Tag . ' is not found.');
            }
            $p2++;
            $p3 = strpos($Txt, $close, $p2);
            if ($p3 == false) {
                exit("strpos($Txt, $close, $p2) p=$p ; nbr=$nbr");
            }
            if ($p3 == false) {
                return $this->RaiseError('(XML) the closing tag ' . $Tag . ' is not found.');
            }
...
*/
        ...
        if ($o->FicType === false) {
            ...
            if ($FicIdx === false) {
                ...
                while (($p = strpos($Txt, $zImg, $p + 1)) !== false) {
                    ...
                    if ($p2 === false) {
                        return $this->RaiseError(
                            "(OpenXML) end of attribute Target not found in position " . $p1 . " of subfile " . $o->FicPath
                        );
                    }
                    ...
                    if ($p1 === false) {
                        return $this->RaiseError(
                            "(OpenXML) begining of tag not found in position " . $p . " of subfile " . $o->FicPath
                        );
                    }
                    ...
                    if ($p1 !== false) {
                        ...
                        if ($p2 === false) {
                            return $this->RaiseError(
                                "(OpenXML) end of attribute Id not found in position " . $p1 . " of subfile " . $o->FicPath
                            );
                        }
        ...
        if (isset($o->RidLst[$ImageName])) {
            return $o->RidLst[$ImageName];
        }
...
*/
        ...
        $ext = strtolower($ext);
        if (isset($this->OpenXmlCTypes[$ext]) && ($this->OpenXmlCTypes[$ext] !== '')) {
            return;
        }
    ...
    function OpenXML_ChartChangeSeries(
        $ChartNameOrNum,
        $SeriesNameOrNum,
        $NewValues,
        $NewLegend = false,
        $CopyFromSeries = false
    )
    {
        if (!isset($this->OpenXmlCharts)) {
            $this->OpenXML_ChartInit();
        ...
        if (!isset($this->OpenXmlCharts[$ref])) {
            return $this->RaiseError(
                "(ChartChangeSeries) unable to found the chart corresponding to '" . $ChartNameOrNum . "'."
            );
        }
        ...
        if ($ser === false) {
            return $this->RaiseError(
                "(ChartChangeSeries) unable to found series '" . $SeriesNameOrNum . "' in the chart '" . $ref . "'."
            );
        }
        if ($Delete) {
    ...
    function OpenXML_SharedStrings_GetVal($id)
    {
        ...
        if (!isset($this->OpenXmlSharedStr)) {
        ...
        for ($i = strlen($ColRef) - 1; $i >= 0; $i--) {
            ...
            if (!is_numeric($l)) {
                ...
                if ($l > 0 && $l < 27) {
                    ...
                    return $this->RaiseError(
                        '(Excel Consistency) Reference of cell \'' . $ColRef . '\' cannot be recognized.'
                    );
                }
                $rank++;
            }
    ...
    function MsExcel_ChangeCellType(&$Txt, &$Loc, $Ope)
    {
        ...
        static $OpeLst = ['xlsxBool' => ' t="b"', 'xlsxDate' => '', 'xlsxNum' => ''];
        if (!isset($OpeLst[$Ope])) {
            return false;
        }
    ...
    {
        if (isset($this->MsExcel_Sheets)) {
            return;
        }
        $this->MsExcel_Sheets = [];   // sheet info sorted by location
        $this->MsExcel_SheetsById = []; // shorcut for ids
        $this->MsExcel_SheetsByName = []; // shorcut for names
        ...
        while ($loc = clsTinyButStrong::f_Xml_FindTag($Txt, 'sheet', true, $p, true, false, true, true)) {
            if (isset($loc->PrmLst['sheetid'])) {
                ...
                if (isset($loc->PrmLst['r:id'])) {
                    $rels[$loc->PrmLst['r:id']] = $idx;
                }
                $this->MsExcel_SheetsById[$id] =& $this->MsExcel_Sheets[$idx];
                if (isset($loc->PrmLst['name'])) {
                    $this->MsExcel_SheetsByName[$loc->PrmLst['name']] =& $this->MsExcel_Sheets[$idx];
                }
                $idx++;
            }
            $p = $loc->PosEnd;
        ...
        while ($loc = clsTinyButStrong::f_Xml_FindTag($Txt, 'Relationship', true, $p, true, false, true, false)) {
            if (isset($loc->PrmLst['id']) && isset($loc->PrmLst['target'])) {
                $rid = $loc->PrmLst['id'];
                if (isset($rels[$rid])) {
                    $this->MsExcel_Sheets[$rels[$rid]]->xlsxTarget = 'xl/' . $loc->PrmLst['target'];
                }
            }
            $p = $loc->PosEnd;
        }
    ...
    function MsExcel_SheetGet($IdOrName, $Caller, $CheckTarget = false)
    {
        ...
        if ($CheckTarget && (!isset($loc->xlsxTarget))) {
            return $this->RaiseError(
                "($Caller) Error with sheet '$IdOrName'. The corresponding XML subfile is not referenced."
            );
        }
        return $loc;
    }
    ...
    function MsWord_CleanDuplicatedLayout(&$Txt)
    {
        ...
        while (($wro_p = $this->XML_FoundTagStart($Txt, $wro, $wro_p)) !== false) {
            $wto_p = $this->XML_FoundTagStart($Txt, $wto, $wro_p);
            if ($wto_p === false) {
                return false;
            ...
            do {
                $ok = false;
                $wtc_p = $this->XML_FoundTagStart($Txt, $wtc, $wto_p);
                if ($wtc_p === false) {
                    return false;
                } // error in the structure of the <w:r> element
                $wrc_p = $this->XML_FoundTagStart($Txt, $wrc, $wro_p);
                if ($wrc_p === false) {
                    return false;
                } // error in the structure of the <w:r> element
                if (($wto_p < $wrc_p) && ($wtc_p < $wrc_p)) { // if the found <w:t> is actually included in the <w:r> element
                    if ($first) {
                        $superflous = '</w:t></w:r>' . substr(
                                ...
                                $wro_p,
                                ($wto_p + $wto_len) - $wro_p
                            ); // should be like: '</w:t></w:r><w:r>....<w:t'
	...
	*/
    ...
    {
        if (isset($this->OpenDoc_Sheets) && (!$force)) {
            return;
        }
        $this->OpenDoc_Sheets = [];     // sheet info sorted by location
        $idx = $this->FileGetIdx($this->ExtInfo['main']);
        if ($idx === false) {
            return;
        }
    ...
    function OpenDoc_SheetDebug($nl, $sep, $bull)
    {
        ...
        foreach ($this->OpenDoc_Sheets as $idx => $loc) {
            $name = str_replace(['&amp;', '&quot;', '&lt;', '&gt;'], ['&', '"', '<', '>'], $loc->PrmLst['table:name']);
...
class clsTbsZip
{
    ...
    function CentralDirRead()
    {
        ...
        $b = $this->_ReadData(4);
        if ($b !== $cd_info) {
            return $this->RaiseError('The footer of the Central Directory is not found.');
        }
        ...
        $this->CdPos = $this->CdInfo['p_cd'];
        if ($this->CdFileNbr <= 0) {
            return $this->RaiseError('No file found in the Central Directory.');
        }
        if ($this->CdPos <= 0) {
            return $this->RaiseError('No position found for the Central Directory listing.');
        }
    ...
    function CentralDirRead_File($idx)
    {
        ...
        if ($x !== 'h:02014b50') {
            return $this->RaiseError(
                'Signature of file information not found in the Central Directory in position ' . (ftell(
                        $this->ArchHnd
                    ) - 46) . ' for file #' . $idx . '.'
            );
        }
    ...
    function RaiseError($Msg)
    {
        if ($this->DisplayError) {
            echo '<strong>' . get_class($this) . ' ERROR :</strong> ' . $Msg . '<br>' . "\r\n";
        }
        $this->Error = $Msg;
        return false;
    }
    ...
    function _ReadFile($idx, $ReadData)
    {
        ...
        if ($x !== 'h:04034b50') {
            return $this->RaiseError(
                'Signature of file information not found in the Data Section in position ' . (ftell(
                        $this->ArchHnd
                    ) - 30) . ' for file #' . $idx . '.'
            );
        }
