...
var AC_descriptionElements = new Array(); // menampung data element untuk masing-masing tipe
...
function initAutoComplete() {
    AC_DetectBrowser();
    AC_totalType = AC_allCanons.length;
    ...
    setTimeout("AC_OnLoad()", 100);
}

function AC_OnLoad() {
    //window.AC_selectedItem = [];
    ...
    // TODO: need a more-general version of this for other properties (e.g., not directly description-related)
...
function refreshAutoComplete() {
    ...
    for (var tg = 0; tg < AC_totalType; tg++) {
        if (window.AC_nickTokens[tg] != null && AC_nickTokens[tg].length > 0 && AC_nickTokens[tg] [AC_nickTokens[tg].length - 1] == null)
            AC_nickTokens[tg].length -= 1;
        if (window.AC_allCanons[tg] != null && AC_allCanons[tg].length > 0 && AC_allCanons[tg] [AC_allCanons[tg].length - 1] == null)
            AC_allCanons[tg].length -= 1;
        ...
        if (typeof AC_targetElements[tg] != "undefined" && AC_targetElements[tg] != null) {
            for (var index = 0, numEls = AC_targetElements[tg].length; index < numEls; ++index) {
                ...
                if (field != null) {
                    ...
                    field.descriptionElement = null;
                    if (typeof AC_descriptionElements[tg] != "undefined" && AC_descriptionElements[tg] != null)
                        if (AC_descriptionElements[tg][index] != null || AC_descriptionElements[tg][index] != '') {
                            var descEl = window.document.getElementById(AC_descriptionElements[tg][index]);
                            if (descEl)
                                field.descriptionElement = descEl;
                        }
...
function AC_DetectBrowser() {
    ...
    this.AC_isGecko = ( navigator.product == 'Gecko' );
    this.AC_isIE = !AC_isGecko && ( ua.indexOf("msie") != -1 );
    this.AC_isSafari = !AC_isGecko && !AC_isIE && ( ua.indexOf("safari") != -1 );
    this.AC_isOther = !AC_isGecko && !AC_isIE && !AC_isSafari;
}
...
function AC_UpdateDropDown(editCtrl, op) {
    ...
    switch (op) {
        ...
        case "selectNext":
            ...
            break;
        case "selectNextPage":
            newSelection += AC_MaxVisibleItems;
            break;
        case "selectPrevPage":
            newSelection -= AC_MaxVisibleItems;
            break;
...
function AC_InsertSuggestion(editCtrl) {
    ...
    // also remember the description for a send-time check:
    if (editCtrl.descriptionElement != null)
        editCtrl.descriptionElement.innerHTML = AC_Rot13(AC_allCanons[intType] [item.value + 1]);
    //AC_selectedItem [ AC_selectedItem.length ] = AC_Rot13 ( AC_allCanons[intType] [ item.value+1 ]);
...
function AC_Rot13(inputText) {
    ...
    for (var i = 0, n = inputText.length; i < n; ++i) {
        resultText [i] = inputText.charAt(i);
    }
...
function AC_DeLatin(inputText) {
    ...
    for (var i = 0, n = inputText.length; i < n; ++i) {
        ...
        resultText [resultText.length] = inputText.charAt(i);
    }
...
function AC_OuterMenuExpr(input) {
    ...
    // menu text includes HTML escaped description
    input = input.replace(/</, "&lt;");
...
// takes the search results, sorts them and formats for display (e.g. highlight the match text)
function AC_CompileSuggestions() {
    ...
    var total = 0;
    var seenAddresses = []; // TODO: this is index-based.  make it actual-description-based.
...
function AC_FindLinear(inputText) {
    ...
    if (AC_substringMatchesLinear[intType][inputText] == null) {
        ...
        if (inputText.length > 1 /*&& ( window.document.Compose.Narrow == null || window.document.Compose.Narrow.checked )*/) {
            ...
            if (prefixMatches == null) AC_FindLinear(prefix); // fast typing can cause this - recur
...
function AC_FindLinearCanons(inputText, fromList) {
    ...
    for (var i = 0, n = fromList.length; i < n; i += 2) { // note increment - this is a canon list
        ...
        if (name.search(inputExpr) != -1 ||
            ...
            // note how this works: we check for a match against EITHER the name or the description.
            ...
            // BOTH name and description as the string gets longer.  we rely on the fact that this
