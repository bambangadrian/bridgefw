            ...
            case 'radio' :
                if ($obj['visible']) {
                    if ($this->readonly) {
                        ...
                        $strPreResult = "<label class=\"radio-inline\"><input $required class=r type=\"radio\" id=\"" . $obj['name'] . "\" name=\"" . $obj['name'] . "\" " . $obj['attribute'];
                        if (!$obj['enabled']) {
            ...
            if ($i < $colCount) {
                ...
                ($j < (($i + 1) * $jmlBaris) && $j < count($this->formObject[$tabPageNumber][$groupNumber])); $j++
                    ...
                    if (!($this->formObject[$tabPageNumber][$groupNumber][$j]['renderLabel'])) {
                        ...
                        $strResult .= "<div class=\"col-sm-8 $addedClass\">" . $this->_printObject(
                                $this->formObject[$tabPageNumber][$groupNumber][$j]
                            ) . "</div></div>";
                    }
                }
    ...
    function render()
    {
        ...
        if ($this->message != '') {
            ...
            if ($this->msgClass == "bgOK") {
                $addedClass = "alert alert-success";
            }
            $strResult .= "<div id=\"formMessage\" name=\"formMessage\" class=\"" . $this->msgClass . " " . $addedClass . "\">" . $this->message . "</div>";
        } else
