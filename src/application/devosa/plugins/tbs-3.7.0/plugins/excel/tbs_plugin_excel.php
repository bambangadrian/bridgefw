...
class clsTbsExcel
{
    ...
    function BeforeLoadTemplate(&$File, &$Charset)
    {
        ...
        if ($Charset === '') {
            $this->TBS->LoadTemplate('', [&$this, 'ConvXmlUtf8']);
        } // Define the function for string conversion
    }
    ...
    function OnCacheField($BlockName, &$Loc, &$Txt, $PrmProc)
    {
        if (isset($Loc->PrmLst['ope'])) {
            ...
            foreach ($ope_lst as $ope) {
                $ope = trim($ope, ' ');
                if (isset($this->TypeLst[$ope])) {
                    $ope_att = $this->TypeLst[$ope];
                }
            }
            ...
            if ($ope_att !== false) {
                ...
                $Txt = substr_replace(
                    ...
                    $Loc->PosEnd + 1,
                    0
                ); // the new field is added after the current field so it will be scanned by TBS
    ...
    function BeforeMergeBlock(&$TplSource, &$BlockBeg, &$BlockEnd, $PrmLst, &$DataSrc, &$LocR)
    {
        ...
        */
        ...
        if ($LocR->SectionNbr > 0) {
            ...
            $Start = '';
            if (substr($Src1, 0, 4) === '<Row') {
                $Start = '<Row';
            }
            if (substr($Src1, 0, 5) === '<Cell') {
                $Start = '<Cell';
            }
            if ($Start !== '') {
    ...
    function AfterShow(&$Render, $File = '')
    {
        ...
        if (($TBS->ErrCount > 0) && (!$TBS->NoErr)) {
            $TBS->meth_Misc_Alert(
                'Show() Method',
                'The output is cancelled by the Excel plugin because at least one TBS error has occured.'
            );
            exit;
        }
        if ($File === '') {
    ...
    function tag_DelOptionalTableAtt()
    {
        ...
        $Txt =& $this->TBS->Source;
        $att_lst = ['ss:ExpandedColumnCount', 'ss:ExpandedRowCount'];
        $Pos = 0;
        while (($Loc = clsTinyButStrong370::f_Xml_FindTag(
                $Txt,
                'Table',
                true,
                $Pos,
