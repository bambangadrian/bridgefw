...
*/
// Check PHP version
if (version_compare(PHP_VERSION, '4.0.6') < 0) {
    ...
    function DataOpen(&$Query)
    {
        ...
        switch ($this->Type) {
            ...
            case 4: // Custom method from ObjectRef
                $this->RecSet = call_user_func_array($this->FctOpen, [&$this->SrcId, &$Query]);
                if ($this->RecSet === false) {
                    $this->DataAlert(
                        'method ' . get_class(
                            $this->FctOpen[0]
    ...
    function DataFetch()
    {
        ...
        switch ($this->Type) {
            ...
            case 5: // Custom method of object
                ...
                break;
            case 7: // PostgreSQL
                $this->CurrRec = @pg_fetch_array(
                    $this->RecSet,
                    $this->RecNum,
                    PGSQL_ASSOC
                ); // warning comes when no record left.
                break;
            case 8: // SQLite
    ...
    function meth_Locator_FindTbs(&$Txt, $Name, $Pos, $ChrSub)
    {
        ...
        do {
            ...
            if ($Pos === false) {
                ...
                $Loc->PosBeg = $Pos;
                if ($ReadPrm) {
                    clsTinyButStrong::f_Loc_PrmRead(
                        $Txt,
                        $PosX,
                        false,
                        '\'',
                ...
                if ($BegCnt === 0) {
                    ...
                    if (($Chr === ' ') or ($Chr === "\r") or ($Chr === "\n")) {
                        if ($Status === 1) {
                            ...
                            clsTinyButStrong::f_Loc_PrmCompute(
                                $Txt,
                                $Loc,
                                $SubName,
                                $Status,
...
*/
    ...
    function f_Loc_EnlargeToTag(&$Txt, &$Loc, $TagLst, $RetInnerSrc)
    {
        ...
        if ($TagIsSgl[$Ref]) {
            ...
            if ($LevelStop > 0) {
                $TagC = clsTinyButStrong::f_Xml_FindTag(
                    $Txt,
                    $TagLst[$Ref],
                    false,
                    $Loc->PosEnd + 1,
...
*/
        ...
        if ($Case) {
            ...
            do {
                ...
                if (strcasecmp($x, substr($Txt, $p, $xl)) != 0) {
                    continue;
                } // For PHP 4 only
                $z = substr($Txt, $p + $xl, 1);
            } while (($z !== ' ') && ($z !== "\r") && ($z !== "\n") && ($z !== '>') && ($z !== '/') && ($Tag !== '/') && ($Tag !== ''));
