...
if (isset($_GET['source'])) {
    exit('<!DOCTYPE HTML><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>OpenTBS plug-in for TinyButStrong - demo source</title></head><body>' . highlight_file(
            __FILE__,
            true
        ) . '</body></html>');
}
...
if (!isset($_POST['btn_go'])) {
    exit("You must use demo.html");
}
$suffix = (isset($_POST['suffix']) && (trim(
            $_POST['suffix']
        ) !== '') && ($_SERVER['SERVER_NAME'] == 'localhost')) ? trim($_POST['suffix']) : '';
...
$data = [];
$data[] = ['firstname' => 'Sandra', 'name' => 'Hill', 'number' => '1523d'];
$data[] = ['firstname' => 'Roger', 'name' => 'Smith', 'number' => '1234f'];
$data[] = ['firstname' => 'William', 'name' => 'Mac Dowell', 'number' => '5491y'];
$x_num = 3152.456;
$x_pc = 0.2567;
