...
class clsOpenTBS extends clsTbsZip
{
                ...
                if ($idx === false) {
                    ...
                    if (isset($this->TbsSrcParking[$idx])) {
                        ...
                        if ($ok && ($this->ArchExtInfo !== false)) {
                            if (isset($this->ArchExtInfo['rpl_what'])) {
                                $TBS->Source = str_replace(
                                    $this->ArchExtInfo['rpl_what'],
                                    $this->ArchExtInfo['rpl_with'],
                                    $TBS->Source
                                );
                            } // auto replace strings in the loaded file
                            if (($this->ArchExt === 'docx') && isset($TBS->OtbsClearMsWord) && $TBS->OtbsClearMsWord) {
                                $this->MsWord_Clean($TBS->Source);
                            }
                        }
                    }
                    // apply default TBS behaviors on the uncompressed content: other plug-ins + [onload] fields
                    if ($ok & $TbsLoadTemplate) {
    ...
    function BeforeShow(&$Render, $File = '')
    {
        ...
        foreach ($idx_lst as $idx) {
            ...
            if ($Debug) {
                $this->DebugLst[$this->CdFileLst[$idx]['v_name']] = $TBS->Source;
            }
...
If they are blank spaces, line beaks, or other unexpected characters, then you have to check your code in order to avoid them.";
    ...
    function RaiseError($Msg, $NoErrMsg = false)
    {
        ...
        if ($exit) {
            if ($this->DebugLst !== false) {
                if ($this->TbsCurrIdx !== false) {
                    $this->DebugLst[$this->CdFileLst[$this->TbsCurrIdx]['v_name']] = $this->TBS->Source;
                }
                $this->TbsDebug(true);
            }
            exit;
    ...
    function TbsPicAdd(&$Value, &$PrmLst, $Loc, $Prm)
    {
        ...
        if ($ok) {
            ...
            if (isset($this->ArchExtInfo['pic_path'])) {
                $InternalPath = $this->ArchExtInfo['pic_path'] . $InternalPath;
            }
        ...
        if (isset($GLOBAL['_OPENTBS_AutoExt'][$Ext])) {
            ...
            if ($Ext === 'docx') {
                ...
                $i['load'] = $this->OpenXML_MapGetVal(
                    ...
                    ';',
                    true
                ); // footnotes and endnotes omitted for perf
            } elseif ($Ext === 'xlsx') {
                $i = [
                    'load'     => 'xl/worksheets/sheet1.xml;xl/sharedStrings.xml',
                    'br'       => false,
                    'frm'      => 'openxml',
                ...
                $i['load'] = $this->OpenXML_MapGetVal(
                    ['spreadsheetml.worksheet+xml', 'spreadsheetml.sharedStrings+xml'],
                    $i['load'],
                    ';',
                    true
                );
                ...
                $i['load'] = $this->OpenXML_MapGetVal(
                    ...
                    ';',
                    true
                ); // masternotes and slidenotes omitted for perf
            }
            $i['pic_ext'] = [
                'png'  => 'png',
                'bmp'  => 'bmp',
    ...
    function OpenXML_CTypesPrepare($FileOrExt, $ct = '')
    {
        ...
        */
        ...
        if (($ct === '') && isset($this->ArchExtInfo['pic_ext'][$ext])) {
            $ct = 'image/' . $this->ArchExtInfo['pic_ext'][$ext];
        }
    ...
    function MsWord_Clean(&$Txt)
    {
        ...
        $this->MsWord_CleanTag($Txt, ['<w:proofErr', '<w:noProof', '<w:lang', '<w:lastRenderedPageBreak']);
    ...
    function MsWord_CleanDuplicatedLayout(&$Txt)
    {
        ...
        while (($wro_p = $this->MsWord_FoundTag($Txt, $wro, $wro_p)) !== false) {
            $wto_p = $this->MsWord_FoundTag($Txt, $wto, $wro_p);
            if ($wto_p === false) {
                return false;
            ...
            do {
                $ok = false;
                $wtc_p = $this->MsWord_FoundTag($Txt, $wtc, $wto_p);
                if ($wtc_p === false) {
                    return false;
                } // error in the structure of the <w:r> element
                $wrc_p = $this->MsWord_FoundTag($Txt, $wrc, $wro_p);
                if ($wrc_p === false) {
                    return false;
                } // error in the structure of the <w:r> element
    ...
    function OpenDoc_ManifestChange($Path, $Type)
    {
        ...
        if (($Type === '') && (substr($Path, 0, 9) === 'Pictures/')) {
            ...
            if ($p !== false) {
                ...
                if (isset($this->ArchExtInfo['pic_ext'][$ext])) {
                    $Type = 'image/' . $this->ArchExtInfo['pic_ext'][$ext];
                }
            }
        }
    ...
    function OpenDoc_ManifestCommit($Debug)
    {
        ...
        if (isset($this->TbsSrcParking[$idx])) {
            ...
            if ($this->LastReadComp > 0) {
                return $this->RaiseError("(OpenDocumentFormat) unable to uncompress 'META-INF/manifest.xml'.");
            }
        }
        // Perform all changes
...
class clsTbsZip
{
    ...
    function RaiseError($Msg)
    {
        if ($this->DisplayError) {
            echo '<strong>' . get_class($this) . ' ERROR : ' . $Msg . '</strong><br>' . "\r\n";
        }
        $this->Error = $Msg;
        return false;
    }
