...
class clsTbsCacheSytem
{
    ...
    function OnInstall($CacheDir = false, $CacheMask = false)
    {
        ...
        $this->CacheFile = [];
        $TBS =& $this->TBS;
        if (!isset($TBS->CacheMask)) {
            $TBS->CacheMask = 'cache_tbs_*.php';
        } // for compatibility
        if (!isset($TBS->CacheDir)) {
            $TBS->CacheDir = '';
        }
        if ($CacheMask !== false) {
            $TBS->CacheMask = $CacheMask;
        }
        if ($CacheDir !== false) {
            $TBS->CacheDir = $CacheDir;
        }
        return ['OnCommand', 'BeforeShow', 'AfterShow'];
    }
    ...
    {
        if ($this->ShowFromCache) {
            return false;
        } // Cancel automatic merges
    }
    ...
    function meth_Cache_Save($CacheFile, &$Txt)
    {
        ...
        if ($fid === false) {
            $this->TBS->meth_Misc_Alert(
                'CacheSystem plug-in',
                'The cache file \'' . $CacheFile . '\' can not be saved.'
            );
            return false;
        } else {
