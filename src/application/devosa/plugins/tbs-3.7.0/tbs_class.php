...
// Plug-ins actions
if (!defined('TBS_INSTALL')) {
    define('TBS_INSTALL', -1);
}
if (!defined('TBS_ISINSTALLED')) {
                ...
                if ($this->SubType === 0) {
                    ...
                    if ($Item0[0] === '~') {
                        ...
                        if (isset($GLOBALS[$Item0])) {
                            ...
                            $i = $this->DataAlert(
                                'invalid query \'' . $Query . '\' because global variable \'' . $Item0 . '\' is not found.'
                            );
                        }
                    }
                    // Check sub-items
        ...
        if ($Chrs !== '') {
            ...
            if ($Ok) {
                ...
                $this->meth_Misc_Alert(
                    'with clsTinyButStrong370() function',
                ...
                if ($BegCnt === 0) {
                    ...
                    if (($Chr === ' ') or ($Chr === "\r") or ($Chr === "\n")) {
                        if ($Status === 1) {
                            ...
                            clsTinyButStrong370::f_Loc_PrmCompute(
                                $Txt,
                                $Loc,
                                $SubName,
                                $Status,
...
*/
    ...
    static function f_Loc_EnlargeToTag(&$Txt, &$Loc, $TagLst, $RetInnerSrc)
    {
        ...
        $TagO = clsTinyButStrong370::f_Xml_FindTag(
            $Txt,
            $TagLst[$Ref],
            true,
            $Loc->PosBeg - 1,
        ...
        if ($TagIsSgl[$Ref]) {
            ...
            if ($LevelStop > 0) {
                $TagC = clsTinyButStrong370::f_Xml_FindTag(
                    $Txt,
                    $TagLst[$Ref],
                    false,
                    $Loc->PosEnd + 1,
    ...
    static function f_Xml_AttFind(&$Txt, &$Loc, $Move = false, $AttDelim = false)
    {
        ...
        foreach ($TagLst as $i => $Tag) {
            ...
            unset($LocO);
            $LocO = clsTinyButStrong370::f_Xml_FindTag(
                $Txt,
                $Tag,
                true,
                $Pos,
        ...
        if ($Move) {
            return clsTinyButStrong370::f_Xml_AttMove($Txt, $Loc, $AttDelim);
        }
        return true;
    }
...
*/
    ...
    public function LoadTemplate($File, $Charset = '')
    {
        ...
        if ($Ok !== false) {
            if (!is_null($File)) {
                $x = '';
                if (!$this->f_Misc_GetFile($x, $File, $this->_LastFile)) {
    ...
    function meth_Locator_FindTbs(&$Txt, $Name, $Pos, $ChrSub)
    {
        ...
        do {
            ...
            if ($Pos === false) {
                ...
                $Loc->PosBeg = $Pos;
                if ($ReadPrm) {
                    clsTinyButStrong370::f_Loc_PrmRead(
                        $Txt,
                        $PosX,
                        false,
                        '\'',
    ...
    function meth_Locator_SectionAddGrp(&$LocR, $BlockName, &$BDef, $Type, $Field, $Prm)
    {
        ...
        if (strpos($Field, $this->_ChrOpen) === false) {
            $Field = $this->_ChrOpen . $BlockName . '.' . $Field . $this->_ChrClose;
        }
        $BDef->FDef = &$this->meth_Locator_SectionNewBDef($LocR, $BlockName, $Field, []);
            ...
            for ($i = $SubStart; $i < $Loc->SubNbr; $i++) {
                ...
                if (is_array($Value)) {
                    ...
                    if (method_exists($Value, $x)) {
                        if ($this->MethodsAllowed or !in_array(
                                strtok($Loc->FullName, '.'),
                                ['onload', 'onshow', 'var']
                            )
                        ) {
            ...
            foreach ($Loc->OpeAct as $i => $ope) {
                switch ($ope) {
                    ...
                    case 13:
                        ...
                        break;
                    case 14:
                        $CurrVal = clsTinyButStrong370::f_Loc_AttBoolean(
                            $CurrVal,
                            $Loc->PrmLst['atttrue'],
                            $Loc->AttName
                        );
                ...
                if (isset($Loc->PrmLst['.'])) {
                    ...
                    if ($Loc->PrmLst['magnet'] === '#') {
                        if (isset($Loc->AttBeg)) {
                            ...
                            $this->meth_Misc_Alert(
                                $Loc,
                                'parameter \'magnet=#\' cannot be processed because parameter \'att\' is not set or the attribute is not found.',
                                true
                            );
                        }
    ...
    function meth_Locator_FindBlockLst(&$Txt, $BlockName, $Pos, $SpePrm)
    {
            ...
            if ($Loc === false) {
                ...
                if ($Loc->PosDefBeg >= 0) {
                    ...
                    foreach ($dynprm as $dp) {
                        $n = 0;
                        if ((isset($Loc->PrmLst[$dp])) and (strpos(
                                    $Loc->PrmLst[$dp],
                ...
                if (isset($Loc->PrmLst['nodata'])) { // Nodata section
                    ...
                    $BDef->WhenCond = &$this->meth_Locator_SectionNewBDef($LocR, $BlockName, $Loc->PrmLst['when'], []);
        ...
        if ($LocR->WhenFound and ($LocR->SectionNbr === 0)) {
            ...
            $BDef = &$this->meth_Locator_SectionNewBDef($LocR, $BlockName, '', []);
            $LocR->SectionNbr = 1;
    ...
    function meth_Merge_FieldOutside(&$Txt, &$CurrRec, $RecNum, $PosMax)
    {
        ...
        do {
            ...
            if ($Loc !== false) {
                if (($PosMax !== false) and ($Loc->PosEnd > $PosMax)) {
                    return;
                }
                if ($Loc->SubName === '#') {
    ...
    function meth_Misc_Assign($Name, &$ArgLst, $CallingMeth)
    {
        ...
        if (($CallingMeth !== false) and (strcasecmp($CallingMeth, $meth) != 0)) {
            return $this->meth_Misc_Alert(
                'with ' . $CallingMeth . '() method',
        ...
        if (strpos($FrmStr, '|') !== false) {
            ...
            if (($nDecOk) and ($nPos >= 5)) {
                if ((substr(
                            ...
                            $nPos - 3,
                            3
                        ) === '000') and ($FrmStr[$nPos - 4] !== '0') and ($FrmStr[$nPos - 5] === '0')
                ) {
                    $nPos = $nPos - 4;
                    $nThsSep = $FrmStr[$nPos];
                }
            ...
            $nLen = $nPosEnd - $nPos;
            if (($nThsSep === '') and ($nLen > ($nDecNbr + $nPadZ + 1))) {
                $nPad = $nLen - $nPadZ;
            }
            // Percent
            $nPerCent = (strpos($FrmStr, '%') === false) ? false : true;
            $this->_FormatLst[$FrmStr] = [
                'type'    => 'num',
                'Str'     => $FrmStr,
                'Pos'     => ($nPos + 1),
                'Len'     => $nLen,
            ...
            if ($Cnt > 0) {
                ...
                $this->_FormatLst[$FrmStr] = ['type' => 'else', 'string' => $FrmStr];
            }
        }
        if ($Alias != '') {
            $this->_FormatLst[$Alias] = &$this->_FormatLst[$FrmStr];
        }
        return $this->_FormatLst[$FrmStr];
    }
}
